...
def test_kwargs_match_signature() -> None:
    """Assert the option declarations agree with `ak.any`'s parameters."""
...
def st_kwargs(draw: st.DrawFn, array: ak.Array) -> Kwargs:
    """Strategy for options of `ak.any` on `array`.
...
def test_properties(data: st.DataObject) -> None:
    """Assert `ak.any` does not raise on a draw expected to succeed."""
...
def _should_not_raise(form: ak.forms.Form, kwargs: Kwargs) -> bool:
    ...
    are drawn, never affect whether `ak.any` raises.

    The rules, written against `ak.any`'s deliberate error paths
    (records, irreducible unions, axis beyond depth):

    - A form `ak.any` refuses to run on (see `_reducible`): never
    ...
    - On an accepted form: `axis=None` (reduce everything) always
      ...
      reducible leaf — `ak.any` runs over its characters, and
...
def _reducible(form: ak.forms.Form) -> bool:
    """Return `True` if `ak.any` runs on this form at every axis it has.

    A record anywhere makes `ak.any` raise at every axis, `None` and
