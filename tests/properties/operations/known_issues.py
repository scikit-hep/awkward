...
from tests.properties.operations import util
...
def has_issue_4261(a: ak.Array) -> bool:
    ...
    family that NumPy itself cannot convert is refused by design
    (`util.merges_unconvertible_temporal_units`), not by this
    trigger. Unprobed combinations may still fail the tests; such a
    failure indicates this trigger is missing a combination.
...
def has_issue_4280(a: ak.Array) -> bool:
    ...
    `NaT` converts safely. The dtype pair alone stays under the
    factor bound of `util.merges_unconvertible_temporal_units`, so
    the stored values decide.
    ...
    while stack:
        ...
        if node.is_record or node.is_union:
            ...
            attos = util.ATTOSECONDS_PER_TIME_UNIT.get(unit)
