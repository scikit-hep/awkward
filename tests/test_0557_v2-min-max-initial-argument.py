...
def test():
    ...
    assert data.typetracer.min(axis=1, initial=4).form == data.min(axis=1, initial=4).form
    ...
    assert data.typetracer.max(axis=1, initial=4).form == data.max(axis=1, initial=4).form
