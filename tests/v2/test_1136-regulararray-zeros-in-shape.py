...
def test_actual():
    x = ak._v2.operations.convert.from_numpy(np.arange(2 * 3 * 4, dtype=np.int64).reshape(2, 3, 4))
    s = x[..., :0]
    result = ak._v2.operations.structure.zip({"q": s, "t": s})
    if not ak._util.py27 and not ak._util.py35:
        assert str(ak._v2.operations.describe.type(result)) == "2 * 3 * 0 * {q: int64, t: int64}"
