...
def test():
    ...
    assert ak._v2.operations.convert.to_pandas(simple)["values"].values.tolist() == [
        0.0,
        1.1,
        2.2,
        3.3,
...
def test_broken():
    ...
    p4 = ak._v2.operations.structure.zip({"x": ex})
    p4c = ak._v2.operations.structure.cartesian({"a": p4, "b": p4})
    df = ak._v2.operations.convert.to_pandas(p4c)
...
def test_union_to_record():
    ...
    df_unionarray = ak._v2.operations.convert.to_pandas(unionarray)
    ...
    df_unionarray2 = ak._v2.operations.convert.to_pandas(unionarray2)
    np.testing.assert_array_equal(
