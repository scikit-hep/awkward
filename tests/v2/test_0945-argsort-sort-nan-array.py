...
def test_nan():
    ...
    assert str(ak._v2.operations.structure.sort(array).tolist()) == "[nan, nan, 0.0, 1.0, 2.0, 3.0]"


def test_bool():
    array = ak._v2.Array([True, False, False, True, True, True])

    assert ak._v2.operations.structure.argsort(array).tolist() == [1, 2, 0, 3, 4, 5]
    assert ak._v2.operations.structure.sort(array).tolist() == [False, False, True, True, True, True]


def test_argsort():
    array = ak._v2.Array([1, 2, None, 3, 0, None])
    assert ak._v2.operations.structure.argsort(array).tolist() == [4, 0, 1, 3, 2, 5]
