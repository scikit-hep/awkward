...
def test_from_iter():
    assert ak._v2.operations.from_iter([1 + 1j, 2 + 2j, 3 + 3j]).tolist() == [
        1 + 1j,
        2 + 2j,
        3 + 3j,
    ]
    ...
    assert ak._v2.operations.from_iter([1, 2.2, 3 + 3j]).tolist() == [
        1.0 + 0j,
        2.2 + 0j,
        3.0 + 3j,
    ]
    assert ak._v2.operations.from_iter([1, 3 + 3j]).tolist() == [
        1.0 + 0j,
        3.0 + 3j,
    ]
    ...
    # into reals:
    assert str(ak._v2.operations.from_iter([1, 2.2]).type) == "2 * float64"
    assert ak._v2.operations.from_iter([1, 2.2]).tolist() == [1.0, 2.2]
...
def test_from_json():
    array = ak._v2.operations.from_json('[{"r": 1.1, "i": 1.0}, {"r": 2.2, "i": 2.0}]')
    assert array.tolist() == [
        {"r": 1.1, "i": 1.0},
        {"r": 2.2, "i": 2.0},
    ]
    ...
    assert ak._v2.operations.from_json(
...
def test_to_json():
    ...
    with pytest.raises(TypeError) as err:
        ak._v2.operations.to_json(ak._v2.operations.from_iter([1 + 1j, 2 + 2j, 3 + 3j]))
    ...
    assert expectation == json.loads(
        ak._v2.operations.to_json(
            ak._v2.operations.from_iter([1 + 1j, 2 + 2j, 3 + 3j]),
            complex_record_fields=("r", "i"),
        )
    )
    expectation = [
...
def test_reducers():
    ...
    assert ak._v2.operations.sum(
        ak._v2.operations.from_iter([[1 + 1j, 2 + 2j], [], [3 + 3j]]), axis=1
    ).tolist() == [
        3 + 3j,
        0 + 0j,
        3 + 3j,
    ]
    assert ak._v2.operations.prod(
        ak._v2.operations.from_iter([[1 + 1j, 2 + 2j], [], [3 + 3j]]), axis=1
    ).tolist() == [
        0 + 4j,
        1 + 0j,
    ...
    assert ak._v2.operations.count(
        ak._v2.operations.from_iter([[1 + 1j, 2 + 2j], [], [3 + 3j]]), axis=1
    ).tolist() == [2, 0, 1]
    assert ak._v2.operations.count_nonzero(
        ak._v2.operations.from_iter([[1 + 1j, 2 + 2j], [], [3 + 3j]]), axis=1
    ).tolist() == [2, 0, 1]
    assert ak._v2.operations.any(
        ak._v2.operations.from_iter([[1 + 1j, 2 + 2j], [], [3 + 3j]]), axis=1
    ).tolist() == [
        True,
        False,
    ...
    assert ak._v2.operations.any(
        ak._v2.operations.from_iter([[1 + 1j, 2 + 2j, 0 + 0j], [], [3 + 3j]]),
        axis=1,
...
def test_minmax():
    assert (
        ak._v2.operations.min(
            ak._v2.operations.from_iter([[1 + 5j, 2 + 4j], [], [3 + 3j]])
        )
        == 1 + 5j
    )
    assert (
        ak._v2.operations.max(
            ak._v2.operations.from_iter([[1 + 5j, 2 + 4j], [], [3 + 3j]])
        )
        == 3 + 3j
    )
    ...
    assert ak._v2.operations.min(
        ak._v2.operations.from_iter([[1 + 5j, 2 + 4j], [], [3 + 3j]]), axis=1
    ).tolist() == [
        1 + 5j,
        None,
        3 + 3j,
    ]
    assert ak._v2.operations.max(
        ak._v2.operations.from_iter([[1 + 5j, 2 + 4j], [], [3 + 3j]]), axis=1
    ).tolist() == [
        2 + 4j,
        None,
    ...
    assert ak._v2.operations.argmax(
        ak._v2.operations.from_iter([[1 + 5j, 2 + 4j], [], [3 + 3j]]), axis=1
    ).tolist() == [1, None, 0]
...
def test_sort():
    assert ak._v2.operations.sort(
        ak._v2.operations.from_iter([[2 + 4j, 1 + 5j], [], [3 + 3j]])
    ).tolist() == [
        [1 + 5j, 2 + 4j],
        [],
        [3 + 3j],
...
def test_numpy():
    assert np.array_equal(
        ak._v2.operations.to_numpy(
            ak._v2.operations.from_iter([[1 + 1j, 2 + 2j], [3 + 3j, 4 + 4j]])
        ),
