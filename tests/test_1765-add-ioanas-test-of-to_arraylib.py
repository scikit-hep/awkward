...
def test():
    ...
    assert [d.isoformat() for d in ak._util.to_arraylib(np, c, False).tolist()] == ['1970-01-01T00:00:00', '2020-01-01T00:00:00']

    recordarray = ak.contents.RecordArray([ak.contents.NumpyArray(np.array([1, 2, 3, 4, 5], dtype=np.int64))],fields=["one"])
    assert ak._util.to_arraylib(np, recordarray, False).tolist() == [(1,), (2,), (3,), (4,), (5,)]
