...
def test_unfinished_fragment_exception():
    ...
    with pytest.raises(ValueError):
        awkward.from_json(strs0)

    strs1 = """{"one": 1,
        "two": 2.2,"""
    with pytest.raises(ValueError):
        awkward.from_json(strs1)

    strs2 = """{"one": 1,
        "two": 2.2,
        """
    with pytest.raises(ValueError):
        awkward.from_json(strs2)

    strs3 = """{"one": 1, "two": 2.2, "three": "THREE"}
        {"one": 10, "two": 22,"""
    with pytest.raises(ValueError):
        awkward.from_json(strs3)

    strs4 = """{"one": 1, "two": 2.2, "three": "THREE"}
        {"one": 10, "two": 22,
        """
    with pytest.raises(ValueError):
        awkward.from_json(strs4)

    strs5 = """["one", "two","""
    with pytest.raises(ValueError):
        awkward.from_json(strs5)

    strs6 = """["one",
        "two","""
    with pytest.raises(ValueError):
        awkward.from_json(strs6)

    strs7 = """["one",
        "two",
        """
    with pytest.raises(ValueError):
        awkward.from_json(strs7)

def test_two_arrays():

    str = """{"one": 1, "two": 2.2}{"one": 10, "two": 22}"""
    array = awkward.from_json(str)
    assert awkward.to_list(array) == [
    ...
    array = awkward.from_json('tests/samples/test-two-arrays.json')
    assert awkward.to_list(array) == [
        {'one': 1, 'two': 2.2}, {'one': 10, 'two': 22.0},
        {'one': 1, 'two': 2.2}, {'one': 10, 'two': 22.0},
        {'one': 1, 'two': 2.2}, {'one': 10, 'two': 22.0},
...
def test_blanc_lines():
    ...
    3   """
    array = awkward.from_json(str)
    assert awkward.to_list(array) == [1, 2, 3]
...
def test_tostring():
    ...
    assert awkward.to_json(array) == '[{"x":1.1,"y":[]},{"x":2.2,"y":[1]},{"x":3.3,"y":[1,2]},{"x":4.4,"y":[1,2,3]},{"x":5.5,"y":[1,2,3,4]},{"x":6.6,"y":[1,2,3,4,5]}]'
...
def test_array_tojson():
    ...
    array = awkward.layout.NumpyArray(numpy.array([
        [float('nan'), float('nan'), 1.1],
        [float('inf'), 3.3, float('-inf')]]))

    assert awkward.to_json(array, nan_string='NaN', infinity_string='inf',
        minus_infinity_string='-inf') == '[["NaN","NaN",1.1],["inf",3.3,"-inf"]]'

    array2 = awkward.Array([[0, 2], None, None, None, 'NaN', 'NaN'])
    assert awkward.to_json(array2, nan_string='NaN') == '[[0,2],null,null,null,"NaN","NaN"]'

def test_fromfile():
    # read multiple json fragments from a json file
    array = awkward.from_json('tests/samples/test-record-array.json')
    assert awkward.to_list(array) == [
        {'x': 1.1, 'y': []},
        {'x': 2.2, 'y': [1]},
        {'x': 3.3, 'y': [1, 2]},
    ...
    array = awkward.from_json('tests/samples/test.json',
        infinity_string='inf', minus_infinity_string='-inf')

    assert awkward.to_list(array) == [1.1, 2.2, 3.3, float('inf'), float('-inf'),
    ...
    assert awkward.to_list(array) == [1.1, 2.2, 3.3, 'inf', '-inf',
        [4.4, 5.5, 6.6, 7.7, 8.8, 9.9, 10.1, 11.11],
        [12.12, 13.13, 14.14, 15.15, 16.16, 17.17],
        [[[18.18,   19.19,   20.2,    21.21,   22.22],
          [23.23,   24.24,   25.25,   26.26,   27.27,
    ...
    assert fix(awkward.to_list(array)) == fix([1.1, 2.2, 3.3, float("inf"), float("-inf"),
        [4.4, 5.5, 6.6, 7.7, 8.8, 9.9, 10.1, 11.11],
        [12.12, 13.13, 14.14, 15.15, 16.16, 17.17],
        [[[18.18,   19.19,   20.2,    21.21,   22.22],
          [23.23,   24.24,   25.25,   26.26,   27.27,
    ...
    array = awkward.from_json('tests/samples/test-nan-inf.json',
...
def test_three():
    array = awkward.from_json('["one", \n"two"] \n ["three"]')
    assert awkward.to_list(array) == [['one', 'two'], ['three']]
