...
def test_0459_types():
    ...
    assert ak.parameters(array_isdoc) == {"__array__": "zoinks", "__doc__": "This is a zoink."}
...
def test_0459():
    ...
    assert ak.parameters(ak.concatenate([array_plain, array_plain])) == {"__array__": "zoinks"}
    assert ak.parameters(ak.concatenate([plain_isdoc, plain_isdoc])) == {"__doc__": "This is a zoink."}
    assert ak.parameters(ak.concatenate([array_isdoc, array_isdoc])) == {"__array__": "zoinks", "__doc__": "This is a zoink."}

    assert isinstance(ak.concatenate([plain_plain, plain_plain]).layout, ak.layout.NumpyArray)
    assert isinstance(ak.concatenate([array_plain, array_plain]).layout, ak.layout.NumpyArray)
    assert isinstance(ak.concatenate([plain_isdoc, plain_isdoc]).layout, ak.layout.NumpyArray)
    assert isinstance(ak.concatenate([array_isdoc, array_isdoc]).layout, ak.layout.NumpyArray)
    ...
    assert isinstance(ak.concatenate([plain_plain, array_plain]).layout, ak.layout.UnionArray8_64)
    assert isinstance(ak.concatenate([plain_isdoc, array_isdoc]).layout, ak.layout.UnionArray8_64)
    assert isinstance(ak.concatenate([array_plain, plain_plain]).layout, ak.layout.UnionArray8_64)
    assert isinstance(ak.concatenate([array_isdoc, plain_isdoc]).layout, ak.layout.UnionArray8_64)

    assert ak.parameters(ak.concatenate([plain_plain, plain_isdoc])) == {}
    assert ak.parameters(ak.concatenate([array_plain, array_isdoc])) == {"__array__": "zoinks"}
    assert ak.parameters(ak.concatenate([plain_isdoc, plain_plain])) == {}
    assert ak.parameters(ak.concatenate([array_isdoc, array_plain])) == {"__array__": "zoinks"}

    assert isinstance(ak.concatenate([plain_plain, plain_isdoc]).layout, ak.layout.NumpyArray)
    assert isinstance(ak.concatenate([array_plain, array_isdoc]).layout, ak.layout.NumpyArray)
    assert isinstance(ak.concatenate([plain_isdoc, plain_plain]).layout, ak.layout.NumpyArray)
    assert isinstance(ak.concatenate([array_isdoc, array_plain]).layout, ak.layout.NumpyArray)
...
def test_0522():
    ...
    assert (unionarray + np.arange(0, 100, 10)).tolist() == [0.0, 11.1, 22.2, 30, 140, 53.3, 64.4, 270, 380, 490]
    assert (np.arange(0, 100, 10) + unionarray).tolist() == [0.0, 11.1, 22.2, 30, 140, 53.3, 64.4, 270, 380, 490]

    assert (unionarray + ak.Array(np.arange(0, 100, 10))).tolist() == [0.0, 11.1, 22.2, 30, 140, 53.3, 64.4, 270, 380, 490]
    assert (ak.Array(np.arange(0, 100, 10)) + unionarray).tolist() == [0.0, 11.1, 22.2, 30, 140, 53.3, 64.4, 270, 380, 490]
