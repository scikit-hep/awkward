...
def test_sum_axis_none_non_empty(axis):
    array = ak.Array([1, 2, 3, 4, 5], backend="jax")
    assert ak.sum(array, axis=axis) == 15

    array = ak.Array([1.0, 2.0, 3.0], backend="jax")
    assert ak.sum(array, axis=axis) == 6.0


@pytest.mark.parametrize("axis", [None, 0, -1])
def test_sum_axis_none_empty(axis):
    array = ak.Array(np.array([], dtype=np.int64), backend="jax")
    assert ak.sum(array, axis=axis) == 0

    array = ak.Array(np.array([], dtype=np.float64), backend="jax")
    assert ak.sum(array, axis=axis) == 0.0


@pytest.mark.parametrize("axis", [None, 0, -1])
def test_min_axis_none_non_empty(axis):
    int_array = ak.Array([3, 1, 4, 1, 5, 9, 2, 6], backend="jax")
    float_array = ak.Array([3.0, 1.0, 4.0, 1.0, 5.0, 9.0, 2.0, 6.0], backend="jax")
...
def test_min_axis_none_empty(axis):
    int_array = ak.Array(np.array([], dtype=np.int64), backend="jax")
    float_array = ak.Array(np.array([], dtype=np.float64), backend="jax")
