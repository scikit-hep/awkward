...
def test_tuple():
    ...
    assert ak.testing.arrays_approx_equal(result, np.ravel_multi_index(data, (2, 2)))
...
def test_array():
    ...
    assert isinstance(result, ak.Array)
    assert ak.testing.arrays_approx_equal(result, np.searchsorted(haystack, needle))
...
def test_scalar():
    ...
    assert isinstance(result, ak.Array)
    assert ak.testing.arrays_approx_equal(result, np.partition(data, 4))
...
def test_tuple_of_array():
    ...
    assert isinstance(result, ak.Array)
    assert ak.testing.arrays_approx_equal(result, np.lexsort(data))
