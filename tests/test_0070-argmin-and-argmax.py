...
def test_3d():
    ...
    assert ak.to_list(array.argmin(axis=0)) == [
        ...
        [0, 1, 0, 1, 0],
        [0, 0, 0, 0, 1]]
    assert ak.to_list(array.argmin(axis=1)) == [
        [2, 2, 1, 0, 0],
        [0, 1, 0, 1, 2]]
...
def test_jagged():
    array = ak.from_iter([[2.2, 1.1, 3.3], [], [4.4, 5.5], [5.5], [-4.4, -5.5, -6.6]], highlevel=False)
...
def test_missing():
    array = ak.from_iter([[[2.2, 1.1, 3.3]], [[]], [None, None, None], [[-4.4, -5.5, -6.6]]], highlevel=False)
