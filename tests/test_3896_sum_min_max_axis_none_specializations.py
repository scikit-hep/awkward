...
def test_sum_axis_none_typetracer(forget_length, axis):
    array = ak.Array([1, 2, 3, 4, 5])
    tt = ak.Array(array.layout.to_typetracer(forget_length=forget_length))

    result = ak.sum(tt, axis=axis)
...
@pytest.mark.parametrize("axis", [None, 0, -1])
@pytest.mark.parametrize("forget_length", [False, True])
def test_min_axis_none_typetracer(forget_length, axis):
    array = ak.Array([3, 1, 4, 1, 5])
    tt = ak.Array(array.layout.to_typetracer(forget_length=forget_length))

    # mask_identity=True (default) → MaybeNone
    result = ak.min(tt, axis=axis)
    ...
    result = ak.min(tt, axis=axis, mask_identity=False)
    assert not isinstance(result, MaybeNone)
    assert is_unknown_scalar(result)
    assert result.dtype == np.int64

    # initial with mask_identity=True → MaybeNone
    result = ak.min(tt, axis=axis, initial=0)
    assert isinstance(result, MaybeNone)
    assert is_unknown_scalar(result.content)
    assert result.content.dtype == np.int64

    # initial with mask_identity=False → unknown scalar, not MaybeNone
    result = ak.min(tt, axis=axis, initial=0, mask_identity=False)
    assert not isinstance(result, MaybeNone)
    assert is_unknown_scalar(result)
    assert result.dtype == np.int64
...
@pytest.mark.parametrize("axis", [None, 0, -1])
@pytest.mark.parametrize("forget_length", [False, True])
def test_max_axis_none_typetracer(forget_length, axis):
    array = ak.Array([3, 1, 4, 1, 5])
    tt = ak.Array(array.layout.to_typetracer(forget_length=forget_length))

    # mask_identity=True (default) → MaybeNone
    result = ak.max(tt, axis=axis)
    assert isinstance(result, MaybeNone)
    assert is_unknown_scalar(result.content)
    assert result.content.dtype == np.int64

    # mask_identity=False → unknown scalar, not MaybeNone
    result = ak.max(tt, axis=axis, mask_identity=False)
    assert not isinstance(result, MaybeNone)
    assert is_unknown_scalar(result)
    assert result.dtype == np.int64

    # initial with mask_identity=True → MaybeNone
    result = ak.max(tt, axis=axis, initial=10)
    assert isinstance(result, MaybeNone)
    assert is_unknown_scalar(result.content)
    assert result.content.dtype == np.int64

    # initial with mask_identity=False → unknown scalar, not MaybeNone
    result = ak.max(tt, axis=axis, initial=10, mask_identity=False)
    assert not isinstance(result, MaybeNone)
    assert is_unknown_scalar(result)
    assert result.dtype == np.int64
