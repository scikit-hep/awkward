...
def test_flat():
    ...
    xs = awkward1.Array(numpy.linspace(-3, 3, 10), checkvalid=True)
...
def test_jagged():
    grad_parabola = awkward1.autograd(parabola)

    array = awkward1.Array([[1.0, 2.0, 3.0], [], [4.0, None, 5.0]], checkvalid=True)

    assert awkward1.tolist(array) == [[1.0, 2.0, 3.0], [], [4.0, None, 5.0]]
    assert awkward1.tolist(parabola(array)) == [[3.0, 12.0, 27.0], [], [48.0, None, 75.0]]
    assert awkward1.tolist(grad_parabola(array)) == [[6.0, 12.0, 18.0], [], [24.0, None, 30.0]]

def test_record():
    grad_parabola = awkward1.autograd(parabola)

    array = awkward1.Array([{"x": 0.0, "y": []}, {"x": 1.0, "y": [1.0]}, None, {"x": 2.0, "y": [1.0, None, 2.0]}], checkvalid=True)

    assert awkward1.tolist(array) == [{"x": 0.0, "y": []}, {"x": 1.0, "y": [1.0]}, None, {"x": 2.0, "y": [1.0, None, 2.0]}]
    assert awkward1.tolist(parabola(array)) == [{"x": 0.0, "y": []}, {"x": 3.0, "y": [3.0]}, None, {"x": 12.0, "y": [3.0, None, 12.0]}]
    assert awkward1.tolist(grad_parabola(array)) == [{"x": 0.0, "y": []}, {"x": 6.0, "y": [6.0]}, None, {"x": 12.0, "y": [6.0, None, 12.0]}]
