...
def test_different_categories():
    ...
    index1 = ak.layout.Index64(np.array([0, 2, 2, 1, 2, 0, 1, 0], dtype=np.int64))
    index2 = ak.layout.Index64(np.array([1, 1, 2, 1, 0, 0, 0, 1], dtype=np.int64))
    categorical1 = ak.layout.IndexedArray64(index1, categories1.layout, parameters={"__array__": "categorical"})
    categorical2 = ak.layout.IndexedArray64(index2, categories2.layout, parameters={"__array__": "categorical"})
    array1 = ak.Array(categorical1)
    array2 = ak.Array(categorical2)
...
def test_option_same_categories():
    ...
    categorical1 = ak.layout.IndexedOptionArray64(index1, categories.layout, parameters={"__array__": "categorical"})
    categorical2 = ak.layout.IndexedOptionArray64(index2, categories.layout, parameters={"__array__": "categorical"})
    array1 = ak.Array(categorical1)
    array2 = ak.Array(categorical2)
    assert (array1 == array2).tolist() == [False, False, True, True, True, False, False, False]
...
def test_option_different_categories():
    ...
    categorical1 = ak.layout.IndexedOptionArray64(index1, categories1.layout, parameters={"__array__": "categorical"})
    categorical2 = ak.layout.IndexedOptionArray64(index2, categories2.layout, parameters={"__array__": "categorical"})
    array1 = ak.Array(categorical1)
    array2 = ak.Array(categorical2)
...
def test_to_categorical():
    array = ak.Array(["one", "two", "three", "one", "two", "three", "one", "two", "three"])
    assert not ak.is_categorical(array)
    categorical = ak.to_categorical(array)
    assert ak.is_categorical(categorical)
    assert ak.to_list(array) == categorical.tolist()
...
def test_to_categorical_masked():
    content = ak.Array(["one", "two", "three", "one", "one", "two", "three", "two", "one", "two", "three", "three"]).layout
    mask = ak.layout.Index8(np.array([False, False, False, True, False, False, False, True, False, False, False, True]))
...
def test_to_categorical_masked():
    content = ak.Array(["one", "two", "three", "one", "one", "two", "three", "two"]).layout
    index = ak.layout.Index64(np.array([0, 1, 2, 3, 4, 5, 6, 7, 0, 1, 2, 3], dtype=np.int64))
...
def test_typestr():
    if not ak._util.py27:
        assert str(ak.type(ak.to_categorical(ak.Array([1.1, 2.2, 2.2, 3.3])))) == "4 * categorical[type=float64]"
        assert str(ak.type(ak.to_categorical(ak.Array([1.1, 2.2, None, 2.2, 3.3])))) == "5 * categorical[type=?float64]"
        assert str(ak.type(ak.to_categorical(ak.Array(["one", "two", "two", "three"])))) == "4 * categorical[type=string]"
        assert str(ak.type(ak.to_categorical(ak.Array(["one", "two", None, "two", "three"])))) == "5 * categorical[type=option[string]]"


def test_zip():
    x = ak.Array([1.1, 2.2, 3.3])
    y = ak.Array(["one", "two", "three"])
    assert ak.zip({"x": x, "y": y}).tolist() == [{"x": 1.1, "y": "one"}, {"x": 2.2, "y": "two"}, {"x": 3.3, "y": "three"}]
