...
Building `awkward-cpp` requires generated code and datafiles (kernel specification, header-only includes). These are generated automatically when `awkward-cpp` is built from the repository — CMake runs the generation scripts at configure time — so no separate preparation step is needed to build the packages.
...
This is useful e.g. for generating the kernel documentation or the kernel test suites without compiling the C++ package. The `prepare` session accepts flags to specify exact generation targets:
```bash
...
```bash
python -m pip install "scikit-build-core[pyproject,color]" pybind11 ninja cmake pyyaml numpy
```
...
These test suites are generated by the `nox -s prepare` session's `--tests` target.
