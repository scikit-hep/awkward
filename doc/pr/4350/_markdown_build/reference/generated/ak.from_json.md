...
Defined in [awkward.operations.ak_from_json](https://github.com/scikit-hep/awkward/blob/eb8319fc07e72e9c77dc6c53b77980ef89c43fe4/src/awkward/operations/ak_from_json.py) on [line 28](https://github.com/scikit-hep/awkward/blob/eb8319fc07e72e9c77dc6c53b77980ef89c43fe4/src/awkward/operations/ak_from_json.py#L28).
...
* `"type": "string"` → UTF-8 encoded strings. All JSON escape sequences are
  ...
  as categorical values ([`ak.contents.IndexedArray`](sphinx-llm:0ec798a3207a4746a193b3e535c37f41#ak.contents.IndexedArray) or [`ak.contents.IndexedOptionArray`](sphinx-llm:3f1e6f3c5e154ca39fcd977247e4a221#ak.contents.IndexedOptionArray)).
* `"type": "array"` → nested lists. The `"items"` must be specified. If
  `"minItems"` and `"maxItems"` are specified and equal to each other, the
  list has regular-type ([`ak.types.RegularType`](sphinx-llm:a729849acf504584b9a7d4ae728c6263#ak.types.RegularType)); otherwise, it has variable-length
  type ([`ak.types.ListType`](sphinx-llm:649f7f37622a4fbc999e70526300cf12#ak.types.ListType)).
...
See also [`ak.to_json`](sphinx-llm:7af8124fc8d14a3caf95668155d7904d#ak.to_json).
...
* **type source:**
  bytes/str, pathlib.Path, or file-like object
* **param line_delimited:**
  If False, a single JSON document is read as an
...
* **type schema:**
  None, JSON str or equivalent lists/dicts
* **param nan_string:**
  If not None, strings with this value will be
  interpreted as floating-point NaN values.
* **type nan_string:**
  None or str
* **param posinf_string:**
  If not None, strings with this value will
  be interpreted as floating-point positive infinity values.
* **type posinf_string:**
  None or str
* **param neginf_string:**
  If not None, strings with this value
  will be interpreted as floating-point negative infinity values.
* **type neginf_string:**
  None or str
* **param complex_record_fields:**
  If not None, defines a pair of
  field names to interpret 2-field records as complex numbers.
* **type complex_record_fields:**
  None or (str, str)
* **param buffersize:**
  Number of bytes in each read from source: larger
  values use more memory but read less frequently. (Python GIL is
  released before and after read events.)
* **type buffersize:**
  int
* **param initial:**
  Initial size (in bytes) of buffers used by the `ak::ArrayBuilder`.
* **type initial:**
  int
* **param resize:**
  Resize multiplier for buffers used by the `ak::ArrayBuilder`;
  should be strictly greater than 1.
* **type resize:**
  float
* **param highlevel:**
  If True, return an [`ak.Array`](sphinx-llm:c265a53f66b3419cb9df4fa097ec719d#ak.Array); otherwise, return
  a low-level [`ak.contents.Content`](sphinx-llm:bda3fe6771b14645a594aa3bbe64f5f8#ak.contents.Content) subclass.
* **type highlevel:**
  bool
* **param behavior:**
  Custom [`ak.behavior`](sphinx-llm:4c349445204c475eb8c2285d838e2099#ak.behavior) for the output array, if
  high-level.
* **type behavior:**
  None or dict
* **param attrs:**
  Custom attributes for the output array, if
  high-level.
* **type attrs:**
  None or dict
* **returns:**
  An [`ak.Array`](sphinx-llm:c265a53f66b3419cb9df4fa097ec719d#ak.Array) read from the given JSON (string, bytes, file, or URL).
...
[`ak.from_iter`](sphinx-llm:40c924e275044a0993aef7f20508d933#ak.from_iter) interprets Python lists and Python dicts. It could be used as a
synonym for Python’s `json.loads` followed by [`ak.from_iter`](sphinx-llm:40c924e275044a0993aef7f20508d933#ak.from_iter), but the direct
...
As shown above, reading JSON may result in [`ak.Array`](sphinx-llm:c265a53f66b3419cb9df4fa097ec719d#ak.Array) or [`ak.Record`](sphinx-llm:e158df99f80043309c758f6e933352b2#ak.Record), but line-delimited
(`line_delimited=True`) only results in [`ak.Array`](sphinx-llm:c265a53f66b3419cb9df4fa097ec719d#ak.Array):
...
document, regardless of whether there’s only one of them (may be an [`ak.Record`](sphinx-llm:e158df99f80043309c758f6e933352b2#ak.Record))
or many of them (must be an [`ak.Array`](sphinx-llm:c265a53f66b3419cb9df4fa097ec719d#ak.Array)).
