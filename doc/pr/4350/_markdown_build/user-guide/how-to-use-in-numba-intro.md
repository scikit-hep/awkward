...
* The matching problem described in [How to find the best match between two collections using Cartesian (cross) product](sphinx-llm:5f600bdf539a4563b0605bf5b3afea86) is already rather complex—if a problem is more intricate than that, you may want to consider doing it in imperative code, so that you or anyone reading your code don’t get lost in indices.
...
```myst-ansi
35.9 ms ± 139 μs per loop (mean ± std. dev. of 7 runs, 10 loops each)
```
...
```myst-ansi
6.93 ms ± 14.5 μs per loop (mean ± std. dev. of 7 runs, 100 loops each)
```
...
The [next section](sphinx-llm:f1715bc5f13f4794a5af5a41008add0d) lists what you can and can’t do with Awkward Arrays in Numba-compiled code.
