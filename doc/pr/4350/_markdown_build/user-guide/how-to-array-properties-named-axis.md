...
Named axes can be attached to an array using the high-level [`ak.with_named_axis()`](sphinx-llm:850fa57157f54411a7f413a32c8bd96a#ak.with_named_axis) function.
Awkward Array allows strings as named axes and integers as positional axes.

The `named_axis` argument of [`ak.with_named_axis()`](sphinx-llm:850fa57157f54411a7f413a32c8bd96a#ak.with_named_axis) accepts either a `tuple` or `dict`:

- `tuple`:
  - `named axis`: item
  - `positional axis`: index of the item
  - *additional*: `None` represents a wildcard for not specifying a name, e.g.: `("x", None)` means that the first axis is named “x” and the second is not named.
- `dict`:
  - `named axis`: key
  - `positional axis`: value
  - *additional*: not specifying a name is not allowed, e.g.: `{"x": 0}` means that the first axis is named “x”, all other existing dimensions are unnamed. The `dict` option also allows for renaming negative axes, e.g.: `{"x": -1}` means that the last axis is named “x”.
...
If you want to remove the named axes from an array, you can use the [`ak.without_named_axis()`](sphinx-llm:30b66a6c5ad241b0b374a553292778a2#ak.without_named_axis) function:
...
```myst-ansi
classic indexing: [{event_no: 0, jetpt: 50}]
named indexing  : [{event_no: 0, jetpt: 50}]
```
...
```myst-ansi
Sum over axis 'x': [[13, 8], [7]]
Sum over axis 'y': [[1, 2], [3], [4], [12, 6]]
Sum over axis 'z': [[3], [3], [4], [11, 7]]
```
