...
[`ak.concatenate()`](sphinx-llm:9d688ff77559406881cf7aa931110693#ak.concatenate) is an analog of [np.concatenate](https://numpy.org/doc/stable/reference/generated/numpy.concatenate.html) (in fact, you can use [np.concatenate](https://numpy.org/doc/stable/reference/generated/numpy.concatenate.html) where you mean [`ak.concatenate()`](sphinx-llm:9d688ff77559406881cf7aa931110693#ak.concatenate)). However, it applies to data of arbitrary data structures:
...
Concatenation in Awkward Array combines arrays lengthwise: by adding the lengths of the arrays or adding the lengths of lists within an array. It does not refer to adding fields to a record (that is, “adding columns to a table”). To add fields to a record, see [`ak.zip()`](sphinx-llm:8d4df9d3d238429d957dd1efd0afa844#ak.zip) or [`ak.Array.__setitem__()`](sphinx-llm:c265a53f66b3419cb9df4fa097ec719d#ak.Array.__setitem__) in [how to zip/unzip and project](sphinx-llm:1e7f8d6b666b42acaeaba080d60b9027) and [how to add fields](sphinx-llm:35af87efb6334326b4911a658392607d). This is important to note because [pandas.concat](https://pandas.pydata.org/docs/reference/api/pandas.concat.html) does both, depending on its `axis` argument (and there’s no equivalent in NumPy).
...
[`ak.concatenate()`](sphinx-llm:9d688ff77559406881cf7aa931110693#ak.concatenate) accepts any `axis` less than the number of dimensions in the arrays, but Pandas has only two choices, `axis=0` and `axis=1`.

Fields (“columns”) of an Awkward Array are unrelated to array dimensions. If you want what [pandas.concat](https://pandas.pydata.org/docs/reference/api/pandas.concat.html) does with `axis=1`, you would use [`ak.zip()`](sphinx-llm:8d4df9d3d238429d957dd1efd0afa844#ak.zip):

```ipython3
ak.zip({"column1": array1.column, "column2": array2.column}, depth_limit=1)
```

The `depth_limit` prevents [`ak.zip()`](sphinx-llm:8d4df9d3d238429d957dd1efd0afa844#ak.zip) from interleaving the lists further:
