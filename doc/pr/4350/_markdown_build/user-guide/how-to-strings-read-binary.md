...
We can then ask Awkward Array to treat this array of lists of characters as an array of strings, using [`ak.enforce_type()`](sphinx-llm:ba952268b279401086f48d422c6fd7b9#ak.enforce_type)
...
The `__array__` parameter is special. It is reserved by Awkward Array, and signals that the layout is a special pre-undertood built-in type. In this case, that type of the outer [`ak.contents.ListOffsetArray`](sphinx-llm:00659f65177e4a1db8f94cec2f25984d#ak.contents.ListOffsetArray) is “string”. It can also be seen that the inner [`ak.contents.NumpyArray`](sphinx-llm:6f8cd60f60b04ce7a9499f2967197e79#ak.contents.NumpyArray) also has an `__array__` parameter, this time with a value of `char`. In Awkward Array, an array of strings *must* look like this layout; a list with the `__array__="string"` parameter wrapping a [`ak.contents.NumpyArray`](sphinx-llm:6f8cd60f60b04ce7a9499f2967197e79#ak.contents.NumpyArray) with the `__array__="char"` parameter.
...
This is reflected in the [`ak.num()`](sphinx-llm:137e3487aaa0453fa61d9c86fc644b7b#ak.num) function
...
Many of the functions in the `ak.str` module treat bytestrings and strings differently; in the latter case, strings are often manipulated in terms of code-points instead of code-units. Consider [`ak.str.length()`](sphinx-llm:dbce9ba23b1a4f48a1550ed179a58557#ak.str.length) for this array
