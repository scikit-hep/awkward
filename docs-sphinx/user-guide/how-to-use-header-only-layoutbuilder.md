...
```{code-cell}
  ...
  assert (error == "Record node0 has field \"two\" length 2 that differs from the first length 3\n");
```
...
A Layout Builder Form is a unique description of an Awkward Array and returns a JSON-like `std::string` and its form keys.
