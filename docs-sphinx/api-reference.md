...
**Pandas compatibility:** {doc}`ak.to_pandas` turns an Awkward Array into a list of DataFrames or joins them with [pd.merge](https://pandas.pydata.org/pandas-docs/version/1.0.3/reference/api/pandas.merge.html) if necessary.
...
**Layout nodes:** the high-level {doc}`_auto/ak.Array` and {doc}`_auto/ak.Record` types hide the tree-structure that build the array, but they can be accessed with [ak.Array.layout](_auto/ak.Array.html#ak-array-layout). This layout structure is the core of the library, but usually doesn't have to be accessed by data analysts.
