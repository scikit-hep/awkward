...
namespace awkward {
  ...
  class EXPORT_SYMBOL ForthMachineOf {
    ...
    using IndexTypeOf = typename std::vector<TYPE>::size_type;
  ...
  public:
    ...
    /// @brief HERE
    const std::string
    decompiled_segment(int64_t segment_position,
                       const std::string& indent = "",
                       bool endline = true) const;

    /// @brief HERE
    const std::string
    decompiled_at(int64_t bytecode_position,
    ...
    /// @brief HERE
    void
    maybe_throw(util::ForthError err,
  ...
  private:
    ...
    /// @brief HERE
    void
    internal_run(bool single_step,
                 int64_t recursion_target_depth_top);  // noexcept
