...
namespace {
  ...
  public:
    ...
    empty_buffer(const std::string& name, int64_t num_bytes) override {
      py::object pyarray =
          py::module::import("numpy").attr("empty")(num_bytes, "u1");
    ...
    full_buffer(const std::string& name,
      ...
      py::object pyarray = py::module::import("numpy").attr("full")(
          py::int_(length), py::int_(value), py::str(dtype));
      container_[py::str(name)] = pyarray;
    }
  ...
  public:
    void*
    empty_buffer(const std::string& /* name */,
                 int64_t /* num_bytes */) override {
      return nullptr;
    }
