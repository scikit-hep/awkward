...
#define FILENAME(line)                                                         \
  FILENAME_FOR_EXCEPTIONS_C("src/cpu-kernels/awkward_UnionArray_validity.cpp", \
                            line)
...
awkward_UnionArray_validity(const T* tags,
  ...
  for (int64_t i = 0; i < length; i++) {
    ...
    if (tag >= numcontents) {
      return failure(
          "tags[i] >= len(contents)", i, kSliceNone, FILENAME(__LINE__));
    }
    int64_t lencontent = lencontents[tag];
    if (idx >= lencontent) {
      return failure("index[i] >= len(content[tags[i]])",
                     i,
                     kSliceNone,
                     FILENAME(__LINE__));
    }
...
awkward_UnionArray8_32_validity(const int8_t* tags,
  ...
  return awkward_UnionArray_validity<int8_t, int32_t>(
      tags, index, length, numcontents, lencontents);
}
ERROR
