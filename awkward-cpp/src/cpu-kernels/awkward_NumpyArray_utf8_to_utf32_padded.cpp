...
#define FILENAME(line)       \
  FILENAME_FOR_EXCEPTIONS_C( \
      "src/cpu-kernels/awkward_NumpyArray_utf8_to_utf32_padded.cpp", line)

#include "awkward/kernels.h"
#include "awkward/unicode.h"

ERROR
awkward_NumpyArray_utf8_to_utf32_padded(const uint8_t *fromptr,
  ...
  for (auto k_sublist = 0; k_sublist < offsetslength - 1; k_sublist++) {
    ...
    for (auto j_code_unit_last = i_code_unit + n_code_units;
         i_code_unit < j_code_unit_last;) {
      // Parse a single codepoint
      code_point_width = utf8_codepoint_size(fromptr[i_code_unit]);
      switch (code_point_width) {
        case 1:
          toptr[n_code_point] =
              ((uint32_t)fromptr[i_code_unit] & ~UTF8_ONE_BYTE_MASK);
          break;
        case 2:
          toptr[n_code_point] =
        ...
        default:
          return failure(
              "could not convert UTF8 code point to UTF32: invalid byte in "
              "UTF8 string",
              kSliceNone,
              fromptr[i_code_unit],
              FILENAME(__LINE__));
