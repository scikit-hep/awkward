...
#define FILENAME(line)       \
  FILENAME_FOR_EXCEPTIONS_C( \
      "src/cpu-kernels/awkward_IndexedArray_flatten_none2empty.cpp", line)
...
awkward_IndexedArray_flatten_none2empty(T* outoffsets,
  ...
  for (int64_t i = 0; i < outindexlength; i++) {
    ...
    if (idx < 0) {
      ...
      return failure(
          "flattening offset out of range", i, kSliceNone, FILENAME(__LINE__));
    } else {
      T count = offsets[idx + 1] - offsets[idx];
