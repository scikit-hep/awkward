...
#define FILENAME(line)       \
  FILENAME_FOR_EXCEPTIONS_C( \
      "src/cpu-kernels/awkward_Index_nones_as_index.cpp", line)

#include "awkward/kernels.h"

template <typename T>
ERROR
awkward_Index_nones_as_index(T* toindex, int64_t length) {
...
ERROR
awkward_Index_nones_as_index_64(int64_t* toindex, int64_t length) {
  return awkward_Index_nones_as_index<int64_t>(toindex, length);
}
