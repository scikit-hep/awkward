...
#define FILENAME(line)       \
  FILENAME_FOR_EXCEPTIONS_C( \
      "src/cpu-kernels/awkward_ListArray_getitem_jagged_descend.cpp", line)

#include "awkward/kernels.h"

template <typename C, typename T>
ERROR
awkward_ListArray_getitem_jagged_descend(T* tooffsets,
                                         const T* slicestarts,
                                         const T* slicestops,
  ...
  for (int64_t i = 0; i < sliceouterlen; i++) {
    int64_t slicecount = (int64_t)(slicestops[i] - slicestarts[i]);
    int64_t count = (int64_t)(fromstops[i] - fromstarts[i]);
    if (slicecount != count) {
      return failure(
...
ERROR
awkward_ListArray32_getitem_jagged_descend_64(int64_t* tooffsets,
...
ERROR
awkward_ListArrayU32_getitem_jagged_descend_64(int64_t* tooffsets,
                                               const int64_t* slicestarts,
                                               const int64_t* slicestops,
                                               int64_t sliceouterlen,
...
ERROR
awkward_ListArray64_getitem_jagged_descend_64(int64_t* tooffsets,
                                              const int64_t* slicestarts,
                                              const int64_t* slicestops,
                                              int64_t sliceouterlen,
