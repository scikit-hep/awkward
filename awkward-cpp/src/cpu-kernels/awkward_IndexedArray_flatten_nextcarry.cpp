// BSD 3-Clause License; see https://github.com/scikit-hep/awkward/blob/main/LICENSE

#define FILENAME(line) FILENAME_FOR_EXCEPTIONS_C("src/cpu-kernels/awkward_IndexedArray_flatten_nextcarry.cpp", line)

#include "awkward/kernels.h"

template <typename C, typename T>
ERROR awkward_IndexedArray_flatten_nextcarry(
  T* __restrict__ tocarry,
  const C* __restrict__ fromindex,
  int64_t lenindex,
  int64_t lencontent) {
  int64_t k = 0;
  for (int64_t i = 0;  i < lenindex;  i++) {
    C j = fromindex[i];
    if (j >= lencontent) {
      return failure("index out of range", i, j, FILENAME(__LINE__));
    }
    else if (j >= 0) {
      tocarry[k] = j;
      k++;
    }
  }
  return success();
}

#define WRAPPER(FUNC, C, T) \
  ERROR FUNC(T* tocarry, const C* fromindex, int64_t lenindex, int64_t lencontent) { \
    return awkward_IndexedArray_flatten_nextcarry<C, T>(tocarry, fromindex, lenindex, lencontent); \
  }

WRAPPER(awkward_IndexedArray32_flatten_nextcarry_64, int32_t, int64_t)
WRAPPER(awkward_IndexedArrayU32_flatten_nextcarry_64, uint32_t, int64_t)
WRAPPER(awkward_IndexedArray64_flatten_nextcarry_64, int64_t, int64_t)
