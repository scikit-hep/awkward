...
public:
  ...
  int64_t
  read(int64_t num_bytes, char* buffer) {
    ...
    if (!PyBytes_Check(data.ptr())) {
      throw py::type_error(
          "obj.read(num_bytes) should return bytes (is the file mode 'rb'?)");
    }
    ...
    if (num_bytes_read > num_bytes) {
      throw py::type_error(
          "obj.read(num_bytes) returned a larger bytes object than num_bytes");
    }

    std::strncpy(buffer,
                 PyBytes_AsString(data.ptr()),
                 std::min(num_bytes, num_bytes_read));
  ...
  m.def(
      ...
      [](py::object& source,
        ...
        for (int64_t i = 0; i < out.num_outputs(); i++) {
          ...
          py::object value =
              py::module::import("numpy").attr("empty")(num_items, dtype);
          size_t data_pointer =
              value.attr("ctypes").attr("data").cast<size_t>();
