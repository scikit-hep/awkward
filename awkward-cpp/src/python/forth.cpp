...
py::object
maybe_throw(const ak::ForthMachineOf<T, I>& self,
  ...
  switch (err) {
    ...
    default:
      throw std::invalid_argument(std::string("unrecognized ForthError: ") +
                                  std::to_string((int64_t)err) +
                                  FILENAME(__LINE__));
  }
}
...
template <typename T>
py::object
capsule_for_shared_pointer(std::shared_ptr<T> ptr) {
  /**
...
py::object
output_buffer_to_numpy(std::shared_ptr<ak::ForthOutputBuffer> output) {
  auto ptr = output->ptr();
...
template <typename T, typename I>
py::object
machine_bytecodes_at_to_python_content(
    std::shared_ptr<ak::ForthMachineOf<T, I>> machine, int64_t index) {
  ...
  const auto bytecodes_holder =
      std::make_shared<std::vector<I>>(std::move(machine->bytecodes()));
  ...
  auto bytecodes_array = py::array_t<I>(
      bytecodes_holder->size(), bytecodes_holder->data(), bytecodes_capsule);
...
make_ForthMachineOf(const py::handle& m, const std::string& name) {
  return (
      py::class_<ak::ForthMachineOf<T, I>,
          ...
          .def(py::init([](const std::string& source,
                 ...
                 return std::make_shared<ak::ForthMachineOf<T, I>>(
                     source,
                     stack_size,
                     recursion_depth,
                     string_buffer_size,
          ...
          .def("__getitem__",
               [](const std::shared_ptr<ak::ForthMachineOf<T, I>>& self,
                 ...
                 if (self.get()->is_variable(key)) {
                   ...
                   return machine_bytecodes_at_to_python_content(self,
                                                                 index + 1);
                 } else {
                   throw std::invalid_argument(
                       std::string("unrecognized AwkwardForth "
                                   "variable/output/dictionary word: ") +
                       key + FILENAME(__LINE__));
                 }
               })
          .def_property_readonly("abi_version",
                                 &ak::ForthMachineOf<T, I>::abi_version)
          .def_property_readonly("source", &ak::ForthMachineOf<T, I>::source)
          ...
          .def_property_readonly("stack", &ak::ForthMachineOf<T, I>::stack)
          .def("stack_push",
               [](ak::ForthMachineOf<T, I>& self, T value) -> void {
                 if (!self.stack_can_push()) {
                   throw std::invalid_argument(
                       std::string("AwkwardForth stack overflow") +
                       FILENAME(__LINE__));
                 }
                 self.stack_push(value);
               })
          .def("stack_pop",
               [](ak::ForthMachineOf<T, I>& self) -> T {
                 if (!self.stack_can_pop()) {
                   throw std::invalid_argument(
                       std::string("AwkwardForth stack underflow") +
                       FILENAME(__LINE__));
                 }
          ...
          .def("string_at",
               [](ak::ForthMachineOf<T, I>& self, int64_t at) -> py::str {
                 return self.string_at(at);
               })
          .def_property_readonly("variables",
                                 &ak::ForthMachineOf<T, I>::variables)
          .def("input_position",
               [](ak::ForthMachineOf<T, I>& self,
          ...
          .def(
              ...
              [](ak::ForthMachineOf<T, I>& self,
                ...
                for (auto pair : inputs) {
                  ...
                  py::buffer_info info =
                      obj.request(self.input_must_be_writable(name));
                  ...
                  std::shared_ptr<void> ptr = std::shared_ptr<uint8_t>(
                      ...
                      pyobject_deleter<uint8_t>(obj.ptr()));
                  ins[name] =
                      std::make_shared<ak::ForthInputBuffer>(ptr, 0, length);
                }
                self.begin(ins);
              },
          ...
          .def("count_reset", &ak::ForthMachineOf<T, I>::count_reset)
          ...
          .def("is_variable", &ak::ForthMachineOf<T, I>::is_variable)
          .def("is_input", &ak::ForthMachineOf<T, I>::is_input)
          .def("is_output", &ak::ForthMachineOf<T, I>::is_output)
          .def("is_defined", &ak::ForthMachineOf<T, I>::is_defined)
          .def_property_readonly("is_ready",
                                 &ak::ForthMachineOf<T, I>::is_ready)
          .def_property_readonly("is_done", &ak::ForthMachineOf<T, I>::is_done)
