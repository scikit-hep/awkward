...
builder_datetime(ak::ArrayBuilder& self, const py::handle& obj) {
  if (py::isinstance<py::str>(obj)) {
    ...
    auto ptr =
        date_time.attr("astype")(py::module::import("numpy").attr("int64"));
    auto units = py::str(py::module::import("numpy").attr("dtype")(date_time))
                     .cast<std::string>();
    self.datetime(ptr.cast<int64_t>(), units);
  } else if (py::isinstance(obj,
    ...
    auto time_since_epoch =
        obj - datetime.attr("datetime")(1970, 1, 1, 0, 0, 0);
    auto resolution_microseconds =
        datetime.attr("timedelta")("microseconds"_a = 1);
    auto time_since_epoch_us =
        time_since_epoch.attr("__floordiv__")(resolution_microseconds)
            .cast<int64_t>();
    ...
    auto time_since_epoch_days =
        time_since_epoch.attr("__floordiv__")(resolution_days).cast<int64_t>();
    ...
    auto time_since_midnight =
        datetime.attr("timedelta")("hours"_s = obj.attr("hour"),
                                   "minutes"_s = obj.attr("minute"),
                                   "seconds"_s = obj.attr("second"));
    auto resolution_microseconds =
        datetime.attr("timedelta")("microseconds"_a = 1);
    auto time_since_midnight_us =
        time_since_midnight.attr("__floordiv__")(resolution_microseconds)
            .cast<int64_t>();
...
builder_timedelta(ak::ArrayBuilder& self, const py::handle& obj) {
  if (py::isinstance<py::str>(obj)) {
    ...
    self.timedelta(ptr.cast<int64_t>(), units);
                            ...
                            py::module::import("numpy").attr("timedelta64"))) {
    auto ptr = obj.attr("astype")(py::module::import("numpy").attr("int64"));
    self.timedelta(ptr.cast<int64_t>(), py::str(obj.attr("dtype")));
  } else if (py::isinstance(obj,
                            py::module::import("datetime").attr("timedelta"))) {
...
builder_fromiter(ak::ArrayBuilder& self, const py::handle& obj) {
  if (obj.is(py::none())) {
    ...
    self.integer(obj.cast<int64_t>());
                            ...
                            py::module::import("numpy").attr("floating"))) {
    self.real(obj.cast<double>());
...
make_ArrayBuilder(const py::handle& m, const std::string& name) {
  return (
      py::class_<ak::ArrayBuilder>(m, name.c_str())
          .def(py::init([](const int64_t initial,
                           double resize) -> ak::ArrayBuilder {
          ...
          .def("form",
               [](const ak::ArrayBuilder& self) -> py::object {
          ...
          .def("string",
               [](ak::ArrayBuilder& self, const py::str& x) -> void {
          ...
          .def(
              ...
              py::arg("name") = py::none())
          .def("field",
               [](ak::ArrayBuilder& self, const std::string& x) -> void {
                 self.field_check(x);
               })
