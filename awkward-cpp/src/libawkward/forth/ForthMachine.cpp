...
#define FILENAME(line) \
  FILENAME_FOR_EXCEPTIONS("src/libawkward/forth/ForthMachine.cpp", line)
...
#define BOUND_DICTIONARY 71
  ...
  const std::set<std::string> input_parser_words_({// single little-endian
                                                   "?->",
                                                   "b->",
                                                   "h->",
                                                   "i->",
  ...
  ForthMachineOf<T, I>::decompiled() const {
    ...
    for (IndexTypeOf<int64_t> i = 0; i < dictionary_names_.size(); i++) {
      ...
      out << ": " << dictionary_names_[i] << std::endl
          ...
          << decompiled_segment(segment_position, "  ") << ";" << std::endl;
    }
  ...
  ForthMachineOf<T, I>::decompiled_segment(int64_t segment_position,
                                           ...
                                           bool endline) const {
    if (segment_position < 0 || (IndexTypeOf<int64_t>)segment_position + 1 >=
                                    bytecodes_offsets_.size()) {
      throw std::runtime_error(
          std::string("segment ") + std::to_string(segment_position) +
          std::string(" does not exist in the bytecode") + FILENAME(__LINE__));
    }
    std::stringstream out;
    int64_t bytecode_position =
        bytecodes_offsets_[(IndexTypeOf<int64_t>)segment_position];
    while (bytecode_position <
           bytecodes_offsets_[(IndexTypeOf<int64_t>)segment_position + 1]) {
      if (bytecode_position !=
          bytecodes_offsets_[(IndexTypeOf<int64_t>)segment_position]) {
        out << indent;
      }
  ...
  ForthMachineOf<T, I>::decompiled_at(int64_t bytecode_position,
    ...
    if (bytecode_position < 0 ||
        (IndexTypeOf<int64_t>)bytecode_position >= bytecodes_.size()) {
      throw std::runtime_error(std::string("absolute position ") +
                               std::to_string(bytecode_position) +
                               std::string(" does not exist in the bytecode") +
                               FILENAME(__LINE__));
    }
    ...
    if (bytecode < 0) {
      ...
      switch (~bytecode & READ_MASK) {
        ...
        case READ_ZIGZAG:
          ...
          break;
        case READ_NBIT:
          nbits = bytecodes_[(IndexTypeOf<int64_t>)bytecode_position +
                             (IndexTypeOf<int64_t>)next_pos];
          next_pos++;
      ...
      if (~bytecode & READ_DIRECT) {
        I out_num = bytecodes_[(IndexTypeOf<int64_t>)bytecode_position +
                               (IndexTypeOf<int64_t>)next_pos];
      ...
      return std::move(std::string("begin\n") +
                       ...
                       decompiled_segment(body, indent + "  ") + indent +
                       "again");
    }
      ...
      int64_t postcondition =
          bytecodes_[(IndexTypeOf<int64_t>)bytecode_position + 2] -
          BOUND_DICTIONARY;
      return std::move(
          std::string("begin\n") +
          (segment_nonempty(precondition) ? indent + "  " : "") +
          decompiled_segment(precondition, indent + "  ") + indent + "while\n" +
          (segment_nonempty(postcondition) ? indent + "  " : "") +
          decompiled_segment(postcondition, indent + "  ") + indent + "repeat");
    }
      ...
      switch (bytecode) {
        case CODE_LITERAL: {
          return std::to_string(
              bytecodes_[(IndexTypeOf<int64_t>)bytecode_position + 1]);
        }
        case CODE_HALT: {
          return "halt";
        }
        ...
        case CODE_IF_ELSE: {
          ...
          return std::move(
              std::string("if\n") +
              (segment_nonempty(consequent) ? indent + "  " : "") +
              decompiled_segment(consequent, indent + "  ") + indent +
              "else\n" + (segment_nonempty(alternate) ? indent + "  " : "") +
              decompiled_segment(alternate, indent + "  ") + indent + "then");
        }
        case CODE_CASE_REGULAR: {
          ...
          if (alt_nonempty) {
            ...
            alt_segment = alt_segment.substr(
                0, alt_segment.length() - indent.length() - 9);
            out << indent << "  ( default )\n"
                << indent << "    " << alt_segment;
          }
          out << "endcase";
        ...
        case CODE_ENUM: {
          ...
          int64_t start =
              bytecodes_[(IndexTypeOf<int64_t>)bytecode_position + 2];
          int64_t stop =
              bytecodes_[(IndexTypeOf<int64_t>)bytecode_position + 3];
          std::stringstream out;
      ...
      return std::move(std::string("(unrecognized bytecode ") +
                       std::to_string(bytecode) + ")");
    }
  }
  ...
  ForthMachineOf<T, I>::variable_at(const std::string& name) const {
    ...
    throw std::invalid_argument(std::string("variable not found: ") + name +
                                FILENAME(__LINE__));
  }
  ...
  ForthMachineOf<T, I>::input_must_be_writable(const std::string& name) const {
    ...
    throw std::invalid_argument(std::string("input not found: ") + name +
                                FILENAME(__LINE__));
  }
  ...
  ForthMachineOf<T, I>::output_at(const std::string& name) const {
    ...
    throw std::invalid_argument(std::string("output not found: ") + name +
                                FILENAME(__LINE__));
  }
  ...
  ForthMachineOf<T, I>::string_at(int64_t index) const noexcept {
    return ((index >= 0 && index < (int64_t)strings_.size())
                ? strings_[(IndexTypeOf<int64_t>)index]
                : std::string("a string at ") + std::to_string(index) +
  ...
  ForthMachineOf<T, I>::begin(
    ...
    for (auto name : input_names_) {
      ...
      if (!found) {
        throw std::invalid_argument(
            std::string("AwkwardForth source code defines an input that was "
                        "not provided: ") +
    ...
    for (IndexTypeOf<int64_t> i = 0; i < output_names_.size(); i++) {
      ...
      switch (output_dtypes_[i]) {
        ...
        default: {
          throw std::runtime_error(
              std::string("unhandled ForthOutputBuffer type") +
              FILENAME(__LINE__));
        }
      }
  ...
  ForthMachineOf<T, I>::begin_again(
    ...
    if (!is_ready()) {
      throw std::invalid_argument(
          std::string("'begin' not called on the AwkwardForth machine, "
                      "'begin_again' invalid") +
          FILENAME(__LINE__));
    }
    if (current_error_ != util::ForthError::none) {
  ...
  ForthMachineOf<T, I>::call(const std::string& name) {
    ...
    throw std::runtime_error(std::string("AwkwardForth unrecognized word: ") +
                             name + FILENAME(__LINE__));
  }
  ...
  ForthMachineOf<T, I>::call(int64_t index) {
    ...
    bytecodes_pointer_push(dictionary_bytecodes_[(IndexTypeOf<int64_t>)index] -
                           BOUND_DICTIONARY);
  ...
  template <typename T, typename I>
  void
  ForthMachineOf<T, I>::maybe_throw(
      util::ForthError /* err */,  // FIXME: this argument is not needed
    ...
    if (ignore.count(current_error_) == 0) {
      switch (current_error_) {
        case util::ForthError::not_ready: {
          throw std::invalid_argument(
              "'not ready' in AwkwardForth runtime: call 'begin' before 'step' "
              "or "
              "'resume' (note: check 'is_ready')");
        }
        case util::ForthError::is_done: {
          throw std::invalid_argument(
              "'is done' in AwkwardForth runtime: reached the end of the "
              "program; "
              "call 'begin' to 'step' again (note: check 'is_done')");
        }
        case util::ForthError::user_halt: {
          throw std::invalid_argument(
              "'user halt' in AwkwardForth runtime: user-defined error or "
              "stopping "
              "condition");
        }
        case util::ForthError::recursion_depth_exceeded: {
          throw std::invalid_argument(
              "'recursion depth exceeded' in AwkwardForth runtime: too many "
              "words "
              "calling words or a recursive word is looping endlessly");
        }
        case util::ForthError::stack_underflow: {
          throw std::invalid_argument(
              "'stack underflow' in AwkwardForth runtime: tried to pop from an "
              "empty "
              "stack");
        }
        case util::ForthError::stack_overflow: {
          throw std::invalid_argument(
              "'stack overflow' in AwkwardForth runtime: tried to push beyond "
              "the "
              "predefined maximum stack depth");
        }
        case util::ForthError::read_beyond: {
          throw std::invalid_argument(
              "'read beyond' in AwkwardForth runtime: tried to read beyond the "
              "end "
              "of an input");
        }
        case util::ForthError::seek_beyond: {
          throw std::invalid_argument(
              "'seek beyond' in AwkwardForth runtime: tried to seek beyond the "
              "bounds "
              "of an input (0 or length)");
        }
        case util::ForthError::skip_beyond: {
          throw std::invalid_argument(
              "'skip beyond' in AwkwardForth runtime: tried to skip beyond the "
              "bounds "
              "of an input (0 or length)");
        }
        case util::ForthError::rewind_beyond: {
          throw std::invalid_argument(
              "'rewind beyond' in AwkwardForth runtime: tried to rewind beyond "
              "the "
              "beginning of an output");
        }
        case util::ForthError::division_by_zero: {
          throw std::invalid_argument(
              "'division by zero' in AwkwardForth runtime: tried to divide by "
              "zero");
        }
        case util::ForthError::varint_too_big: {
          throw std::invalid_argument(
              "'varint too big' in AwkwardForth runtime: variable-length "
              "integer is "
              "too big to represent as a fixed-width integer");
        }
        case util::ForthError::text_number_missing: {
          throw std::invalid_argument(
              "'text number missing' in AwkwardForth runtime: expected a "
              "number in "
              "input text, didn't find one");
        }
        case util::ForthError::quoted_string_missing: {
          throw std::invalid_argument(
              "'quoted string missing' in AwkwardForth runtime: expected a "
              "quoted string in "
              "input text, didn't find one");
        }
        case util::ForthError::enumeration_missing: {
          throw std::invalid_argument(
              "'enumeration missing' in AwkwardForth runtime: expected one of "
              "several "
  ...
  ForthMachineOf<T, I>::current_bytecode_position() const noexcept {
    if (recursion_current_depth_ == 0) {
      ...
      if (where < bytecodes_offsets_[(IndexTypeOf<int64_t>)which + 1] -
                      bytecodes_offsets_[(IndexTypeOf<int64_t>)which]) {
  ...
  template <typename T, typename I>
  bool
  ForthMachineOf<T, I>::is_integer(const std::string& word,
                                   int64_t& value) const {
    if (word.size() >= 2 && word.substr(0, 2) == std::string("0x")) {
      try {
        value =
            (int64_t)std::stoul(word.substr(2, word.size() - 2), nullptr, 16);
  ...
  ForthMachineOf<T, I>::is_variable(const std::string& word) const {
    return std::find(variable_names_.begin(), variable_names_.end(), word) !=
           variable_names_.end();
  }
  ...
  ForthMachineOf<T, I>::is_input(const std::string& word) const {
    return std::find(input_names_.begin(), input_names_.end(), word) !=
           input_names_.end();
  }
  ...
  ForthMachineOf<T, I>::is_output(const std::string& word) const {
    return std::find(output_names_.begin(), output_names_.end(), word) !=
           output_names_.end();
  }
  ...
  ForthMachineOf<T, I>::is_nbit(const std::string& word, I& value) const {
    ...
    if (parser.length() > 5 &&
        parser.substr(parser.length() - 5, 5) == "bit->") {
  ...
  ForthMachineOf<T, I>::segment_nonempty(int64_t segment_position) const {
    return bytecodes_offsets_[(IndexTypeOf<int64_t>)segment_position] !=
           bytecodes_offsets_[(IndexTypeOf<int64_t>)segment_position + 1];
  }

  template <typename T, typename I>
  int64_t
  ForthMachineOf<T, I>::bytecodes_per_instruction(
      int64_t bytecode_position) const {
    I bytecode = bytecodes_[(IndexTypeOf<int64_t>)bytecode_position];
    I next_bytecode = -1;
    if ((IndexTypeOf<int64_t>)bytecode_position + 1 < bytecodes_.size()) {
      next_bytecode = bytecodes_[(IndexTypeOf<int64_t>)bytecode_position + 1];
  ...
  template <typename T, typename I>
  const std::string
  ForthMachineOf<T, I>::err_linecol(
    ...
    std::stringstream out;
    out << "in AwkwardForth source code, line " << lc.first << " col "
        << lc.second << ", " << message << ":" << std::endl
        << std::endl
        << "    ";
    int64_t line = 1;
    int64_t col = 1;
    ...
    while (stop < source_.length()) {
      ...
      if ((IndexTypeOf<int64_t>)stoppos < linecol.size() &&
          linecol[(IndexTypeOf<int64_t>)stoppos].first == line &&
          linecol[(IndexTypeOf<int64_t>)stoppos].second == col) {
        break;
      }
      if (source_[stop] == '\n') {
        line++;
    ...
    while (stop < source_.size()) {
      ...
      if (!tokenized.empty() && (tokenized[tokenized.size() - 1] == ".\"" ||
        ...
        if (stop == source_.size()) {
          throw std::invalid_argument(
              std::string("unclosed string after .\" or s\" word") +
              FILENAME(__LINE__));
        }
      ...
      if (word == "(") {
        ...
        while (nesting > 0) {
          ...
          if (substop >= stop) {
            throw std::invalid_argument(
                err_linecol(
                    linecol, pos, substop, "'(' is missing its closing ')'") +
                FILENAME(__LINE__));
          }
        ...
        int64_t substop = pos;
        while (substop < stop &&
               tokenized[(IndexTypeOf<std::string>)substop] != "\n") {
          substop++;
        }
        ...
        if (pos + 1 >= stop ||
            tokenized[(IndexTypeOf<std::string>)pos + 1] == ";") {
          throw std::invalid_argument(
              err_linecol(
                  linecol, pos, pos + 2, "missing name in word definition") +
              FILENAME(__LINE__));
        }
        ...
        if (is_input(name) || is_output(name) || is_variable(name) ||
          ...
          throw std::invalid_argument(
              err_linecol(
                  ...
                  "words must all be unique and not reserved words or "
                  "integers") +
              FILENAME(__LINE__));
        }
        ...
        parse(
            name, tokenized, linecol, substart, substop, body, dictionary, 0, 0);
        ...
        if (pos + 1 >= stop) {
          throw std::invalid_argument(
              err_linecol(
                  linecol, pos, pos + 2, "missing name in input declaration") +
              FILENAME(__LINE__));
        }
        std::string name = tokenized[(IndexTypeOf<std::string>)pos + 1];
        ...
        if (!found_dtype) {
          throw std::invalid_argument(
              err_linecol(
                  linecol, pos, pos + 3, "output dtype not recognized") +
              FILENAME(__LINE__));
        }
        ...
        if (subelse == -1) {
          ...
          parse(defn,
                ...
                exitdepth + 1,
                dodepth);
          dictionary[(IndexTypeOf<int64_t>)bytecode - BOUND_DICTIONARY] =
              consequent;
          ...
          parse(defn,
                ...
                exitdepth + 1,
                dodepth);
          dictionary[(IndexTypeOf<int64_t>)bytecode1 - BOUND_DICTIONARY] =
              consequent;
          ...
          parse(defn,
                ...
                exitdepth + 1,
                dodepth);
          dictionary[(IndexTypeOf<int64_t>)bytecode2 - BOUND_DICTIONARY] =
              alternate;
          ...
          if (substop >= stop) {
            throw std::invalid_argument(
                err_linecol(linecol,
                            pos,
                            stop,
                            "'case' is missing its closing 'endcase'") +
                FILENAME(__LINE__));
            ...
            nesting++;
                     ...
                     "endcase") {
            nesting--;
          } else if (tokenized[(IndexTypeOf<std::string>)substop] == "of" &&
                     nesting == 1) {
            ofs.push_back(substop);
          } else if (tokenized[(IndexTypeOf<std::string>)substop] == "endof" &&
                     nesting == 1) {
        ...
        if (ofs.size() != endofs.size()) {
          throw std::invalid_argument(
              err_linecol(linecol,
                          pos,
                          stop,
                          "in 'case' .. 'endcase', there must be an 'endof' "
                          "for every 'of'") +
              FILENAME(__LINE__));
        }
        ...
        for (int64_t i = 0; (size_t)i < ofs.size(); i++) {
          ...
          dictionary[(IndexTypeOf<int64_t>)pred_bytecode - BOUND_DICTIONARY] =
              pred;
          ...
          parse(defn,
                ...
                exitdepth + 1,
                dodepth);
          dictionary[(IndexTypeOf<int64_t>)cons_bytecode - BOUND_DICTIONARY] =
              cons;
...

        {
          ...
          parse(defn,
                ...
                exitdepth + 1,
                dodepth);
          dictionary[(IndexTypeOf<int64_t>)alt_bytecode - BOUND_DICTIONARY] =
              alt;
        ...
        if (can_specialize) {
          ...
          for (int64_t i = 0; (size_t)i < ofs.size(); i++) {
            auto pred =
                dictionary.begin() + (predicates[(size_t)i] - BOUND_DICTIONARY);
            pred->push_back(CODE_OVER);  // append "over"
            pred->push_back(CODE_EQ);    // append "="
            auto cons = dictionary.begin() +
                        (consequents[(size_t)i] - BOUND_DICTIONARY);
          ...
          for (int64_t i = (int64_t)ofs.size() - 1; i >= 0; i--) {
            ...
            dictionary[(IndexTypeOf<int64_t>)ifthenelse_bytecode -
                       BOUND_DICTIONARY] = ifthenelse;
        ...
        while (nesting > 0) {
          ...
          if (substop >= stop) {
            throw std::invalid_argument(
                err_linecol(
                    linecol, pos, stop, "'do' is missing its closing 'loop'") +
                FILENAME(__LINE__));
          ...
          if (substop >= stop) {
            throw std::invalid_argument(
                err_linecol(linecol,
                            pos,
                            stop,
                            "'begin' is missing its closing 'until' or 'while "
                            "... repeat'") +
                FILENAME(__LINE__));
        ...
        if (pos + 1 < stop &&
          ...
          throw std::invalid_argument(err_linecol(linecol,
                                                  pos,
                                                  pos + 2,
                                                  "missing '!', '+!', or '@' "
                                                  "after variable name"));
        ...
        if (pos + 1 < stop &&
            (tokenized[(IndexTypeOf<std::string>)pos + 1] == "enum" ||
            ...
            if (tokenized[(IndexTypeOf<std::string>)pos + 1] == "enum") {
              throw std::invalid_argument(
                  err_linecol(
                      ...
                      pos,
                      pos + 2,
                      "need at least one string (s\" word) after \"enum\"") +
                  FILENAME(__LINE__));
            } else {
              throw std::invalid_argument(
                  err_linecol(linecol,
                              pos,
                              pos + 2,
                              "need at least one string (s\" word) after "
                              "\"enumonly\"") +
                  FILENAME(__LINE__));
            }
          }
          ...
          bytecodes.push_back((int32_t)strings_.size());
                   ...
                   tokenized[(IndexTypeOf<std::string>)pos + 1] == "peek") {
          bytecodes.push_back(CODE_PEEK);
          bytecodes.push_back((int32_t)input_index);

          pos += 2;
        } else if (pos + 1 < stop &&
                   tokenized[(IndexTypeOf<std::string>)pos + 1] == "len") {
          bytecodes.push_back(CODE_LEN_INPUT);
          bytecodes.push_back((int32_t)input_index);

          pos += 2;
        } else if (pos + 1 < stop &&
                   tokenized[(IndexTypeOf<std::string>)pos + 1] == "pos") {
          bytecodes.push_back(CODE_POS);
          bytecodes.push_back((int32_t)input_index);

          pos += 2;
        } else if (pos + 1 < stop &&
                   tokenized[(IndexTypeOf<std::string>)pos + 1] == "end") {
          bytecodes.push_back(CODE_END);
          bytecodes.push_back((int32_t)input_index);

          pos += 2;
        } else if (pos + 1 < stop &&
                   tokenized[(IndexTypeOf<std::string>)pos + 1] == "seek") {
          bytecodes.push_back(CODE_SEEK);
          bytecodes.push_back((int32_t)input_index);

          pos += 2;
        } else if (pos + 1 < stop &&
                   tokenized[(IndexTypeOf<std::string>)pos + 1] == "skip") {
          bytecodes.push_back(CODE_SKIP);
          bytecodes.push_back((int32_t)input_index);

          pos += 2;
        } else if (pos + 1 < stop &&
                   tokenized[(IndexTypeOf<std::string>)pos + 1] == "skipws") {
          ...
          if (!good || parser != "->") {
            throw std::invalid_argument(
                err_linecol(linecol,
                            ...
                            "'seek', 'skip', 'skipws', 'end', 'pos', or 'len' "
                            "after input name") +
                FILENAME(__LINE__));
          }
          ...
          if (pos + 2 < stop &&
              tokenized[(IndexTypeOf<std::string>)pos + 2] == "stack") {
            // not READ_DIRECT
            ...
            if ((bytecode & READ_MASK) == READ_QUOTEDSTR) {
              throw std::invalid_argument(
                  ...
                  FILENAME(__LINE__));
                     ...
                     is_output(tokenized[(IndexTypeOf<std::string>)pos + 2])) {
            for (; output_index < output_names_.size(); output_index++) {
              if (output_names_[output_index] ==
                  tokenized[(IndexTypeOf<std::string>)pos + 2]) {
                found_output = true;
                break;
              }
            }
          ...
          throw std::invalid_argument(
              err_linecol(linecol,
                          pos,
                          pos + 3,
                          "missing '*-> stack/output', 'seek', 'skip', "
                          "'skipws', 'end', "
        ...
        if (pos + 1 < stop &&
          ...
          if (pos + 2 < stop &&
            ...
            throw std::invalid_argument(
                err_linecol(
                    linecol, pos, pos + 3, "missing 'stack' after '<-'") +
                FILENAME(__LINE__));
          }
        } else if (pos + 1 < stop &&
                   tokenized[(IndexTypeOf<std::string>)pos + 1] == "+<-") {
          if (pos + 2 < stop &&
            ...
            throw std::invalid_argument(
                ...
                FILENAME(__LINE__));
                   ...
                   tokenized[(IndexTypeOf<std::string>)pos + 1] == "dup") {
          ...
          pos += 2;
                   ...
                   tokenized[(IndexTypeOf<std::string>)pos + 1] == "rewind") {
        ...
        if (pos + 1 >= stop) {
          throw std::invalid_argument(
              err_linecol(
                  linecol, pos, pos + 2, "unclosed string after s\" word") +
              FILENAME(__LINE__));
        }
        strings_.push_back(tokenized[(IndexTypeOf<std::string>)pos + 1]);
        ...
        if (pos + 1 >= stop) {
          throw std::invalid_argument(
              err_linecol(
                  linecol, pos, pos + 2, "unclosed string after .\" word") +
              FILENAME(__LINE__));
        }
        strings_.push_back(tokenized[(IndexTypeOf<std::string>)pos + 1]);
        ...
        for (auto pair : generic_builtin_words_) {
          if (pair.first == word) {
            ...
            if (word == "i" && dodepth < 1) {
              throw std::invalid_argument(
                  err_linecol(
                      linecol, pos, pos + 1, "only allowed in a 'do' loop") +
                  FILENAME(__LINE__));
            }
            if (word == "j" && dodepth < 2) {
              throw std::invalid_argument(
        ...
        if (!found_in_builtins) {
          ...
          for (IndexTypeOf<std::string> i = 0; i < dictionary_names_.size();
               i++) {
  ...
  static uint8_t bitswap_lookup[16] = {0x0,
                                       0x8,
                                       0x4,
                                       0xc,
                                       0x2,
  ...
  template <typename T, typename I>
  void
  ForthMachineOf<T, I>::internal_run(
      bool single_step, int64_t recursion_target_depth_top) {  // noexcept
      ...
      while (
        ...
        if (bytecode < 0) {
          ...
          if (format == READ_VARINT) {
            ForthInputBuffer* input =
                current_inputs_[(IndexTypeOf<int64_t>)in_num].get();
            ...
            if (output == nullptr) {
              ...
              for (int64_t count = 0; count < num_items; count++) {
                ...
                output->write_one_uint64(
                    result, false);  // note: writing result as unsigned
              }
            }
          }
            ...
            if (output == nullptr) {
              ...
              for (int64_t count = 0; count < num_items; count++) {
                ...
                output->write_one_int64(
                    result, false);  // note: writing value as signed
              }
            }
          }
            ...
            if (items_remaining != 0) {
              ...
              if (flip) {
                // For bit-flipping: https://stackoverflow.com/a/2603254/1623645
                tmp = (uint64_t)(bitswap_lookup[tmp & 0b1111] << 4) |
                      bitswap_lookup[tmp >> 4];
              }
              data = tmp;
            }
            while (items_remaining != 0) {
              if (bits_wnd_r >= 8) {
                ...
                if (output == nullptr) {
                  ...
                  output->write_one_uint64(
                      tmp, false);  // note: writing value as unsigned
                }
                items_remaining--;
            ...
            I out_num = bytecode_get();
            bytecodes_pointer_where()++;
            ForthOutputBuffer* output =
                current_outputs_[(IndexTypeOf<int64_t>)out_num].get();
            ...
            for (int64_t count = 0; count < num_items; count++) {
              ...
              output->write_one_float64(
                  result, false);  // note: writing value as floating-point
            }
          }
            ...
            for (int64_t count = 0; count < num_items; count++) {
              ...
              int64_t length;
              input->read_quotedstr(
                  string_buffer_, string_buffer_size_, length, current_error_);
              if (current_error_ != util::ForthError::none) {
                return;
              }
              if (stack_cannot_push()) {
              ...
              if (length != 0) {
                output->write_one_string(string_buffer_,
                                         length);  // note: copying string
              }
            }
          }
...
#define WRITE_DIRECTLY(TYPE, SUFFIX)                                         \
  {                                                                          \
    ...
    if (num_items == 1) {                                                    \
      current_outputs_[(IndexTypeOf<int64_t>)out_num]                        \
          .get()                                                             \
          ->write_one_##SUFFIX(*ptr, byteswap);                              \
    } else {                                                                 \
...
#define WRITE_TO_STACK_SWAP_INTP(TYPE)                             \
    ...
    break;                                                         \
            ...
            switch (format) {
              case READ_BOOL:
                WRITE_TO_STACK(bool)
              case READ_INT8:
                WRITE_TO_STACK(int8_t)
              case READ_INT16:
                WRITE_TO_STACK_SWAP(int16_t, byteswap16)
              case READ_INT32:
                WRITE_TO_STACK_SWAP(int32_t, byteswap32)
              case READ_INT64:
                WRITE_TO_STACK_SWAP(int64_t, byteswap64)
              case READ_INTP:
                WRITE_TO_STACK_SWAP_INTP(ssize_t)
              case READ_UINT8:
                WRITE_TO_STACK(uint8_t)
              case READ_UINT16:
                WRITE_TO_STACK_SWAP(uint16_t, byteswap16)
              case READ_UINT32:
                WRITE_TO_STACK_SWAP(uint32_t, byteswap32)
              case READ_UINT64:
                WRITE_TO_STACK_SWAP(uint64_t, byteswap64)
              case READ_UINTP:
                WRITE_TO_STACK_SWAP_INTP(size_t)
              case READ_FLOAT32:
                WRITE_TO_STACK_SWAP(float, byteswap32)
              case READ_FLOAT64:
                WRITE_TO_STACK_SWAP(double, byteswap64)
            }
          ...
          switch (bytecode) {
            ...
            case CODE_ENUM: {
              ...
              I stop = bytecode_get();
              bytecodes_pointer_where()++;
              T result = (T)(current_inputs_[(IndexTypeOf<int64_t>)in_num]
                                 .get()
                                 ->read_enum(strings_, start, stop));
              if (stack_cannot_push()) {
                current_error_ = util::ForthError::stack_overflow;
                return;
              }
            ...
            case CODE_PEEK: {
              ...
              T after = stack_pop();
              T result =
                  current_inputs_[(IndexTypeOf<int64_t>)in_num].get()->peek_byte(
            ...
            case CODE_LEN_INPUT: {
              ...
              stack_push(
                  (T)current_inputs_[(IndexTypeOf<int64_t>)in_num].get()->len());
              break;
            }
            ...
            case CODE_POS: {
              ...
              stack_push(
                  (T)current_inputs_[(IndexTypeOf<int64_t>)in_num].get()->pos());
              break;
            }
            ...
            case CODE_END: {
              ...
              stack_push(
                  current_inputs_[(IndexTypeOf<int64_t>)in_num].get()->end()
                      ? -1
                      : 0);
              break;
            }
            ...
            case CODE_SEEK: {
              ...
              current_inputs_[(IndexTypeOf<int64_t>)in_num].get()->seek(
                  stack_pop(), current_error_);
              if (current_error_ != util::ForthError::none) {
                return;
              }
              break;
            ...
            case CODE_SKIP: {
              ...
              current_inputs_[(IndexTypeOf<int64_t>)in_num].get()->skip(
                  stack_pop(), current_error_);
              if (current_error_ != util::ForthError::none) {
                return;
              }
            ...
            case CODE_WRITE_DUP: {
              ...
              current_outputs_[(IndexTypeOf<int64_t>)out_num].get()->dup(
                  stack_pop(), current_error_);
              if (current_error_ != util::ForthError::none) {
                return;
              }
            ...
            case CODE_LEN_OUTPUT: {
              ...
              stack_push((T)current_outputs_[(IndexTypeOf<int64_t>)out_num]
                             .get()
                             ->len());
              break;
            }
            ...
            case CODE_REWIND: {
              ...
              current_outputs_[(IndexTypeOf<int64_t>)out_num].get()->rewind(
                  stack_pop(), current_error_);
              if (current_error_ != util::ForthError::none) {
                return;
              }
            ...
            case CODE_DIV: {
              ...
              T tmp = pair[0] / pair[1];
              pair[0] = tmp * pair[1] == pair[0]
                            ? tmp
                            : tmp - ((pair[0] < 0) ^ (pair[1] < 0));
              break;
            }
            ...
            case CODE_DIVMOD: {
              ...
              stack_buffer_[stack_depth_ - 2] = (two + (one % two)) % two;
              break;
            }
  ...
  template <>
  void
  ForthMachineOf<int32_t, int32_t>::write_from_stack(int64_t num,
                                                     int32_t* top) noexcept {
    current_outputs_[(IndexTypeOf<int64_t>)num].get()->write_one_int32(*top,
                                                                       false);
  }

  template <>
  void
  ForthMachineOf<int64_t, int32_t>::write_from_stack(int64_t num,
                                                     int64_t* top) noexcept {
    current_outputs_[(IndexTypeOf<int64_t>)num].get()->write_one_int64(*top,
                                                                       false);
  }

  template <>
  void
  ForthMachineOf<int32_t, int32_t>::write_add_from_stack(
      int64_t num, int32_t* top) noexcept {
    current_outputs_[(IndexTypeOf<int64_t>)num].get()->write_add_int32(*top);
  }

  template <>
  void
  ForthMachineOf<int64_t, int32_t>::write_add_from_stack(
      int64_t num, int64_t* top) noexcept {
