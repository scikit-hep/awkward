...
class FuncDecl(object):
    ...
    def getargs(self, tokens):
        cargs = OrderedDict()
        for i in range(len(self.args)):
            if self.args[i]["name"] in tokens.keys():
                cargs[self.args[i]["name"]] = (
                    "List[" * self.args[i]["list"]
                    + tokens[self.args[i]["name"]]
                    + "]" * self.args[i]["list"]
                )
            else:
                cargs[self.args[i]["name"]] = (
...
def getargs(tokens, pfile):
    ...
    for i in range(len(ast.ext)):
        decl = FuncDecl(ast.ext[i].decl)
        if "childfunc" not in tokens[decl.name].keys():
            funcs[decl.name] = decl.getargs(tokens[decl.name])
    return funcs
...
if __name__ == "__main__":
    ...
    for filename in kernelfiles:
        ...
        allfuncargs.update(getargs(tokens, pfile))
    ...
    if kernelname is None:
        ...
        for funcname in alltokens.keys():
            if "gen" not in alltokens[funcname].keys():
                    ...
                    for childfunc in alltokens[funcname]["childfunc"]:
                        ...
                        for arg in allfuncargs[childfunc].keys():
                            print(
                                ...
                                + arg
                                + ": "
                                + allfuncargs[childfunc][arg]
                            )
                else:
                    print(" " * 4 + "args:")
                    for arg in allfuncargs[funcname].keys():
                        print(" " * 6 + "- " + arg + ": " + allfuncargs[funcname][arg])
        ...
        if kernelname in alltokens.keys():
            ...
            if "childfunc" in alltokens[kernelname].keys():
                ...
                for childfunc in alltokens[kernelname]["childfunc"]:
                    ...
                    print(" " * 4 + "args:")
                    for arg in allfuncargs[childfunc].keys():
                        print(" " * 6 + "- " + arg + ": " + allfuncargs[childfunc][arg])
            else:
                for arg in allfuncargs[kernelname].keys():
                    print(" " * 2 + "- " + arg + ": " + allfuncargs[kernelname][arg])
