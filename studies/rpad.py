...
def compact_strides(itemsize, shape):
    strides = len(shape)*[1]
    strides[len(shape) - 1] = itemsize
    for i in range(len(shape) - 1):
        strides[len(shape) - 2 - i] = strides[len(shape) - 1 - i] * shape[len(shape) - 1 - i]
    return strides
...
def NumpyArray_pad_no_shape(self, length, axis=0):
    ...
    if axis == 0:
        ...
        strides = compact_strides(self.strides[2], self.shape)
        print("Here:", strides)
...
def NumpyArray_pad_and_clip(self, length, axis=0):
    ...
    if axis == 0:
        ...
        if length > self.shape[axis]:
            ...
            insert_index = 0
            for x in range(chunks):
                insert_index = insert_index + self.shape[axis] * strides[axis]
        ...
        for i in range(len(out)):
            if k >= self.shape[-1]:
                offsets.append(i)
            if k >= self.shape[-1] + length - self.shape[axis]:
                k = 0
            k = k + 1

        return RegularArray(ListOffsetArray(offsets, out), shape[axis])
...
def RegularArray_pad_and_clip(self, length, axis=0):
    if axis < 0:
        raise NotImplementedError
    elif axis == 0:
        if length > self.size:
            ...
            outindex = []
            out = self.content
            if isinstance(out, IndexedOptionArray) or isinstance(out, IndexedArray):
                ...
                n = out.__len__() // self.size
                for i in range(n*self.size):
                    outindex.append(out.index[i])
                for i in range((length - self.size)*self.size):
                    outindex.append(-1)
                ...
                n = out.__len__() // self.size
                for x in range(n*self.size):
                    outindex.append(x)
                for y in range((length - self.__len__())*self.size):
                    outindex.append(-1)
            ...
            if isinstance(out, IndexedOptionArray) or isinstance(out, IndexedArray):
                ...
                out = out.pad(self.size*(out.__len__() // self.size))
                # pad truncated content
                out = out.pad(self.size*length, axis)
            ...
            for x in range(n):
                for y in range(self.size):
                    if y < length:
                        outindex.append(out.index[i])
                    i = i + 1
                for z in range(length - self.size):
                    outindex.append(-1)
            out = IndexedOptionArray(outindex, out.content)
        else:
...
RegularArray.pad_and_clip = RegularArray_pad_and_clip
        ...
        if length > self.__len__():
            ...
            if isinstance(self.content, IndexedOptionArray):
                ...
                for i in range(len(self.offsets) - 1):
                    outindex.append(i)
                for i in range(length - len(self.offsets) + 1):
                    outindex.append(-1)
...
def IndexedArray_pad(self, length, axis=0):
    ...
    if axis == 0:
        ...
        return IndexedArray(self.index, self.content.pad(length, axis))
...
def RecordArray_pad(self, length, axis=0):
    if axis < 0:
        ...
        outindex = []
        if self.contents.__len__() == 0:
            for x in range(self.__len__()):
                outindex.append(x)
            for y in range(length - self.__len__()):
                outindex.append(-1)

        else:
            if length > self.contents.__len__():
                for x in range(self.contents.__len__()):
                    outindex.append(x)

                for i in range(length - self.contents.__len__()):
                    outindex.append(-1)
            else:
        ...
        return self.pad_contents(length, axis)

RecordArray.pad = RecordArray_pad

def RecordArray_pad_contents(self, length, axis=0):
    if axis < 0:
        raise NotImplementedError
    else:
        contents = []
        for c in self.contents:
            contents.append(c.pad(length, axis))

        return RecordArray(contents, self.recordlookup, self.length)

RecordArray.pad_contents = RecordArray_pad_contents
...
def UnionArray_pad_contents(self, length, axis=0):
    if axis < 0:
        raise NotImplementedError
    else:
        contents = []
...
UnionArray.pad_contents = UnionArray_pad_contents
...
for i in range(10):
    ...
    # OK array = RegularArray(IndexedArray([13, 9, 13, 4, 8, 3, 15, 1, 16, 2, 8], NumpyArray([2.1, 8.4, 7.4, 1.6, 2.2, 3.4, 6.2, 5.4, 1.5, 3.9, 3.8, 3.0, 8.5, 6.9, 4.3, 3.6, 6.7, 1.8, 3.2], [18], [1], 1)), 3)
    # rowwise is not OK:
    # >>> print(array.constructor())
    # RegularArray(NumpyArray([4.1, 6.0, 2.8, 2.7, 7.9, 6.2, 7.2, 5.8, 4.7, 5.8, 0.6, 1.1], [12, 1], [1, 1], 0), 3)
    # >>> print(rowwise)
    # [[[4.1], [6.0], [2.8]], [[2.7], [7.9], [6.2]], [[7.2], [5.8], [4.7]], [[5.8], [0.6], [1.1]]]
    # >>> print(columnar)
    # [[[4.1, None, None], [6.0, None, None], [2.8, None, None]], [[2.7, None, None], [7.9, None, None], [6.2, None, None]], [[7.2, None, None], [5.8, None, None], [4.7, None, None]], [[5.8, None, None], [0.6, None, None], [1.1, None, None]]]
    # array = RegularArray(NumpyArray([4.1, 6.0, 2.8, 2.7, 7.9, 6.2, 7.2, 5.8, 4.7, 5.8, 0.6, 1.1], [12, 1], [1, 1], 0), 3)
    for axis in range(5):
        print(" axis =", axis)
        try:
