...
with open("/home/jpivarski/storage/data/chep-2021-jagged-jagged-jagged/zlib1-jagged3.avro", "rb") as file:
    begintime = time.time()
    for item in fastavro.reader(file):
        data.append(item)
        if len(data) == 285080:
            break
    endtime = time.time()
    print("their time", endtime - begintime)
...
cumulative_entries = 0
data = np.memmap("/home/jpivarski/storage/data/chep-2021-jagged-jagged-jagged/zlib1-jagged3.avro", np.uint8)
...
while pos + 16 < len(data):
    ...
    assert len(array) == num_items
    # print(array)
    cumulative_entries += len(array)
    if cumulative_entries == 285080:
        break

endtime = time.time()
print("my time", endtime - begintime)
