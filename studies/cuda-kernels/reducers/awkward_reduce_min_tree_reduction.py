...
extern "C" {
    __global__ void awkward_reduce_min_a(int* toptr, int* fromptr, int* parents, int lenparents, int outlength) {
       ...
       if (thread_id < outlength) {
          toptr[thread_id] = 9223372036854775807;
       }
    }
}
    extern "C" {
        ...
        if (thread_id < lenparents) {
            ...
            for (int stride = 1; stride < blockDim.x; stride *= 2) {
                ...
                if (index >= 0 && parents[index] == parents[idx]) {
                    shared[tid] = min(shared[tid], shared[index]);
                }
                __syncthreads();
            }
...
awkward_reduce_min_a((grid_size,), (block_size,), (toptr, fromptr, parents, lenparents, outlength))
awkward_reduce_min_b((grid_size,), (block_size,), (toptr, fromptr, parents, lenparents, outlength))
