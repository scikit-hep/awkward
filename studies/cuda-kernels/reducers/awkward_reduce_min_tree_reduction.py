...
cuda_kernel = """
extern "C" {
    __global__ void awkward_reduce_min_a(int* toptr, int* fromptr, int* parents, int lenparents, int outlength, int identity) {
    ...
    extern "C" {
    __global__ void awkward_reduce_min_b(int *toptr, int *fromptr, int *parents, int lenparents, int outlength, int identity) {
        ...
        if (thread_id < lenparents) {
            ...
            for (int stride = 1; stride < blockDim.x; stride *= 2) {
                ...
                if (index >= 0 && parents[index] == parents[idx]) {
                    shared[idx] = min(shared[idx], shared[index]);
                }
                __syncthreads();
            }
...
identity = cp.int32(cp.iinfo(cp.int32).max)
...
awkward_reduce_min_a((grid_size,), (block_size,), (toptr, fromptr, parents, lenparents, outlength, identity))
awkward_reduce_min_b((grid_size,), (block_size,), (toptr, fromptr, parents, lenparents, outlength, identity))
