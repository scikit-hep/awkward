...
def test_symbol_in_connect_all():
    # Internal _connect.cuda entry point (not a public ak.* name, per review).
...
def test_list_array_metadata():
    ...
    _it, meta = cuda.to_cccl_iterator(arr)
...
def test_dtype_cast():
    ...
    it, _meta = cuda.to_cccl_iterator(arr, dtype=np.float32)
...
def test_record_array_builds_zip_iterator():
    ...
    it, _meta = cuda.to_cccl_iterator(arr)
...
def test_matches_internal_helper():
    ...
    _, meta_public = cuda.to_cccl_iterator(arr)
