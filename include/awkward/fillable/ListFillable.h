...
public:
  ListFillable(const FillableOptions &options,
               const GrowableBuffer<int64_t> &offsets,
               std::shared_ptr<Fillable> content, bool begun)
      : options_(options), offsets_(offsets),
        content_(std::shared_ptr<Fillable>(content)), begun_(begun) {}
