...
namespace awkward {
template <typename T> class GrowableBuffer {
public:
  GrowableBuffer(const FillableOptions &options, std::shared_ptr<T> ptr,
                 int64_t length, int64_t reserved)
      : options_(options), ptr_(ptr), length_(length), reserved_(reserved) {}
  ...
  static GrowableBuffer<T> full(const FillableOptions &options, T value,
                                int64_t length) {
  ...
  static GrowableBuffer<T> arange(const FillableOptions &options,
                                  int64_t length) {
    size_t actual = (size_t)options.initial();
  ...
  T getitem_at_nowrap(int64_t at) const { return ptr_.get()[at]; }

  IndexOf<T> toindex() const { return IndexOf<T>(ptr_, 0, length_); }
