...
template <typename T> class RawArrayOf : public Content {
public:
  RawArrayOf<T>(const std::shared_ptr<Identity> id,
                const std::shared_ptr<Type> type, const std::shared_ptr<T> ptr,
                const int64_t offset, const int64_t length,
                const int64_t itemsize)
      : Content(id, type), ptr_(ptr), offset_(offset), length_(length),
        itemsize_(itemsize) {
  ...
  RawArrayOf<T>(const std::shared_ptr<Identity> id,
      ...
      : Content(id, type), ptr_(ptr), offset_(0), length_(length),
        itemsize_(sizeof(T)) {}

  RawArrayOf<T>(const std::shared_ptr<Identity> id,
                const std::shared_ptr<Type> type, const int64_t length)
      : Content(id, type),
        ptr_(std::shared_ptr<T>(new T[(size_t)length],
                                awkward::util::array_deleter<T>())),
        offset_(0), length_(length), itemsize_(sizeof(T)) {}
  ...
  T *borrow(int64_t at) const {
    return reinterpret_cast<T *>(reinterpret_cast<ssize_t>(ptr_.get()) +
  ...
  virtual void setid(const std::shared_ptr<Identity> id) {
    if (id.get() != nullptr && length() != id.get()->length()) {
      throw std::invalid_argument(
          "content and its id must have the same length");
    }
    id_ = id;
  }
  ...
  virtual void tojson_part(ToJson &builder) const {
    if (std::is_same<T, double>::value) {
      ...
      tojson_integer(builder, reinterpret_cast<uint64_t *>(byteptr()),
                     length());
    } else if (std::is_same<T, int32_t>::value) {
      tojson_integer(builder, reinterpret_cast<int32_t *>(byteptr()), length());
    } else if (std::is_same<T, uint32_t>::value) {
      tojson_integer(builder, reinterpret_cast<uint32_t *>(byteptr()),
                     length());
    } else if (std::is_same<T, int16_t>::value) {
      tojson_integer(builder, reinterpret_cast<int16_t *>(byteptr()), length());
    } else if (std::is_same<T, uint16_t>::value) {
      tojson_integer(builder, reinterpret_cast<uint16_t *>(byteptr()),
                     length());
  ...
  virtual const std::shared_ptr<Type> innertype(bool bare) const {
    if (std::is_same<T, double>::value) {
      return std::shared_ptr<Type>(
          new PrimitiveType(Type::Parameters(), PrimitiveType::float64));
    } else if (std::is_same<T, float>::value) {
      return std::shared_ptr<Type>(
          new PrimitiveType(Type::Parameters(), PrimitiveType::float32));
    } else if (std::is_same<T, int64_t>::value) {
      return std::shared_ptr<Type>(
          new PrimitiveType(Type::Parameters(), PrimitiveType::int64));
    } else if (std::is_same<T, uint64_t>::value) {
      return std::shared_ptr<Type>(
          new PrimitiveType(Type::Parameters(), PrimitiveType::uint64));
    } else if (std::is_same<T, int32_t>::value) {
      return std::shared_ptr<Type>(
          new PrimitiveType(Type::Parameters(), PrimitiveType::int32));
    } else if (std::is_same<T, uint32_t>::value) {
      return std::shared_ptr<Type>(
          new PrimitiveType(Type::Parameters(), PrimitiveType::uint32));
    } else if (std::is_same<T, int16_t>::value) {
      return std::shared_ptr<Type>(
          new PrimitiveType(Type::Parameters(), PrimitiveType::int16));
    } else if (std::is_same<T, uint16_t>::value) {
      return std::shared_ptr<Type>(
          new PrimitiveType(Type::Parameters(), PrimitiveType::uint16));
    } else if (std::is_same<T, int8_t>::value) {
      return std::shared_ptr<Type>(
          new PrimitiveType(Type::Parameters(), PrimitiveType::int8));
    } else if (std::is_same<T, uint8_t>::value) {
      return std::shared_ptr<Type>(
          new PrimitiveType(Type::Parameters(), PrimitiveType::uint8));
    } else if (std::is_same<T, bool>::value) {
      return std::shared_ptr<Type>(
          new PrimitiveType(Type::Parameters(), PrimitiveType::boolean));
    } else {
      throw std::invalid_argument(
  ...
  virtual bool accepts(const std::shared_ptr<Type> type) {
    ...
    if (std::is_same<T, double>::value) {
      ...
      return check.get()->equal(std::shared_ptr<Type>(new PrimitiveType(
                                    Type::Parameters(), PrimitiveType::int64)),
                                false);
    } else if (std::is_same<T, uint64_t>::value) {
      return check.get()->equal(std::shared_ptr<Type>(new PrimitiveType(
                                    Type::Parameters(), PrimitiveType::uint64)),
                                false);
    } else if (std::is_same<T, int32_t>::value) {
      return check.get()->equal(std::shared_ptr<Type>(new PrimitiveType(
                                    Type::Parameters(), PrimitiveType::int32)),
                                false);
    } else if (std::is_same<T, uint32_t>::value) {
      return check.get()->equal(std::shared_ptr<Type>(new PrimitiveType(
                                    Type::Parameters(), PrimitiveType::uint32)),
                                false);
    } else if (std::is_same<T, int16_t>::value) {
      return check.get()->equal(std::shared_ptr<Type>(new PrimitiveType(
                                    Type::Parameters(), PrimitiveType::int16)),
                                false);
    } else if (std::is_same<T, uint16_t>::value) {
      return check.get()->equal(std::shared_ptr<Type>(new PrimitiveType(
                                    Type::Parameters(), PrimitiveType::uint16)),
                                false);
    } else if (std::is_same<T, int8_t>::value) {
      return check.get()->equal(std::shared_ptr<Type>(new PrimitiveType(
                                    Type::Parameters(), PrimitiveType::int8)),
                                false);
    } else if (std::is_same<T, uint8_t>::value) {
      return check.get()->equal(std::shared_ptr<Type>(new PrimitiveType(
                                    Type::Parameters(), PrimitiveType::uint8)),
                                false);
    } else if (std::is_same<T, bool>::value) {
      return check.get()->equal(
          std::shared_ptr<Type>(
  ...
  virtual const std::shared_ptr<Content> shallow_copy() const {
    return std::shared_ptr<Content>(
        new RawArrayOf<T>(id_, type_, ptr_, offset_, length_, itemsize_));
  }

  virtual void check_for_iteration() const {
    if (id_.get() != nullptr && id_.get()->length() < length_) {
      util::handle_error(
          failure("len(id) < len(array)", kSliceNone, kSliceNone),
  ...
  getitem_range_nowrap(int64_t start, int64_t stop) const {
    ...
    return std::shared_ptr<Content>(new RawArrayOf<T>(
        id, type_, ptr_, offset_ + start, stop - start, itemsize_));
  }
  ...
  virtual const std::shared_ptr<Content> carry(const Index64 &carry) const {
    ...
    struct Error err = awkward_numpyarray_getitem_next_null_64(
        ...
        reinterpret_cast<uint8_t *>(ptr_.get()), carry.length(), itemsize_,
        byteoffset(), carry.ptr().get());
    ...
    return std::shared_ptr<Content>(
        new RawArrayOf<T>(id, type_, ptr, 0, carry.length(), itemsize_));
  }
...
protected:
  ...
  getitem_next(const SliceRange &range, const Slice &tail,
    ...
    if (range.step() == Slice::none() || range.step() == 1) {
      ...
      awkward_regularize_rangeslice(&start, &stop, step > 0, range.hasstart(),
                                    range.hasstop(), length_);
  ...
  getitem_next(const SliceArray64 &array, const Slice &tail,
    ...
    struct Error err = awkward_regularize_arrayslice_64(
        flathead.ptr().get(), flathead.length(), length_);
    util::handle_error(err, classname(), id_.get());
  ...
  getitem_next(const SliceField &field, const Slice &tail,
               const Index64 &advanced) const {
    throw std::invalid_argument(field.tostring() +
                                std::string(" is not a valid slice type for ") +
                                classname());
  }
  ...
  getitem_next(const SliceFields &fields, const Slice &tail,
               const Index64 &advanced) const {
    throw std::invalid_argument(fields.tostring() +
                                std::string(" is not a valid slice type for ") +
                                classname());
  }
