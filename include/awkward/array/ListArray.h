...
namespace awkward {
template <typename T> class ListArrayOf : public Content {
public:
  ListArrayOf<T>(const std::shared_ptr<Identity> id,
                 const std::shared_ptr<Type> type, const IndexOf<T> starts,
                 const IndexOf<T> stops, const std::shared_ptr<Content> content)
      : Content(id, type), starts_(starts), stops_(stops), content_(content) {}
