...
namespace awkward {
template <typename T> class ListOffsetArrayOf : public Content {
public:
  ListOffsetArrayOf<T>(const std::shared_ptr<Identity> id,
                       const std::shared_ptr<Type> type,
                       const IndexOf<T> offsets,
                       const std::shared_ptr<Content> content)
      : Content(id, type), offsets_(offsets), content_(content) {}
