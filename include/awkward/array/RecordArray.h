...
public:
  ...
  RecordArray(const std::shared_ptr<Identity> id,
              ...
              const std::vector<std::shared_ptr<Content>> &contents,
              const std::shared_ptr<Lookup> &lookup,
              const std::shared_ptr<ReverseLookup> &reverselookup)
      : Content(id, type), contents_(contents), lookup_(lookup),
        reverselookup_(reverselookup), length_(0) {
    assert(contents.size() != 0);
  }
  RecordArray(const std::shared_ptr<Identity> id,
              const std::shared_ptr<Type> type,
              const std::vector<std::shared_ptr<Content>> &contents)
      : Content(id, type), contents_(contents), lookup_(nullptr),
        reverselookup_(nullptr), length_(0) {
    assert(contents.size() != 0);
  }
  RecordArray(const std::shared_ptr<Identity> id,
              const std::shared_ptr<Type> type, int64_t length, bool istuple)
      : Content(id, type), contents_(), lookup_(istuple ? nullptr : new Lookup),
        reverselookup_(istuple ? nullptr : new ReverseLookup), length_(length) {
  }
