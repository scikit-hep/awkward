...
public:
  SliceRange(int64_t start, int64_t stop, int64_t step)
      : start_(start), stop_(stop), step_(step == none() ? 1 : step) {
    assert(step_ != 0);
  }
...
template <typename T> class SliceArrayOf : public SliceItem {
public:
  SliceArrayOf<T>(const IndexOf<T> &index, const std::vector<int64_t> &shape,
                  const std::vector<int64_t> &strides)
      : index_(index), shape_(shape), strides_(strides) {
  ...
  virtual const std::shared_ptr<SliceItem> shallow_copy() const {
    return std::shared_ptr<SliceItem>(
        new SliceArrayOf<T>(index_, shape_, strides_));
  }
  virtual const std::string tostring() const;
  virtual bool preserves_type(const std::shared_ptr<Type> &type,
                              const Index64 &advanced) const {
...
public:
  ...
  virtual bool preserves_type(const std::shared_ptr<Type> &type,
                              const Index64 &advanced) const {
    return type.get() != nullptr && type.get()->numfields() != -1 &&
           util::subset(keys_, type.get()->keys());
  }
...
public:
  ...
  template <typename T> void append(const SliceArrayOf<T> &item);
  void become_sealed();
