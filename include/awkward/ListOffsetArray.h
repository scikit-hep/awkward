...
namespace awkward {
  ...
  public:
    ListOffsetArray(const Index offsets, const std::shared_ptr<Content> content, std::shared_ptr<Index> keys)
        : offsets_(offsets)
        , content_(content)
        , keys_(keys) { }
    ...
    std::shared_ptr<Index> keys() const { return keys_; }
