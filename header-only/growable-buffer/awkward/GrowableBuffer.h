...
namespace awkward {
  ...
  public:
    ...
    /// to be copied.
    void
    concatenate_to_from(PRIMITIVE *to_ptr,
                        size_t offset,
  ...
  public:
    ...
    extend(const PRIMITIVE *ptr, size_t size) {
      ...
      if (size > unfilled_items) {
        ...
        add_panel(size - unfilled_items > ptr_->reserved()
                      ? size - unfilled_items
                      : ptr_->reserved());
    ...
    /// to one contiguously allocated `external_pointer`.
    void
    concatenate_from(PRIMITIVE *external_pointer,
                     size_t to,
                     size_t from) const noexcept {
      if (external_pointer) {
    ...
    /// `external_pointer`.
    void
    append(PRIMITIVE *external_pointer,
           size_t offset,
           size_t from,
           int64_t length) const noexcept {
      if (external_pointer) {
