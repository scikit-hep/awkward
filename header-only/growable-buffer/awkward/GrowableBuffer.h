...
template <template <class...> class TT> std::false_type is_tt_impl(...);

template <template <class...> class TT, class T>
using is_tt =
    decltype(is_tt_impl<TT>(std::declval<typename std::decay<T>::type>()));
...
public:
  ...
  Panel(size_t reserved)
      : ptr_(std::unique_ptr<PRIMITIVE[]>(new PRIMITIVE[reserved])), length_(0),
        reserved_(reserved), next_(nullptr) {}
  ...
  Panel(std::unique_ptr<PRIMITIVE[]> ptr, size_t length, size_t reserved)
      : ptr_(std::move(ptr)), length_(length), reserved_(reserved),
        next_(nullptr) {}
  ...
  ~Panel() {
    for (std::unique_ptr<Panel> current = std::move(next_); current;
  ...
  void fill_panel(PRIMITIVE datum) { ptr_.get()[length_++] = datum; }

  /// @brief Pointer to the next panel.
  std::unique_ptr<Panel> &next() { return next_; }

  /// @brief Currently used number of elements in the panel.
  size_t current_length() { return length_; }
  ...
  std::unique_ptr<PRIMITIVE[]> &data() { return ptr_; }

  /// @brief Copies the data from a panel to one contiguously allocated
  /// `to_ptr`.
  ///
  /// @param to_ptr One contiguously allocated panel.
  /// @param offset Distance between `to_ptr` and the pointer to the destination
  /// where the accumulated data is copied.
  /// @param from Distance between `ptr` and pointer to the source of the data
  /// to be copied.
  /// @param length Length of the data to be copied.
  void append(PRIMITIVE *to_ptr, size_t offset, size_t from,
              int64_t length) const noexcept {
    memcpy(to_ptr + offset, reinterpret_cast<void *>(ptr_.get() + from),
           length * sizeof(PRIMITIVE) - from);
  }

  /// @brief Copies and concatenates the accumulated data from multiple panels
  /// `ptr_` to one contiguously allocated `to_ptr`.
  ///
  /// @param to_ptr One contiguously allocated panel.
  /// @param offset Distance between `to_ptr` and the pointer to the destination
  /// where the accumulated data is copied.
  /// @param from Distance between `ptr` and pointer to the source of the data
  /// to be copied.
  void concatenate_to_from(PRIMITIVE *to_ptr, size_t offset,
  ...
  void concatenate_to(PRIMITIVE *to_ptr, size_t offset) const noexcept {
    memcpy(to_ptr + offset, reinterpret_cast<void *>(ptr_.get()),
...
template <typename PRIMITIVE> class GrowableBuffer {
public:
  ...
  static GrowableBuffer<PRIMITIVE> empty(const BuilderOptions &options,
    ...
    return GrowableBuffer(
        options, std::unique_ptr<PRIMITIVE[]>(new PRIMITIVE[(size_t)actual]), 0,
        actual);
  }

  /// @brief Creates a GrowableBuffer in which all elements are initialized to
  /// `0`.
  ///
  /// @param options Initial size configuration for building a panel.
  ...
  static GrowableBuffer<PRIMITIVE> full(const BuilderOptions &options,
                                        PRIMITIVE value, int64_t length) {
    int64_t actual = options.initial();
    if (actual < length) {
      actual = length;
    }
  ...
  copy_as(const GrowableBuffer<PRIMITIVE> &other) {
    ...
    return GrowableBuffer<TO_PRIMITIVE>(
        BuilderOptions(actual, other.options().resize()), std::move(ptr), len,
        actual);
  }
  ...
  GrowableBuffer(const BuilderOptions &options,
                 std::unique_ptr<PRIMITIVE[]> ptr, int64_t length,
                 int64_t reserved)
      : options_(options), length_(0),
  ...
  GrowableBuffer(GrowableBuffer &&other) noexcept
      : options_(other.options_), length_(other.length_),
        panel_(std::move(other.panel_)), ptr_(other.ptr_) {}
  ...
  /// it is always less than or equal to #reserved because of
  /// allocations of new panels.
  size_t length() const { return length_ + ptr_->current_length(); }
  ...
  size_t nbytes() const { return length() * sizeof(PRIMITIVE); }
  ...
  /// @brief Like append, but the type signature returns the reference to
  /// `PRIMITIVE`.
  ...
  /// @brief Copies and concatenates all accumulated data from multiple panels
  /// to one contiguously allocated `external_pointer`.
  ...
  /// @brief Copies and concatenates all accumulated data from multiple panels
  /// to one contiguously allocated `external_pointer`.
  void concatenate_from(PRIMITIVE *external_pointer, size_t to,
                        size_t from) const noexcept {
    if (external_pointer) {
  ...
  /// @brief Copies data from a panel to one contiguously allocated
  /// `external_pointer`.
  void append(PRIMITIVE *external_pointer, size_t offset, size_t from,
              int64_t length) const noexcept {
    if (external_pointer) {
