...
namespace awkward {
  ...
  namespace LayoutBuilder {
    ...
    private:
      ...
      template <std::size_t... S>
      std::vector<bool>
      field_is_valid(std::index_sequence<S...>,
    ...
    private:
      ...
      template <std::size_t... S>
      std::vector<bool>
      content_is_valid(std::index_sequence<S...>,
                       std::string &error) const noexcept {
      ...
      is_valid(std::string &error) const noexcept {
        if (content_.length() != index_.length()) {
          ...
          out << "Indexed node" << id_ << " has content length "
              << content_.length() << " but index has length "
              << index_.length() << "\n";
          error.append(out.str());
    ...
    public:
      ...
      to_buffer(void *buffer, const char *name) const noexcept {
        if (std::string(name) ==
          ...
          mask_.append(
              reinterpret_cast<uint8_t *>(buffer), mask_.length() - 1, 0, 1);
        }
        content_.to_buffer(buffer, name);
      }
