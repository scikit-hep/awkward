...
template <std::size_t ENUM, typename BUILDER> class Field {
public:
  using Builder = BUILDER;

  /// @brief Converts index as field string.
  std::string index_as_field() const { return std::to_string(index); }
...
template <typename PRIMITIVE> class Numpy {
public:
  ...
  Numpy()
      : data_(awkward::GrowableBuffer<PRIMITIVE>(
  ...
  void append(PRIMITIVE x) noexcept { data_.append(x); }

  /// @brief Inserts an entire array of `PRIMITIVE` type data.
  ///
  /// Just an interface; not actually faster than calling append many times.
  void extend(PRIMITIVE *ptr, size_t size) noexcept { data_.extend(ptr, size); }

  /// @brief Parameters for the builder form.
  const std::string &parameters() const noexcept { return parameters_; }
  ...
  bool is_valid(std::string & /* error */) const noexcept { return true; }
  ...
  /// @brief Copies and concatenates the accumulated data in the builder buffer
  /// to a user-defined pointer if the given node name matches with the node
  /// associated with the builder.
  void to_buffer(void *buffer, const char *name) const noexcept {
    if (std::string(name) ==
        std::string("node" + std::to_string(id_) + "-data")) {
  ...
  std::string form() const {
    ...
    if (std::is_arithmetic<PRIMITIVE>::value) {
      return "{ \"class\": \"NumpyArray\", \"primitive\": \"" +
             type_to_name<PRIMITIVE>() + "\"" + params + ", \"form_key\": \"" +
             form_key.str() + "\" }";
      ...
      throw std::runtime_error("type " + std::string(typeid(PRIMITIVE).name()) +
                               "is not supported");
    }
  }
...
/// to an offsets array, which specifies the starting and stopping index of each
/// list.
///
/// The offsets must have at least length `1` (corresponding to an empty array).
///
/// The offsets values can be 64-bit signed integers `int64`, 32-bit signed
...
template <typename PRIMITIVE, typename BUILDER> class ListOffset {
public:
  /// @brief Creates a new ListOffset layout builder by allocating a new
  /// `offset`
  ///  buffer, using `AWKWARD_LAYOUTBUILDER_DEFAULT_OPTIONS` for initializing
  ///  the buffer.
  ListOffset()
      : offsets_(awkward::GrowableBuffer<PRIMITIVE>(
            AWKWARD_LAYOUTBUILDER_DEFAULT_OPTIONS)) {
    offsets_.append(0);
    size_t id = 0;
    set_id(id);
  ...
  /// @brief Creates a new ListOffset layout builder by allocating a new
  /// `offset` buffer, taking `options` from {@link BuilderOptions
  /// BuilderOptions} for initializing the buffer.
  ///
  /// @param options Initial size configuration of a buffer.
  ...
  size_t length() const noexcept { return offsets_.length() - 1; }
  ...
  /// @brief Copies and concatenates the accumulated data in the builder buffer
  /// to a user-defined pointer if the given node name matches with the node
  /// associated with the builder; otherwise, it searches the builder contents
  /// to locate a matching node.
  void to_buffer(void *buffer, const char *name) const noexcept {
    if (std::string(name) ==
        std::string("node" + std::to_string(id_) + "-offsets")) {
...
public:
  ...
  void buffer_nbytes(
      std::map<std::string, size_t> & /* names_nbytes */) const noexcept {}
  ...
  void to_char_buffers(
      std::map<std::string, uint8_t *> & /* buffers */) const noexcept {}

  /// @brief Generates a unique description of the builder and its
  /// contents in the form of a JSON-like string.
  std::string form() const noexcept { return "{ \"class\": \"EmptyArray\" }"; }
...
public:
  ...
  template <std::size_t INDEX>
  typename RecordFieldType<INDEX>::Builder &content() noexcept {
  ...
  void set_id(size_t &id) noexcept {
    ...
    for (size_t i = 0; i < fields_count_; i++) {
      visit_at(contents, i,
               [&id](auto &content) { content.builder.set_id(id); });
    }
  }
  ...
  bool is_valid(std::string &error) const noexcept {
    ...
    for (size_t i = 0; i < lengths.size(); i++) {
      if (length == -1) {
        ...
        std::stringstream out;
        out << "Record node" << id_ << " has field \"" << fields().at(i)
            << "\" length " << lengths[i]
    ...
    return std::none_of(std::cbegin(valid_fields), std::cend(valid_fields),
...
private:
  ...
  void map_fields(std::index_sequence<S...>) noexcept {
  ...
  /// returns the vector.
  template <std::size_t... S>
  std::vector<size_t> field_lengths(std::index_sequence<S...>) const noexcept {
    return std::vector<size_t>({std::get<S>(contents).builder.length()...});
  }

  /// @brief Helper function of #is_valid for checking the validity.
  template <std::size_t... S>
  std::vector<bool> field_is_valid(std::index_sequence<S...>,
...
///
/// @tparam BUILDERS The types of builder contents.
template <typename... BUILDERS> class Tuple {
...
public:
  ...
  /// @brief Returns the reference to the builder contents at `INDEX`.
  template <std::size_t INDEX> TupleContentType<INDEX> &content() noexcept {
  ...
  void clear() noexcept {
    for (size_t i = 0; i < fields_count_; i++) {
      visit_at(contents, i, [](auto &content) { content.clear(); });
    }
  }

  /// @brief Current number of records in the first index of the tuple.
  size_t length() const noexcept { return (std::get<0>(contents).length()); }
    ...
    for (size_t i = 0; i < lengths.size(); i++) {
      if (length == -1) {
        ...
        out << "Record node" << id_ << " has index \"" << i << "\" length "
            << lengths[i] << " that differs from the first length " << length
            << "\n";
        error.append(out.str());
  ...
  void to_buffers(std::map<std::string, void *> &buffers) const noexcept {
    for (size_t i = 0; i < fields_count_; i++) {
      visit_at(contents, i,
               [&buffers](auto &content) { content.to_buffers(buffers); });
    }
  }
  ...
  to_char_buffers(std::map<std::string, uint8_t *> &buffers) const noexcept {
    for (size_t i = 0; i < fields_count_; i++) {
      visit_at(contents, i,
               [&buffers](auto &content) { content.to_char_buffers(buffers); });
    }
  }
  ...
  std::string form() const noexcept {
    ...
    for (size_t i = 0; i < fields_count_; i++) {
      ...
      auto contents_form = [&out](auto &content) { out << content.form(); };
      visit_at(contents, i, contents_form);
    }
    out << "], ";
    out << params << "\"form_key\": \"" << form_key.str() << "\" }";
...
private:
  ...
  /// @brief Helper function of #is_valid for checking the validity.
  template <std::size_t... S>
  std::vector<bool> content_is_valid(std::index_sequence<S...>,
                                     std::string &error) const noexcept {
...
///
/// A multidimensional {@link Numpy NumpyArray} is equivalent to a
/// one-dimensional
/// {@link Numpy NumpyArray} nested within several RegularArrays, one for each
/// dimension. However, RegularArrays can be used to make lists of any other
/// type.
///
/// @tparam SIZE
/// @tparam BUILDER The type of builder content.
template <unsigned SIZE, typename BUILDER> class Regular {
public:
  ...
  void end_list() noexcept { length_++; }
...
/// @tparam BUILDER The type of builder content.
template <typename PRIMITIVE, typename BUILDER> class Indexed {
public:
  /// @brief Creates a new Indexed layout builder by allocating a new `index`
  /// buffer, using `AWKWARD_LAYOUTBUILDER_DEFAULT_OPTIONS` for initializing the
  /// buffer.
  Indexed()
      : index_(awkward::GrowableBuffer<PRIMITIVE>(
            AWKWARD_LAYOUTBUILDER_DEFAULT_OPTIONS)) {
    size_t id = 0;
    set_id(id);
  }
  ...
  size_t length() const noexcept { return index_.length(); }
  ...
  void to_buffer(void *buffer, const char *name) const noexcept {
    if (std::string(name) ==
        std::string("node" + std::to_string(id_) + "-index")) {
...
/// @tparam PRIMITIVE The type of `index` buffer.
/// @tparam BUILDER The type of builder content.
template <typename PRIMITIVE, typename BUILDER> class IndexedOption {
public:
  /// @brief Creates a new IndexedOption layout builder by allocating a new
  /// `index` buffer, using `AWKWARD_LAYOUTBUILDER_DEFAULT_OPTIONS` for
  ...
  /// @brief Creates a new IndexedOption layout builder by allocating a new
  /// `index` buffer, taking `options` from {@link BuilderOptions
  /// BuilderOptions} for initializing the buffer.
  ///
  /// @param options Initial size configuration of a buffer.
  IndexedOption(const awkward::BuilderOptions &options)
      : index_(awkward::GrowableBuffer<PRIMITIVE>(options)), last_valid_(-1) {
    size_t id = 0;
    set_id(id);
  }
  ...
  void append_invalid() noexcept { index_.append(-1); }
...
/// It exists to satisfy systems that formally require this high-level type
/// without the overhead of generating an array of all True or all False values.
///
...
///
/// @tparam BUILDER The type of builder content.
template <typename BUILDER> class Unmasked {
public:
  ...
  /// @brief Current length of the content.
  size_t length() const noexcept { return content_.length(); }
...
/// if #valid_when = `false`.
///
/// @tparam VALID_WHEN A boolean value which determines when the builder content
/// are valid.
/// @tparam BUILDER The type of builder content.
template <bool VALID_WHEN, typename BUILDER> class ByteMasked {
public:
  /// @brief Creates a new ByteMasked layout builder by allocating a new `mask`
  ///  buffer, using `AWKWARD_LAYOUTBUILDER_DEFAULT_OPTIONS` for initializing
  ///  the buffer.
  ByteMasked()
      : mask_(awkward::GrowableBuffer<int8_t>(
            AWKWARD_LAYOUTBUILDER_DEFAULT_OPTIONS)) {
    size_t id = 0;
    set_id(id);
  }
  ...
  bool valid_when() const noexcept { return valid_when_; }
  ...
  ///
  /// After this, `size` number of valid elements are inserted in the builder
  /// content.
  ///
  /// Just an interface; not actually faster than calling append many times.
  BUILDER &extend_valid(size_t size) noexcept {
  ...
  /// @brief Inserts `size` number of !valid_when in the mask.
  ///
  /// After this, `size` number of dummy (invalid) values are inserted in the
  /// builder content.
  ///
  /// Just an interface; not actually faster than calling append many times.
  ...
  size_t length() const noexcept { return mask_.length(); }
  ...
  void to_buffer(void *buffer, const char *name) const noexcept {
    if (std::string(name) ==
        std::string("node" + std::to_string(id_) + "-mask")) {
...
/// @class BitMasked
///
/// @brief Builds a BitMaskedArray in which mask values are packed into a
/// bitmap.
///
/// It has an additional parameter, #lsb_order; If `true`, the position of each
/// bit is in `Least-Significant Bit order (LSB)` and if it is `false`, then in
/// `Most-Significant Bit order (MSB)`.
///
/// This is similar to NumPy's
...
/// @tparam VALID_WHEN A boolean value which determines when the builder content
/// are valid.
/// @tparam LSB_ORDER A boolean value which determines whether the position of
/// each bit is in LSB order or not.
/// @tparam BUILDER The type of builder content.
template <bool VALID_WHEN, bool LSB_ORDER, typename BUILDER> class BitMasked {
public:
  /// @brief Creates a new BitMasked layout builder by allocating a new `mask`
  /// buffer, using `AWKWARD_LAYOUTBUILDER_DEFAULT_OPTIONS` for initializing the
  /// buffer.
  BitMasked()
      : mask_(awkward::GrowableBuffer<uint8_t>(
            AWKWARD_LAYOUTBUILDER_DEFAULT_OPTIONS)),
  ...
  bool lsb_order() const noexcept { return lsb_order_; }

  /// @brief Sets a bit in the mask.
  /// If current_byte_ and cast_: `0` indicates `null`, `1` indicates `valid`
  /// and vice versa.
  ///
  /// After this, a valid element is inserted in the builder content.
  BUILDER &append_valid() noexcept {
  ...
  /// @brief Sets current_byte_ and cast_ default to null, no change in
  /// current_byte_.
  ///
  /// After this, a dummy (invalid) value is inserted in the builder content.
  BUILDER &append_invalid() noexcept {
  ...
  /// @brief Current length of the `mask` buffer.
  size_t length() const noexcept {
    return mask_.length() > 0 ? (mask_.length() - 1) * 8 + current_index_
                              : current_index_;
  }
  ...
  void to_buffer(void *buffer, const char *name) const noexcept {
    if (std::string(name) ==
      ...
      mask_.append(reinterpret_cast<uint8_t *>(buffer), mask_.length() - 1, 0,
                   1);
    }
    content_.to_buffer(buffer, name);
  }
...
private:
  ...
  /// If #valid_when equals `false`: `0` indicates `valid`, `1` indicates
  /// `null`.
  void append_end() {
    current_index_ += 1;
    if (valid_when_) {
...
/// index, which is an array of integers indicating which element from the
/// content to draw from.
///
/// The index values can be 64-bit signed integers `int64`, 32-bit signed
/// integers `int32` or 32-bit unsigned integers `uint32` and the tags values
/// can be 8-bit signed integers.
///
/// @tparam TAGS The type of `tags` buffer.
/// @tparam INDEX The type of `index` buffer.
/// @tparam BUILDERS The types of builder contents.
template <typename TAGS, typename INDEX, typename... BUILDERS> class Union {
public:
  ...
  /// index buffers, using `AWKWARD_LAYOUTBUILDER_DEFAULT_OPTIONS` for
  /// initializing the buffer.
  Union()
      : tags_(awkward::GrowableBuffer<TAGS>(
            AWKWARD_LAYOUTBUILDER_DEFAULT_OPTIONS)),
        index_(awkward::GrowableBuffer<INDEX>(
            AWKWARD_LAYOUTBUILDER_DEFAULT_OPTIONS)) {
    size_t id = 0;
    set_id(id);
  ...
  template <std::size_t I> ContentType<I> &content() noexcept {
    return std::get<I>(contents_);
  }

  /// @brief Inserts the current tag in the `tags` buffer and the next index in
  /// the `index` buffer and returns the reference to the content of the current
  /// builder.
  template <std::size_t TAG> ContentType<TAG> &append_content() noexcept {
  ...
  void clear() noexcept {
    ...
    tags_.clear();
    index_.clear();
    auto clear_contents = [](auto &content) { content.clear(); };
    for (size_t i = 0; i < contents_count_; i++) {
  ...
  size_t length() const noexcept { return tags_.length(); }
  ...
  bool is_valid(std::string &error) const noexcept {
    ...
    return std::none_of(std::cbegin(valid_contents), std::cend(valid_contents),
                        std::logical_not<bool>());
  }
  ...
  void to_buffers(std::map<std::string, void *> &buffers) const noexcept {
    ...
    for (size_t i = 0; i < contents_count_; i++) {
      visit_at(contents_, i,
               [&buffers](auto &content) { content.to_buffers(buffers); });
    }
  }

  /// @brief Copies and concatenates the accumulated data in the builder buffers
  /// to user-defined pointers if the given node name matches with any one of
  /// the nodes associated with the builder; otherwise, it searches the builder
  /// contents to locate a matching node.
  void to_buffer(void *buffer, const char *name) const noexcept {
    auto index_sequence((std::index_sequence_for<BUILDERS...>()));

    if (std::string(name) ==
        std::string("node" + std::to_string(id_) + "-tags")) {
  ...
  std::string form() const noexcept {
    ...
    for (size_t i = 0; i < contents_count_; i++) {
      ...
      auto contents_form = [&](auto &content) { out << content.form(); };
...
private:
  ...
  /// and returns the vector.
  template <std::size_t... S>
  std::vector<size_t> content_lengths(std::index_sequence<S...>) const {
