...
template <typename T> inline const std::string type_to_name() {
  if (is_integral_v<T>) {
    if (is_signed_v<T>) {
      if (sizeof(T) == 1) {
        return "int8";
...
template <> inline const std::string type_to_name<bool>() {
...
template <> inline const std::string type_to_name<char>() {
  // This takes precedence over the unspecialized template, and therefore any
...
template <typename T>
constexpr bool is_iterable<T, void_t<decltype(std::declval<T>().begin()),
...
template <typename T> bool is_awkward_type() {
...
template <size_t INDEX> struct visit_impl {
...
void visit_at(std::tuple<CONTENTs...> const &contents, size_t index,
              FUNCTION fun) {
...
///
/// Note: use with caution, beware of a potential mismatch between retrieved
/// values!
