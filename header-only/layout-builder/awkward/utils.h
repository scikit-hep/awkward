...
namespace awkward {
  ...
  type_to_form(int64_t form_key_id) {
    ...
    if (is_iterable<T>) {
      ...
      return "{\"class\": \"ListOffsetArray\", \"offsets\": \"" +
             ...
             "\", "
             "\"content\":" +
             type_to_form<value_type, OFFSETS>(form_key_id) + ", " +
  ...
  void
  visit_at(std::tuple<CONTENTs...> const &contents,
           size_t index,
           FUNCTION fun) {
  ...
  buffer_name_helper(const LayoutBuilder *builder) {
  ...
  template <typename LayoutBuilder>
  std::vector<size_t>
  buffer_size_helper(const LayoutBuilder *builder) {
    std::map<std::string, size_t> names_nbytes = {};
  ...
  template <typename LayoutBuilder>
  size_t
  num_buffers_helper(const LayoutBuilder *builder) {
    std::map<std::string, size_t> names_nbytes = {};
    builder->buffer_nbytes(names_nbytes);
