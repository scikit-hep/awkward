...
\begin{align*}
\underbrace{\mbox{ufunc}(f)(\mintinline{python}{array})}_{\mbox{\scriptsize mapped array}}(\mintinline{python}{i}) = & \,f(\underbrace{\mintinline{python}{array}(\mintinline{python}{i})}_{\mbox{\scriptsize mapped scalar}}) \\[0.5\baselineskip]
\mintinline{python}{numpy.vectorize(f)(array)[i]} = & \,\mintinline{python}{f(array[i])}\mbox{.}
\end{align*}
...
\noindent Note that \mintinline{python}{index} is to the right (transforms the input) of \mintinline{python}{array}, whereas $\mbox{ufunc}(f)$ put $f$ to the left (transforms the output) of \mintinline{python}{array}.

Numpy uses the same syntax for this function composition, \mintinline{python}{array[index]}, as it does for function evaluation, \mintinline{python}{array[i]}. Let's illustrate this with an extended example.
...
In \mintinline{python}{GoF}, the functions are composed before being transformed into arrays, and in \mintinline{python}{G[F]}, the arrays themselves are composed via integer-array indexing. Function composition is associative, so we should be able to change the order of two integer-array indexings.

To demonstrate associativity, introduce another array, which need not have integer \mintinline{python}{dtype}.
\begin{minted}{python}
