...
Such an array could still be sliced because slicing composes an integer array $\mintinline{python}{index}: [0, x) -> [0, n_1)$ to the {\it input} of the array. For example,
\begin{align*}
...
Similarly, if we had nested records, with $s_1$ containing \mintinline{python}{dtype} $d$ and $s_2$ containing a record with fields $t_1$ and $t_2$, our options for commuting indexes would be limited to one move:
\begin{align*}
