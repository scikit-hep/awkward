...
template <typename T> class pyobject_deleter {
public:
  pyobject_deleter(PyObject *pyobj) : pyobj_(pyobj) { Py_INCREF(pyobj_); }
  void operator()(T const *p) { Py_DECREF(pyobj_); }

private:
  PyObject *pyobj_;
};

py::class_<ak::Type, std::shared_ptr<ak::Type>> make_Type(py::handle m,
                                                          std::string name) {
  return (py::class_<ak::Type, std::shared_ptr<ak::Type>>(m, name.c_str())
              .def("__eq__",
                   [](std::shared_ptr<ak::Type> self,
                      std::shared_ptr<ak::Type> other) -> bool {
...
py::object box(std::shared_ptr<ak::Content> content) {
  if (ak::NumpyArray *raw = dynamic_cast<ak::NumpyArray *>(content.get())) {
    if (raw->isscalar()) {
      return py::array(py::buffer_info(raw->byteptr(), raw->itemsize(),
                                       raw->format(), raw->ndim(), raw->shape(),
                                       raw->strides()))
          .attr("item")();
    } else {
      return py::cast(*raw);
...
template <typename T> std::string repr(T &self) { return self.tostring(); }

template <typename T> int64_t len(T &self) { return self.length(); }
...
py::class_<ak::IndexOf<T>> make_IndexOf(py::handle m, std::string name) {
  return (
      py::class_<ak::IndexOf<T>>(m, name.c_str(), py::buffer_protocol())
          .def_buffer([](ak::IndexOf<T> &self) -> py::buffer_info {
            return py::buffer_info(
                reinterpret_cast<void *>(
                    reinterpret_cast<ssize_t>(self.ptr().get()) +
                    self.offset() * sizeof(T)),
                sizeof(T), py::format_descriptor<T>::format(), 1,
                {(ssize_t)self.length()}, {(ssize_t)sizeof(T)});
          })

          .def(py::init([name](py::array_t<T, py::array::c_style |
                                                  py::array::forcecast>
                                   array) -> ak::IndexOf<T> {
            py::buffer_info info = array.request();
            if (info.ndim != 1) {
              throw std::invalid_argument(
                  name + std::string(" must be built from a one-dimensional "
                                     "array; try array.ravel()"));
            }
...
template <typename T> py::tuple location(const T &self) {
  if (self.id().get() == nullptr) {
    throw std::invalid_argument(
        self.classname() +
        std::string(" instance has no associated id (use 'setid' to assign one "
                    "to the array it is in)"));
  }
...
template <typename T> py::object getid(T &self) { return box(self.id()); }

template <typename T> void setid(T &self, py::object id) {
  self.setid(unbox_id_none(id));
}

template <typename T> void setid_noarg(T &self) { self.setid(); }

template <typename T>
py::class_<ak::IdentityOf<T>> make_IdentityOf(py::handle m, std::string name) {
  return (
      py::class_<ak::IdentityOf<T>>(m, name.c_str(), py::buffer_protocol())
          .def_buffer([](ak::IdentityOf<T> &self) -> py::buffer_info {
            return py::buffer_info(
                reinterpret_cast<void *>(
                    reinterpret_cast<ssize_t>(self.ptr().get()) +
                    self.offset() * sizeof(T)),
                sizeof(T), py::format_descriptor<T>::format(), 2,
          ...
          .def(py::init([](ak::Identity::Ref ref,
                           ak::Identity::FieldLoc fieldloc, int64_t width,
                           int64_t length) {
            return ak::IdentityOf<T>(ref, fieldloc, width, length);
          }))

          .def(py::init(
              [name](ak::Identity::Ref ref, ak::Identity::FieldLoc fieldloc,
                ...
                if (info.ndim != 2) {
                  throw std::invalid_argument(
                      name +
                      std::string(
                          " must be built from a two-dimensional array"));
                }
                if (info.strides[0] != sizeof(T) * info.shape[1] ||
                  ...
                  throw std::invalid_argument(
                      name +
                      std::string(
                          " must be built from a contiguous array "
                          "(array.stries == (array.shape[1]*array.itemsize, "
                          "array.itemsize)); try array.copy()"));
                }
                return ak::IdentityOf<T>(
                    ref, fieldloc, 0, info.shape[1], info.shape[0],
                    std::shared_ptr<T>(reinterpret_cast<T *>(info.ptr),
                                       pyobject_deleter<T>(array.ptr())));
              }))
          ...
          .def_property_readonly(
              "array",
              [](py::buffer &self) -> py::array { return py::array(self); })
          .def("location_at_str", &ak::IdentityOf<T>::location_at)
          .def("location_at",
               [](const ak::Identity &self, int64_t at) -> py::tuple {
...
void toslice_part(ak::Slice &slice, py::object obj) {
  if (py::isinstance<py::int_>(obj)) {
    // FIXME: what happens if you give this a Numpy integer? a Numpy
    // 0-dimensional array?
    slice.append(
        std::shared_ptr<ak::SliceItem>(new ak::SliceAt(obj.cast<int64_t>())));
  }
    ...
    slice.append(
        std::shared_ptr<ak::SliceItem>(new ak::SliceRange(start, stop, step)));
  }
    ...
    slice.append(std::shared_ptr<ak::SliceItem>(
        new ak::SliceField(obj.cast<std::string>())));
  }
    ...
    if (all_strings && strings.size() != 0) {
      slice.append(
          std::shared_ptr<ak::SliceItem>(new ak::SliceFields(strings)));
    } else {
      ...
      if (info.format.compare("?") == 0) {
        ...
        for (auto x : nonzero_tuple.cast<py::tuple>()) {
          py::object intarray_object = py::module::import("numpy").attr(
              "asarray")(x.cast<py::object>(),
          ...
          ak::Index64 index(std::shared_ptr<int64_t>(
                                reinterpret_cast<int64_t *>(intinfo.ptr),
                                pyobject_deleter<int64_t>(intarray.ptr())),
                            0, shape[0]);
          slice.append(std::shared_ptr<ak::SliceItem>(
              new ak::SliceArray64(index, shape, strides)));
        }
      }
        ...
        if (py::isinstance<py::array>(obj) && format.compare("c") != 0 &&
            format.compare("b") != 0 && format.compare("B") != 0 &&
            format.compare("h") != 0 && format.compare("H") != 0 &&
            format.compare("i") != 0 && format.compare("I") != 0 &&
            format.compare("l") != 0 && format.compare("L") != 0 &&
            format.compare("q") != 0 && format.compare("Q") != 0) {
          throw std::invalid_argument(
              "arrays used as an index must be integer or boolean");
        }

        py::object intarray_object = py::module::import("numpy").attr(
            "asarray")(array, py::module::import("numpy").attr("int64"));
        py::array intarray = intarray_object.cast<py::array>();
        py::buffer_info intinfo = intarray.request();
        std::vector<int64_t> shape;
        std::vector<int64_t> strides;
        ...
        ak::Index64 index(
            std::shared_ptr<int64_t>(reinterpret_cast<int64_t *>(intinfo.ptr),
                                     pyobject_deleter<int64_t>(intarray.ptr())),
            0, shape[0]);
        slice.append(std::shared_ptr<ak::SliceItem>(
            new ak::SliceArray64(index, shape, strides)));
    ...
    throw std::invalid_argument(
        ...
        "(`None`), and integer or boolean arrays (possibly jagged) are valid "
        "indices");
  }
}
...
py::class_<ak::Slice, std::shared_ptr<ak::Slice>> make_Slice(py::handle m,
                                                             std::string name) {
  return (py::class_<ak::Slice, std::shared_ptr<ak::Slice>>(m, name.c_str())
              .def(py::init([](py::object obj) { return toslice(obj); }))
...
make_Iterator(py::handle m, std::string name) {
  ...
  return (
      py::class_<ak::Iterator, std::shared_ptr<ak::Iterator>>(m, name.c_str())
...
template <typename T> py::object getitem(T &self, py::object obj) {
  if (py::isinstance<py::int_>(obj)) {
  ...
  if (py::isinstance<py::slice>(obj)) {
    ...
    // NOTE: control flow can pass through here; don't make the last line an
    // 'else'!
  }
...
void fillable_fill(ak::FillableArray &self, py::handle obj) {
  if (obj.is(py::none())) {
    ...
    throw std::invalid_argument(std::string("cannot convert ") +
                                obj.attr("__repr__")().cast<std::string>() +
                                std::string(" to an array element"));
  }
}

py::class_<ak::FillableArray> make_FillableArray(py::handle m,
                                                 std::string name) {
  return (
      py::class_<ak::FillableArray>(m, name.c_str())
          .def(
              py::init([](int64_t initial, double resize) -> ak::FillableArray {
                return ak::FillableArray(ak::FillableOptions(initial, resize));
              }),
              py::arg("initial") = 1024, py::arg("resize") = 2.0)
          .def_property_readonly("_ptr",
                                 [](ak::FillableArray *self) -> size_t {
          ...
          .def("snapshot",
               [](ak::FillableArray &self) -> py::object {
                 return box(self.snapshot());
               })
          .def("__getitem__", &getitem<ak::FillableArray>)
          .def("__iter__",
               [](ak::FillableArray &self) -> ak::Iterator {
          ...
          .def("bytestring",
               [](ak::FillableArray &self, py::bytes x) -> void {
                 self.bytestring(x.cast<std::string>());
               })
          .def("string",
               [](ak::FillableArray &self, py::str x) -> void {
          ...
          .def(
              "beginrecord",
              [](ak::FillableArray &self, py::object name) -> void {
                if (name.is(py::none())) {
                  self.beginrecord();
                } else {
              ...
              py::arg("name") = py::none())
          .def("field",
               [](ak::FillableArray &self, const std::string &x) -> void {
                 self.field_check(x);
               })
...
template <typename T> std::shared_ptr<ak::Type> inner(T &self) {
  return self.inner();
}
...
template <typename T> py::dict emptydict(T &self) { return py::dict(); }
...
py::dict parameters2dict(const ak::Type::Parameters &in) {
  ...
  for (auto pair : in) {
    ...
    py::str pykey(PyUnicode_DecodeUTF8(cppkey.data(), cppkey.length(),
                                       "surrogateescape"));
    py::str pyvalue(PyUnicode_DecodeUTF8(cppvalue.data(), cppvalue.length(),
                                         "surrogateescape"));
...
template <typename T> py::dict getparameters(T &self) {
  return parameters2dict(self.parameters());
}

template <typename T> void setparameters(T &self, py::object parameters) {
...
type_methods(py::class_<T, std::shared_ptr<T>, ak::Type> &x) {
  return x.def("__repr__", &T::tostring)
      ...
      .def("haskey", &T::haskey)
      .def("keyaliases",
           [](T &self, int64_t fieldindex) -> std::vector<std::string> {
             return self.keyaliases(fieldindex);
           })
      .def("keyaliases",
           [](T &self, std::string key) -> std::vector<std::string> {
...
make_ArrayType(py::handle m, std::string name) {
  return type_methods(
      py::class_<ak::ArrayType, std::shared_ptr<ak::ArrayType>, ak::Type>(
          m, name.c_str())
          .def(py::init([](std::shared_ptr<ak::Type> type, int64_t length,
                           py::object parameters) -> ak::ArrayType {
                 return ak::ArrayType(dict2parameters(parameters), type,
                                      length);
               }),
               py::arg("type"), py::arg("length"),
               py::arg("parameters") = py::none())
          .def_property_readonly("type", &ak::ArrayType::type)
          .def_property_readonly("length", &ak::ArrayType::length)
          .def_property("parameters", &getparameters<ak::ArrayType>,
                        &setparameters<ak::ArrayType>)
          .def(py::pickle(
              [](const ak::ArrayType &self) {
              ...
              [](py::tuple state) {
                return ak::ArrayType(dict2parameters(state[0]),
                                     unbox_type(state[1]),
                                     state[2].cast<int64_t>());
              })));
}

py::class_<ak::UnknownType, std::shared_ptr<ak::UnknownType>, ak::Type>
make_UnknownType(py::handle m, std::string name) {
  return type_methods(
      py::class_<ak::UnknownType, std::shared_ptr<ak::UnknownType>, ak::Type>(
          m, name.c_str())
          .def(py::init([](py::object parameters) -> ak::UnknownType {
               ...
               py::arg("parameters") = py::none())
          .def_property("parameters", &getparameters<ak::UnknownType>,
                        &setparameters<ak::UnknownType>)
          .def(py::pickle(
              [](const ak::UnknownType &self) {
                return py::make_tuple(parameters2dict(self.parameters()));
              },
              [](py::tuple state) {
...
make_PrimitiveType(py::handle m, std::string name) {
  return type_methods(
      py::class_<ak::PrimitiveType, std::shared_ptr<ak::PrimitiveType>,
                 ak::Type>(m, name.c_str())
          .def(py::init([](std::string dtype,
                           py::object parameters) -> ak::PrimitiveType {
          ...
          .def_property_readonly("dtype",
                                 [](ak::PrimitiveType &self) -> std::string {
                                   switch (self.dtype()) {
                                   case ak::PrimitiveType::boolean:
                                     return std::string("bool");
                                   case ak::PrimitiveType::int8:
                                     return std::string("int8");
                                   case ak::PrimitiveType::int16:
                                     return std::string("int16");
                                   case ak::PrimitiveType::int32:
                                     return std::string("int32");
                                   case ak::PrimitiveType::int64:
                                     return std::string("int64");
                                   case ak::PrimitiveType::uint8:
                                     return std::string("uint8");
                                   case ak::PrimitiveType::uint16:
                                     return std::string("uint16");
                                   case ak::PrimitiveType::uint32:
                                     return std::string("uint32");
                                   case ak::PrimitiveType::uint64:
                                     return std::string("uint64");
                                   case ak::PrimitiveType::float32:
                                     return std::string("float32");
                                   case ak::PrimitiveType::float64:
          ...
          .def_property("parameters", &getparameters<ak::PrimitiveType>,
                        &setparameters<ak::PrimitiveType>)
          .def(py::pickle(
              [](const ak::PrimitiveType &self) {
                return py::make_tuple(parameters2dict(self.parameters()),
              ...
              [](py::tuple state) {
                return ak::PrimitiveType(
                    ...
                    (ak::PrimitiveType::DType)state[1].cast<int64_t>());
              })));
}

py::class_<ak::RegularType, std::shared_ptr<ak::RegularType>, ak::Type>
make_RegularType(py::handle m, std::string name) {
  return type_methods(
      py::class_<ak::RegularType, std::shared_ptr<ak::RegularType>, ak::Type>(
          m, name.c_str())
          .def(py::init([](std::shared_ptr<ak::Type> type, int64_t size,
                           py::object parameters) -> ak::RegularType {
                 return ak::RegularType(dict2parameters(parameters), type,
                                        size);
               }),
          ...
          .def_property("parameters", &getparameters<ak::RegularType>,
                        &setparameters<ak::RegularType>)
          .def(py::pickle(
              [](const ak::RegularType &self) {
                return py::make_tuple(parameters2dict(self.parameters()),
                                      box(self.type()), py::cast(self.size()));
              },
              [](py::tuple state) {
...
make_ListType(py::handle m, std::string name) {
  return type_methods(
      py::class_<ak::ListType, std::shared_ptr<ak::ListType>, ak::Type>(
          m, name.c_str())
          .def(py::init([](std::shared_ptr<ak::Type> type,
                           py::object parameters) -> ak::ListType {
          ...
          .def_property("parameters", &getparameters<ak::ListType>,
                        &setparameters<ak::ListType>)
          .def(py::pickle(
              [](const ak::ListType &self) {
                return py::make_tuple(parameters2dict(self.parameters()),
                                      box(self.type()));
              },
              [](py::tuple state) {
...
make_OptionType(py::handle m, std::string name) {
  return type_methods(
      py::class_<ak::OptionType, std::shared_ptr<ak::OptionType>, ak::Type>(
          m, name.c_str())
          .def(py::init([](std::shared_ptr<ak::Type> type,
                           py::object parameters) -> ak::OptionType {
                 return ak::OptionType(dict2parameters(parameters), type);
               }),
               py::arg("type"), py::arg("parameters") = py::none())
          .def_property_readonly("type", &ak::OptionType::type)
          .def_property("parameters", &getparameters<ak::OptionType>,
                        &setparameters<ak::OptionType>)
          .def(py::pickle(
              [](const ak::OptionType &self) {
                return py::make_tuple(parameters2dict(self.parameters()),
                                      box(self.type()));
              },
              [](py::tuple state) {
...
make_UnionType(py::handle m, std::string name) {
  return type_methods(
      py::class_<ak::UnionType, std::shared_ptr<ak::UnionType>, ak::Type>(
          m, name.c_str())
          .def(py::init([](py::iterable types,
                           py::object parameters) -> ak::UnionType {
          ...
          .def_property_readonly("types",
                                 [](ak::UnionType &self) -> py::tuple {
                                   py::tuple types((size_t)self.numtypes());
                                   for (int64_t i = 0; i < self.numtypes();
                                        i++) {
          ...
          .def_property("parameters", &getparameters<ak::UnionType>,
                        &setparameters<ak::UnionType>)
          .def(py::pickle(
              [](const ak::UnionType &self) {
                py::tuple types((size_t)self.numtypes());
...
template <typename T> py::object lookup(const T &self) {
...
template <typename T> py::object reverselookup(const T &self) {
...
void from_lookup(std::shared_ptr<L> lookup, std::shared_ptr<R> reverselookup,
                 py::dict pylookup, py::object pyreverselookup,
                 int64_t numfields) {
  for (auto x : pylookup) {
  ...
  if (pyreverselookup.is(py::none())) {
    ...
    for (auto x : *lookup.get()) {
      if ((int64_t)x.second > numfields) {
        throw std::invalid_argument(
            std::string("lookup[") + ak::util::quote(x.first, true) +
            std::string("] is ") + std::to_string(x.second) +
            std::string(" but there are only ") + std::to_string(numfields) +
            std::string(" fields"));
      }
    ...
    for (auto x : pyreverselookup.cast<py::iterable>()) {
      if (!py::isinstance<py::str>(x)) {
        throw std::invalid_argument(
            "elements of reverselookup must all be strings");
      }
...
make_RecordType(py::handle m, std::string name) {
  return type_methods(
      py::class_<ak::RecordType, std::shared_ptr<ak::RecordType>, ak::Type>(
          m, name.c_str())
          .def(py::init([](py::tuple types,
                           py::object parameters) -> ak::RecordType {
                 std::vector<std::shared_ptr<ak::Type>> out;
                 for (auto x : types) {
                   out.push_back(unbox_type(x));
                 }
                 return ak::RecordType(
                     dict2parameters(parameters), out,
                     std::shared_ptr<ak::RecordType::Lookup>(nullptr),
                     std::shared_ptr<ak::RecordType::ReverseLookup>(nullptr));
               }),
               py::arg("types"), py::arg("parameters") = py::none())
          .def(py::init([](py::dict types,
                           py::object parameters) -> ak::RecordType {
                 std::shared_ptr<ak::RecordType::Lookup> lookup(
                     new ak::RecordType::Lookup);
                 std::shared_ptr<ak::RecordType::ReverseLookup> reverselookup(
                     new ak::RecordType::ReverseLookup);
                 std::vector<std::shared_ptr<ak::Type>> out;
                 for (auto x : types) {
                   std::string key = x.first.cast<std::string>();
                 ...
                 return ak::RecordType(dict2parameters(parameters), out, lookup,
                                       reverselookup);
               }),
               py::arg("types"), py::arg("parameters") = py::none())
          .def_static(
              "from_lookup",
              [](py::iterable types, py::dict pylookup,
                 py::object pyreverselookup,
                 py::object parameters) -> ak::RecordType {
                std::vector<std::shared_ptr<ak::Type>> out;
                for (auto x : types) {
                  out.push_back(unbox_type(x));
                ...
                std::shared_ptr<ak::RecordType::ReverseLookup> reverselookup(
                    new ak::RecordType::ReverseLookup);
                from_lookup<ak::RecordType::Lookup,
                            ak::RecordType::ReverseLookup>(
                    lookup, reverselookup, pylookup, pyreverselookup,
                    (int64_t)out.size());
                return ak::RecordType(dict2parameters(parameters), out, lookup,
                                      reverselookup);
              },
              py::arg("types"), py::arg("lookup"),
              py::arg("reverselookup") = py::none(),
              py::arg("parameters") = py::none())
          .def("__getitem__",
               [](ak::RecordType &self, int64_t fieldindex) -> py::object {
                 return box(self.field(fieldindex));
               })
          .def("__getitem__",
               [](ak::RecordType &self, std::string key) -> py::object {
                 return box(self.field(key));
               })
          .def_property_readonly(
              "types",
              [](ak::RecordType &self) -> py::object {
          ...
          .def_property_readonly("reverselookup",
                                 &reverselookup<ak::RecordType>)
          .def_property("parameters", &getparameters<ak::RecordType>,
                        &setparameters<ak::RecordType>)
          .def(py::pickle(
              [](const ak::RecordType &self) {
                py::tuple fields((size_t)self.numfields());
                for (int64_t i = 0; i < self.numfields(); i++) {
                  fields[(size_t)i] = box(self.field(i));
                }
                return py::make_tuple(parameters2dict(self.parameters()),
                                      fields, lookup<ak::RecordType>(self),
                                      reverselookup<ak::RecordType>(self));
              },
              [](py::tuple state) {
                ...
                from_lookup<ak::RecordType::Lookup,
                            ak::RecordType::ReverseLookup>(
                    lookup, reverselookup, state[2].cast<py::dict>(), state[3],
                    (int64_t)fields.size());
                return ak::RecordType(dict2parameters(state[0]), fields, lookup,
                                      reverselookup);
              })));
}
...
template <typename T> ak::Iterator iter(T &self) {
...
void tojson_file(T &self, std::string destination, bool pretty,
                 py::object maxdecimals, int64_t buffersize) {
#ifdef _MSC_VER
  FILE *file;
...
content_methods(py::class_<T, std::shared_ptr<T>, ak::Content> &x) {
  return x.def("__repr__", &repr<T>)
      .def_property(
          "id", [](T &self) -> py::object { return box(self.id()); },
          [](T &self, py::object id) -> void { self.setid(unbox_id_none(id)); })
      .def(
          "setid",
          [](T &self, py::object id) -> void { self.setid(unbox_id_none(id)); })
      .def("setid", [](T &self) -> void { self.setid(); })
      .def_property_readonly("baretype", &ak::Content::baretype)
      .def_property_readonly("isbare", &ak::Content::isbare)
      .def_property(
          "type", [](T &self) -> py::object { return box(self.type()); },
          [](T &self, py::object type) -> void {
            self.settype(unbox_type(type));
          })
      .def("accepts",
           [](T &self, py::object type) -> bool {
      ...
      .def("tojson", &tojson_file<T>, py::arg("destination"),
           py::arg("pretty") = false, py::arg("maxdecimals") = py::none(),
...
make_Content(py::handle m, std::string name) {
...
make_NumpyArray(py::handle m, std::string name) {
  return content_methods(
      py::class_<ak::NumpyArray, std::shared_ptr<ak::NumpyArray>, ak::Content>(
          m, name.c_str(), py::buffer_protocol())
          .def_buffer([](ak::NumpyArray &self) -> py::buffer_info {
            return py::buffer_info(self.byteptr(), self.itemsize(),
                                   self.format(), self.ndim(), self.shape(),
                                   self.strides());
          })

          .def(py::init([](py::array array, py::object id,
                           py::object type) -> ak::NumpyArray {
                 py::buffer_info info = array.request();
                 if (info.ndim == 0) {
                   throw std::invalid_argument(
                 ...
                 return ak::NumpyArray(
                     unbox_id_none(id), unbox_type_none(type),
...
template <typename T>
py::class_<ak::ListArrayOf<T>, std::shared_ptr<ak::ListArrayOf<T>>, ak::Content>
make_ListArrayOf(py::handle m, std::string name) {
  return content_methods(
      py::class_<ak::ListArrayOf<T>, std::shared_ptr<ak::ListArrayOf<T>>,
                 ak::Content>(m, name.c_str())
          .def(py::init([](ak::IndexOf<T> &starts, ak::IndexOf<T> &stops,
                           py::object content, py::object id,
                           py::object type) -> ak::ListArrayOf<T> {
                 return ak::ListArrayOf<T>(unbox_id_none(id),
                                           unbox_type_none(type), starts, stops,
                                           unbox_content(content));
               }),
               py::arg("starts"), py::arg("stops"), py::arg("content"),
...
template <typename T>
py::class_<ak::ListOffsetArrayOf<T>, std::shared_ptr<ak::ListOffsetArrayOf<T>>,
           ak::Content>
make_ListOffsetArrayOf(py::handle m, std::string name) {
  return content_methods(
      py::class_<ak::ListOffsetArrayOf<T>,
                 std::shared_ptr<ak::ListOffsetArrayOf<T>>, ak::Content>(
          m, name.c_str())
          .def(py::init([](ak::IndexOf<T> &offsets, py::object content,
                           py::object id,
                           py::object type) -> ak::ListOffsetArrayOf<T> {
                 return ak::ListOffsetArrayOf<T>(
                     unbox_id_none(id), unbox_type_none(type), offsets,
                     std::shared_ptr<ak::Content>(unbox_content(content)));
               }),
               py::arg("offsets"), py::arg("content"),
               py::arg("id") = py::none(), py::arg("type") = py::none())

          .def_property_readonly("offsets", &ak::ListOffsetArrayOf<T>::offsets)
          .def_property_readonly("content",
                                 &ak::ListOffsetArrayOf<T>::content));
}

/////////////////////////////////////////////////////////////// EmptyArray

py::class_<ak::EmptyArray, std::shared_ptr<ak::EmptyArray>, ak::Content>
make_EmptyArray(py::handle m, std::string name) {
  return content_methods(
      py::class_<ak::EmptyArray, std::shared_ptr<ak::EmptyArray>, ak::Content>(
          m, name.c_str())
          .def(py::init([](py::object id, py::object type) -> ak::EmptyArray {
                 return ak::EmptyArray(unbox_id_none(id),
                                       unbox_type_none(type));
               }),
               py::arg("id") = py::none(), py::arg("type") = py::none()));
}
...
make_RegularArray(py::handle m, std::string name) {
  return content_methods(
      py::class_<ak::RegularArray, std::shared_ptr<ak::RegularArray>,
                 ak::Content>(m, name.c_str())
          .def(py::init([](py::object content, int64_t size, py::object id,
                           py::object type) -> ak::RegularArray {
...
make_RecordArray(py::handle m, std::string name) {
  return content_methods(
      py::class_<ak::RecordArray, std::shared_ptr<ak::RecordArray>,
                 ak::Content>(m, name.c_str())
          .def(py::init([](py::dict contents, py::object id,
                           py::object type) -> ak::RecordArray {
                 std::shared_ptr<ak::RecordArray::Lookup> lookup(
                     new ak::RecordArray::Lookup);
                 std::shared_ptr<ak::RecordArray::ReverseLookup> reverselookup(
                     new ak::RecordArray::ReverseLookup);
                 ...
                 if (out.size() == 0) {
                   throw std::invalid_argument(
                       "construct RecordArrays without fields using "
                       "RecordArray(length) where length is an integer");
                 }
                 return ak::RecordArray(unbox_id_none(id),
                                        unbox_type_none(type), out, lookup,
                                        reverselookup);
               }),
               py::arg("contents"), py::arg("id") = py::none(),
               py::arg("type") = py::none())
          .def(py::init([](py::iterable contents, py::object id,
                           py::object type) -> ak::RecordArray {
                 std::vector<std::shared_ptr<ak::Content>> out;
                 for (auto x : contents) {
                 ...
                 return ak::RecordArray(
                     unbox_id_none(id), unbox_type_none(type), out,
                     std::shared_ptr<ak::RecordArray::Lookup>(nullptr),
                     std::shared_ptr<ak::RecordArray::ReverseLookup>(nullptr));
               }),
               py::arg("contents"), py::arg("id") = py::none(),
               py::arg("type") = py::none())
          .def(py::init([](int64_t length, bool istuple, py::object id,
                           py::object type) -> ak::RecordArray {
                 return ak::RecordArray(unbox_id_none(id),
                                        unbox_type_none(type), length, istuple);
               }),
               py::arg("length"), py::arg("istuple") = false,
               py::arg("id") = py::none(), py::arg("type") = py::none())
          .def_static(
              "from_lookup",
              [](py::iterable contents, py::dict pylookup,
                 py::object pyreverselookup, py::object id,
                 py::object type) -> ak::RecordArray {
                std::vector<std::shared_ptr<ak::Content>> out;
                for (auto x : contents) {
                  out.push_back(unbox_content(x));
                ...
                std::shared_ptr<ak::RecordArray::ReverseLookup> reverselookup(
                    new ak::RecordArray::ReverseLookup);
                from_lookup<ak::RecordArray::Lookup,
                            ak::RecordArray::ReverseLookup>(
                    lookup, reverselookup, pylookup, pyreverselookup,
                    (int64_t)out.size());
                return ak::RecordArray(unbox_id_none(id), unbox_type_none(type),
                                       out, lookup, reverselookup);
              },
              py::arg("contents"), py::arg("lookup"),
              py::arg("reverselookup") = py::none(), py::arg("id") = py::none(),
              py::arg("type") = py::none())
          ...
          .def("field",
               [](ak::RecordArray &self,
                  int64_t fieldindex) -> std::shared_ptr<ak::Content> {
                 return self.field(fieldindex);
               })
          .def("field",
               [](ak::RecordArray &self, std::string key)
                   -> std::shared_ptr<ak::Content> { return self.field(key); })
          .def("fields",
               [](ak::RecordArray &self) -> py::object {
                 py::list out;
                 for (auto item : self.fields()) {
                   out.append(box(item));
                 }
          ...
          .def(
              "append",
              [](ak::RecordArray &self, py::object content,
                 py::object key) -> void {
                if (key.is(py::none())) {
...
make_Record(py::handle m, std::string name) {
  return py::class_<ak::Record, std::shared_ptr<ak::Record>>(m, name.c_str())
      ...
      .def_property_readonly(
          "id", [](ak::Record &self) -> py::object { return box(self.id()); })
      ...
      .def_property_readonly(
          "type",
          [](ak::Record &self) -> py::object { return box(self.type()); })
      .def("tojson", &tojson_string<ak::Record>, py::arg("pretty") = false,
           py::arg("maxdecimals") = py::none())
      .def("tojson", &tojson_file<ak::Record>, py::arg("destination"),
           py::arg("pretty") = false, py::arg("maxdecimals") = py::none(),
           py::arg("buffersize") = 65536)
      ...
      .def_property_readonly(
          "array",
          [](ak::Record &self) -> py::object { return box(self.array()); })
      ...
      .def(
          "keyaliases",
          [](ak::Record &self, int64_t fieldindex) -> std::vector<std::string> {
            return self.keyaliases(fieldindex);
          })
      .def("keyaliases",
           [](ak::Record &self, std::string key) -> std::vector<std::string> {
             return self.keyaliases(key);
           })
      .def("keys", &ak::Record::keys)
      .def("field",
           [](ak::Record &self, int64_t fieldindex) -> py::object {
             return box(self.field(fieldindex));
           })
      .def("field",
           [](ak::Record &self, std::string key) -> py::object {
             return box(self.field(key));
           })
      .def("fields",
           [](ak::Record &self) -> py::object {
             py::list out;
             for (auto item : self.fields()) {
               out.append(box(item));
             }
...
PYBIND11_MODULE(layout, m) {
  ...
  m.def(
      "fromjson",
      [](std::string source, int64_t initial, double resize,
        ...
        if (isarray) {
          return box(ak::FromJsonString(source.c_str(),
                                        ak::FillableOptions(initial, resize)));
        } else {
#ifdef _MSC_VER
          FILE *file;
          ...
          try {
            out = FromJsonFile(file, ak::FillableOptions(initial, resize),
                               buffersize);
          } catch (...) {
            fclose(file);
            throw;
      ...
      py::arg("source"), py::arg("initial") = 1024, py::arg("resize") = 2.0,
      py::arg("buffersize") = 65536);

  m.def(
      "fromroot_nestedvector",
      [](ak::Index64 &byteoffsets, ak::NumpyArray &rawdata, int64_t depth,
         int64_t itemsize, std::string format, int64_t initial,
         double resize) -> py::object {
        return box(FromROOT_nestedvector(byteoffsets, rawdata, depth, itemsize,
                                         format,
                                         ak::FillableOptions(initial, resize)));
      },
      py::arg("byteoffsets"), py::arg("rawdata"), py::arg("depth"),
      py::arg("itemsize"), py::arg("format"), py::arg("initial") = 1024,
      py::arg("resize") = 2.0);
}
