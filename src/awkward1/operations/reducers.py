...
def count(array, axis=None, keepdims=False, semigroup=False):
    ...
    if axis is None:
        ...
        return awkward1._util.wrap(layout.count(axis=axis, mask=False, keepdims=keepdims, semigroup=semigroup), behavior)

@awkward1._connect._numpy.implements(numpy.count_nonzero)
def count_nonzero(array, axis=None, keepdims=False, semigroup=False):
    layout = awkward1.operations.convert.tolayout(array, allowrecord=False, allowother=False)
    if axis is None:
        def reduce(xs):
            if len(xs) == 1:
        ...
        behavior = awkward1._util.behaviorof(array)
        return awkward1._util.wrap(layout.count_nonzero(axis=axis, mask=False, keepdims=keepdims, semigroup=semigroup), behavior)

@awkward1._connect._numpy.implements(numpy.sum)
def sum(array, axis=None, keepdims=False, semigroup=False):
    layout = awkward1.operations.convert.tolayout(array, allowrecord=False, allowother=False)
    if axis is None:
        def reduce(xs):
            if len(xs) == 1:
        ...
        behavior = awkward1._util.behaviorof(array)
        return awkward1._util.wrap(layout.sum(axis=axis, mask=False, keepdims=keepdims, semigroup=semigroup), behavior)

@awkward1._connect._numpy.implements(numpy.prod)
def prod(array, axis=None, keepdims=False, semigroup=False):
    layout = awkward1.operations.convert.tolayout(array, allowrecord=False, allowother=False)
    if axis is None:
        def reduce(xs):
            if len(xs) == 1:
        ...
        behavior = awkward1._util.behaviorof(array)
        return awkward1._util.wrap(layout.prod(axis=axis, mask=False, keepdims=keepdims, semigroup=semigroup), behavior)

@awkward1._connect._numpy.implements(numpy.any)
def any(array, axis=None, keepdims=False, semigroup=False):
    layout = awkward1.operations.convert.tolayout(array, allowrecord=False, allowother=False)
    if axis is None:
        def reduce(xs):
            if len(xs) == 1:
        ...
        behavior = awkward1._util.behaviorof(array)
        return awkward1._util.wrap(layout.any(axis=axis, mask=False, keepdims=keepdims, semigroup=semigroup), behavior)

@awkward1._connect._numpy.implements(numpy.all)
def all(array, axis=None, keepdims=False, semigroup=False):
    layout = awkward1.operations.convert.tolayout(array, allowrecord=False, allowother=False)
    if axis is None:
        def reduce(xs):
            if len(xs) == 1:
        ...
        behavior = awkward1._util.behaviorof(array)
        return awkward1._util.wrap(layout.all(axis=axis, mask=False, keepdims=keepdims, semigroup=semigroup), behavior)

@awkward1._connect._numpy.implements(numpy.min)
def min(array, axis=None, keepdims=False, semigroup=True):
    layout = awkward1.operations.convert.tolayout(array, allowrecord=False, allowother=False)
    if axis is None:
        def reduce(xs):
            if len(xs) == 0:
        ...
        behavior = awkward1._util.behaviorof(array)
        return awkward1._util.wrap(layout.min(axis=axis, mask=True, keepdims=keepdims, semigroup=semigroup), behavior)

@awkward1._connect._numpy.implements(numpy.max)
def max(array, axis=None, keepdims=False, semigroup=True):
    layout = awkward1.operations.convert.tolayout(array, allowrecord=False, allowother=False)
    if axis is None:
        def reduce(xs):
            if len(xs) == 0:
        ...
        behavior = awkward1._util.behaviorof(array)
        return awkward1._util.wrap(layout.max(axis=axis, mask=True, keepdims=keepdims, semigroup=semigroup), behavior)
