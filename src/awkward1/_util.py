...
def recursively_apply(layout, getfunction, depth=1):
    ...
    if custom is not None:
        ...
        return awkward1.layout.RegularArray(recursively_apply(layout.content, getfunction, depth + 1), layout.size, layout.identities, layout.parameters)

    elif isinstance(layout, awkward1.layout.ListArray32):
        return awkward1.layout.ListArray32(layout.starts, layout.stops, recursively_apply(layout.content, getfunction, depth + 1), layout.identities, layout.parameters)

    elif isinstance(layout, awkward1.layout.ListArrayU32):
        return awkward1.layout.ListArrayU32(layout.starts, layout.stops, recursively_apply(layout.content, getfunction, depth + 1), layout.identities, layout.parameters)

    elif isinstance(layout, awkward1.layout.ListArray64):
        return awkward1.layout.ListArray64(layout.starts, layout.stops, recursively_apply(layout.content, getfunction, depth + 1), layout.identities, layout.parameters)

    elif isinstance(layout, awkward1.layout.ListOffsetArray32):
        return awkward1.layout.ListOffsetArray32(layout.offsets, recursively_apply(layout.content, getfunction, depth + 1), layout.identities, layout.parameters)

    elif isinstance(layout, awkward1.layout.ListOffsetArrayU32):
        return awkward1.layout.ListOffsetArrayU32(layout.offsets, recursively_apply(layout.content, getfunction, depth + 1), layout.identities, layout.parameters)

    elif isinstance(layout, awkward1.layout.ListOffsetArray64):
        return awkward1.layout.ListOffsetArray64(layout.offsets, recursively_apply(layout.content, getfunction, depth + 1), layout.identities, layout.parameters)

    elif isinstance(layout, awkward1.layout.IndexedArray32):
        return awkward1.layout.IndexedArray32(layout.index, recursively_apply(layout.content, getfunction, depth), layout.identities, layout.parameters)

    elif isinstance(layout, awkward1.layout.IndexedArrayU32):
        return awkward1.layout.IndexedArrayU32(layout.index, recursively_apply(layout.content, getfunction, depth), layout.identities, layout.parameters)

    elif isinstance(layout, awkward1.layout.IndexedArray64):
        return awkward1.layout.IndexedArray64(layout.index, recursively_apply(layout.content, getfunction, depth), layout.identities, layout.parameters)

    elif isinstance(layout, awkward1.layout.IndexedOptionArray32):
        return awkward1.layout.IndexedOptionArray32(layout.index, recursively_apply(layout.content, getfunction, depth), layout.identities, layout.parameters)

    elif isinstance(layout, awkward1.layout.IndexedOptionArray64):
        return awkward1.layout.IndexedOptionArray64(layout.index, recursively_apply(layout.content, getfunction, depth), layout.identities, layout.parameters)

    elif isinstance(layout, awkward1.layout.RecordArray):
        return awkward1.layout.RecordArray([recursively_apply(x, getfunction, depth) for x in layout.contents], layout.recordlookup, len(layout), layout.identities, layout.parameters)

    elif isinstance(layout, awkward1.layout.Record):
        return awkward1.layout.Record(recursively_apply(layout.array, getfunction, depth), layout.at)

    elif isinstance(layout, awkward1.layout.UnionArray8_32):
        return awkward1.layout.UnionArray8_32(layout.tags, layout.index, [recursively_apply(x, getfunction, depth) for x in layout.contents], layout.identities, layout.parameters)

    elif isinstance(layout, awkward1.layout.UnionArray8_U32):
        return awkward1.layout.UnionArray8_U32(layout.tags, layout.index, [recursively_apply(x, getfunction, depth) for x in layout.contents], layout.identities, layout.parameters)

    elif isinstance(layout, awkward1.layout.UnionArray8_64):
        return awkward1.layout.UnionArray8_64(layout.tags, layout.index, [recursively_apply(x, getfunction, depth) for x in layout.contents], layout.identities, layout.parameters)
