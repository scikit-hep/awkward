    ...
    def __init__(self, behavior):
        super().__init__(
            name="ak.ArrayBuilderType({})".format(
                ak._connect.numba.arrayview_cuda.repr_behavior(behavior)
            )
        )
...
class type_methods(numba.core.typing.templates.AttributeTemplate):
    ...
    def resolve_append(self, arraybuildertype, args, kwargs):
        if (
            ...
            and isinstance(
                args[0],
                (
                    ak._connect.numba.arrayview_cuda.ArrayViewType,
                    ak._connect.numba.arrayview_cuda.RecordViewType,
                    numba.types.Boolean,
                    numba.types.Integer,
                    numba.types.Float,
                    numba.types.Complex,
    ...
    def resolve_extend(self, arraybuildertype, args, kwargs):
        if (
            len(args) == 1
            and len(kwargs) == 0
            and isinstance(args[0], ak._connect.numba.arrayview_cuda.ArrayViewType)
        ):
            return numba.types.none(args[0])
        else:
            raise TypeError(
...
@numba.extending.lower_builtin(
    "append", ArrayBuilderType, ak._connect.numba.arrayview_cuda.ArrayViewType
)
...
@numba.extending.lower_builtin(
    "append", ArrayBuilderType, ak._connect.numba.arrayview_cuda.RecordViewType
)
...
@numba.extending.lower_builtin(
    "extend", ArrayBuilderType, ak._connect.numba.arrayview_cuda.ArrayViewType
)
