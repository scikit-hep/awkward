...
def offsets_to_segment_ids(offsets):
    ...
    # Binary search: element i belongs to the segment j with
    # offsets[j] <= i < offsets[j+1], i.e. searchsorted with side='right'
    # on the full offsets array, minus 1.  Empty segments (repeated offsets)
    # are skipped correctly because side='right' lands past all of them.
    #
    # Cast to int32 to match the empty-case return and the other index
    # buffers in this module (which assume < 2**31 elements/segments).
    segment_ids = cp.searchsorted(offsets, element_indices, side="right") - 1

    return segment_ids.astype(np.int32, copy=False)
...
def select_segments(
    ...
    segment_indices = offsets_to_segment_ids(offsets_in)
    ...
    # Match the output-offsets dtype so the scan does not reinterpret values
    offsets_dtype = getattr(offsets_out, "dtype", np.int64)
    ...
    segment_counts = (positions[1:] - positions[:-1]).astype(offsets_dtype)
...
def segmented_select(
    ...
    # Match the output-offsets dtype so the scan does not reinterpret values
    offsets_dtype = getattr(d_out_segments, "dtype", np.int64)
    ...
    d_counts = (positions[1:] - positions[:-1]).astype(offsets_dtype)
...
def transform_segments(data_in, data_out, segment_size, op, num_segments):
    ...
    def get_column(it, i):
        ...
        return PermutationIterator(
            it, TransformIterator(CountingIterator(np.int32(0)), col_major_index)
        )
