...
//     cuda_kernel_templates.get_function(fetch_specialization(["awkward_reduce_prod_bool_complex_a", cupy.dtype(toptr.dtype).type, cupy.dtype(fromptr.dtype).type, parents.dtype]))((grid_size,), block, (toptr, fromptr, parents, lenparents, outlength, temp, invocation_index, err_code))
//     cuda_kernel_templates.get_function(fetch_specialization(["awkward_reduce_prod_bool_complex_b", cupy.dtype(toptr.dtype).type, cupy.dtype(fromptr.dtype).type, parents.dtype]))((grid_size,), block, (toptr, fromptr, parents, lenparents, outlength, temp, invocation_index, err_code))
...
__global__ void
awkward_reduce_prod_bool_complex_a(
    T* toptr,
    const C* fromptr,
    const U* parents,
    int64_t lenparents,
...
template <typename T, typename C, typename U>
__global__ void
awkward_reduce_prod_bool_complex_b(
    T* toptr,
    const C* fromptr,
    const U* parents,
    int64_t lenparents,
...
template <typename T, typename C, typename U>
__global__ void
awkward_reduce_prod_bool_complex_c(
    T* toptr,
    const C* fromptr,
    const U* parents,
    int64_t lenparents,
