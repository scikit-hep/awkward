...
enum class LISTARRAY_COMBINATIONS_ERRORS : uint64_t {
    N_NEGATIVE = 1,               // message: "n must be >= 0"
    OVERFLOW_IN_COMBINATORICS = 2 // message: "binomial overflow"
};

template <typename I>
__device__ __forceinline__ I ak_dgcd(I a, I b) {
...
template <typename I>
__device__ __forceinline__ bool ak_mul_will_overflow(I a, I b) {
...
__device__ __forceinline__ bool binom_safe(
  ...
  for (I j = 1; j <= k; ++j) {
    ...
    I g2 = ak_dgcd(result, b);
    result /= g2; b /= g2;

    // b should now be 1; multiply result *= a with overflow check
    if (ak_mul_will_overflow<I>(result, a)) {
        // signal overflow
        err_code[0] = (err_code[0] < static_cast<uint64_t>(
            LISTARRAY_COMBINATIONS_ERRORS::OVERFLOW_IN_COMBINATORICS))
