...
awkward_RegularArray_combinations_64_a(
  ...
  if (err_code[0] == NO_ERROR) {
    ...
    if (thread_id < length) {
      if (n != 2) {
        RAISE_ERROR(RegularArray_COMBINATIONS_ERRORS::NOT_IMPLEMENTED)
      }
      int64_t counts = size;
      if (replacement) {
