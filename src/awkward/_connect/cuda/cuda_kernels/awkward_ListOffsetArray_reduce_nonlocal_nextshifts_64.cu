...
template <typename T_nummissing, typename T_missing, typename T_nextshifts, typename T_offsets, typename T_starts, typename T_parents, typename T_nextcarry>
...
__global__ void awkward_ListOffsetArray_reduce_nonlocal_nextshifts_64_b(
  ...
  if (thread_id < length) {
    T_parents parent_idx = parents[thread_id];
    T_starts group_start = starts[parent_idx];
        
    // Reset at group boundaries
    if (thread_id == group_start) {
      // First item in group, no predecessors
      for (int64_t j = 0; j < maxcount; j++) {
        scan_in_array[thread_id * maxcount + j] = 0;
      }
    } else {
      // Accumulate from previous item in the same group
      for (int64_t j = 0; j < maxcount; j++) {
        int64_t prev_val = scan_in_array[(thread_id - 1) * maxcount + j];
        int64_t curr_marker = scan_in_array[thread_id * maxcount + j];
        scan_in_array[thread_id * maxcount + j] = prev_val + curr_marker;
      }
    }
  }
}
...
__global__ void awkward_ListOffsetArray_reduce_nonlocal_nextshifts_64_c(
  ...
  if (thread_id < length) {
    ...
    // For each j < count, get the accumulated "missing" value
    for (int64_t j = 0; j < count; j++) {
      // The value at scan_in_array[thread_id * maxcount + j] tells us
      // how many items before thread_id (in the same group) had count <= j
      int64_t num_missing = (thread_id > 0) ? scan_in_array[(thread_id - 1) * maxcount + j] : 0;
            
      // Handle group boundaries
      int64_t parent_idx = (int64_t)parents[thread_id];
      int64_t group_start = (int64_t)starts[parent_idx];
      if (thread_id == group_start) {
        num_missing = 0;
      }
