...
//     scan_in_array_outoffsets = cupy.empty(outindexlength, dtype=cupy.int64)
  ...
  if (err_code[0] == NO_ERROR) {
    ...
    if (thread_id < outindexlength) {
      ...
      if (idx < 0) {
        ...
        scan_in_array_outoffsets[thread_id] = 0;
        ...
        scan_in_array_k[thread_id] = 1;
        scan_in_array_outoffsets[thread_id] = count;
      }
    }
  }
}
...
awkward_IndexedArray_flatten_none2empty_b(
  ...
  if (err_code[0] == NO_ERROR) {
    ...
    if (thread_id < outindexlength) {
      C idx = outindex[thread_id];
      if (idx < 0) {
        outoffsets[scan_in_array_k[thread_id]] = scan_in_array_outoffsets[thread_id];
      } else if (idx + 1 >= offsetslength) {
        RAISE_ERROR(INDEXEDARRAY_FLATTEN_NONE2EMPTY_ERRORS::OFF_OUT_OF_RANGE)
      } else {
        outoffsets[scan_in_array_k[thread_id]] = scan_in_array_outoffsets[thread_id];
