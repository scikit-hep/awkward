...
class Torch(ArrayModuleNumpyLike):
    ...
    def ma(self):
        raise ValueError(
            "Torch arrays cannot have missing values until Torch implements "
    ...
    def char(self):
        raise ValueError(
            "Torch arrays cannot do string manipulations until Torch implements numpy.char"
        )
    ...
    def is_own_array_type(cls, type_: type) -> bool:
        ...
        """
        return cls.is_array_type(type_)
    ...
    def is_tracer_type(cls, type_: type) -> bool:
        ...
        Return `True` if the given object is a torch tracer, otherwise `False`.
    ...
    ############################ ufuncs, need to overwrite those because Torch doesn't support `out=` for ufuncs
