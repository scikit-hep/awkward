...
public:
    RAwkwardArrayDS(ssize_t length, ssize_t* ptrs, std::pair<std::string, ROOT::RVec<ColumnTypes>*>... colsNameVals)
      : fLength(length),
        fPtrs(ptrs),
...
template <typename... ColumnTypes>
ROOT::RDataFrame* MakeAwkwardDataFrame(ssize_t length, ssize_t* ptrs, std::pair<std::string, ROOT::RVec<ColumnTypes>*> &&... colNameProxyPairs) {{
    return new ROOT::RDataFrame(std::make_unique<RAwkwardArrayDS<ColumnTypes...>>(
      length, std::forward<ssize_t*>(ptrs),
...
""".strip()
        ...
        print(generator.dataset(), generator.entry())
        return ROOT.MakeAwkwarDataFrame(len(layout), lookup.arrayptrs, {'x': array})
