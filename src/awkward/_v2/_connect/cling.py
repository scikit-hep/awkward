...
namespace awkward {
  ...
  class Iterator {
  public:
    Iterator(ssize_t start, ssize_t stop, ssize_t which, ssize_t* ptrs, PyObject* lookup = nullptr, PyObject* generator = nullptr)
      : start_(start), stop_(stop), which_(which), ptrs_(ptrs), lookup_(lookup), generator_(generator) { }

    VALUE operator*() const noexcept {
      return ARRAY(start_, stop_, which_, ptrs_, lookup_, generator_)[0];
    }
  ...
  class RIterator {
  public:
    RIterator(ssize_t start, ssize_t stop, ssize_t which, ssize_t* ptrs, PyObject* lookup = nullptr, PyObject* generator = nullptr)
      : start_(start), stop_(stop), which_(which), ptrs_(ptrs), lookup_(lookup), generator_(generator) { }
  ...
  class ArrayView {
  public:
    ArrayView(ssize_t start, ssize_t stop, ssize_t which, ssize_t* ptrs, PyObject* lookup = nullptr, PyObject* generator = nullptr)
      : start_(start), stop_(stop), which_(which), ptrs_(ptrs), lookup_(lookup), generator_(generator) {
      }
...
class Generator:
    ...
    Iterator<{self.class_type()}, value_type> begin() const noexcept {{
      return Iterator<{self.class_type()}, value_type>(start_, stop_, which_, ptrs_, lookup_, generator_);
    }}

    Iterator<{self.class_type()}, value_type> end() const noexcept {{
      return Iterator<{self.class_type()}, value_type>(stop_, stop_, which_, ptrs_, lookup_, generator_);
    }}

    RIterator<{self.class_type()}, value_type> rbegin() const noexcept {{
      return RIterator<{self.class_type()}, value_type>(stop_ - 1, stop_, which_, ptrs_, lookup_, generator_);
    }}

    RIterator<{self.class_type()}, value_type> rend() const noexcept {{
      return RIterator<{self.class_type()}, value_type>(start_ - 1, stop_, which_, ptrs_, lookup_, generator_);
    }}
        """.strip()
...
namespace awkward {{
  ...
  public:
    {self.class_type()}(ssize_t start, ssize_t stop, ssize_t which, ssize_t* ptrs, PyObject* lookup = nullptr, PyObject* generator = nullptr)
      : ArrayView(start, stop, which, ptrs, lookup, generator) {{ }}
    {self.class_type()}() : ArrayView(0, 0, 0, 0, 0, 0) {{ }}
...
namespace awkward {{
  ...
  public:
    ...
    value_type operator[](size_t at) const noexcept {{
      ssize_t start = (start_ + at) * {self.size};
      ssize_t stop = start + {self.size};
      return value_type(start, stop, ptrs_[which_ + {self.CONTENT}], ptrs_, lookup_, generator_);
    }}
  }};
}}
""".strip()
...
namespace awkward {{
  ...
  public:
    ...
    value_type operator[](size_t at) const noexcept {{
      ...
      return {self.content.class_type()}(index, index + 1, ptrs_[which_ + {self.CONTENT}], ptrs_, lookup_, generator_)[0];
    }}
  }};
}}
""".strip()
...
namespace awkward {{
  ...
  public:
    ...
    value_type operator[](size_t at) const noexcept {{
      ssize_t index = reinterpret_cast<{self.index_type}*>(ptrs_[which_ + {self.INDEX}])[start_ + at];
      if (index >= 0) {{
        return value_type{{ {self.content.class_type()}(index, index + 1, ptrs_[which_ + {self.CONTENT}], ptrs_, lookup_, generator_)[0] }};
      }}
      else {{
        return std::nullopt;
      }}
  ...
  public:
    ...
    value_type operator[](size_t at) const noexcept {{
      ...
      if ({"mask != 0" if self.valid_when else "mask == 0"}) {{
        return value_type{{ {self.content.class_type()}(start_, stop_, ptrs_[which_ + {self.CONTENT}], ptrs_, lookup_, generator_)[at] }};
      }}
      else {{
        return std::nullopt;
      }}
...
class RecordGenerator(Generator, ak._v2._lookup.RecordLookup):
    ...
    {content.value_type()} {fieldname}() const noexcept {{
      return {content.class_type()}(at_, at_ + 1, ptrs_[which_ + {self.CONTENTS + i}], ptrs_, lookup_, generator_)[0];
    }}
""".strip()
                    )
...
namespace awkward {{
  ...
  public:
    {self.class_type()}(ssize_t at, ssize_t which, ssize_t* ptrs, PyObject* lookup, PyObject* generator)
      : RecordView(at, which, ptrs, lookup, generator) {{ }}
    {self.class_type()}() : RecordView(0, 0, 0, 0, 0) {{ }}
...
namespace awkward {{
  ...
  public:
    ...
    value_type operator[](size_t at) const noexcept {{
      return value_type(start_ + at, which_, ptrs_, lookup_, generator_);
    }}
  }};
}}
""".strip()
        ...
        if not use_cached or key not in cache:
            ...
            for i, content in enumerate(self.contents):
                cases.append(
                    f"""
        case {i}:
          return value_type{{ {content.class_type()}(index, index + 1, ptrs_[which_ + {self.CONTENTS + i}], ptrs_, lookup_, generator_)[0] }};
""".strip()
                )
  ...
  public:
    ...
    value_type operator[](size_t at) const noexcept {{
      ...
      switch (tag) {{
        {eoln.join(cases)}
        default:
          return value_type{{ {self.contents[0].class_type()}(index, index + 1, ptrs_[which_ + {self.CONTENTS + 0}], ptrs_, lookup_, generator_)[0] }};
      }}
    }}
  }};
}}
