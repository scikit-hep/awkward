...
def _impl(array, copy, nan, posinf, neginf, highlevel, behavior):
    ...
    nan_layout = ak._v2.operations.convert.to_layout(nan, allow_other=True)
    if isinstance(nan_layout, ak._v2.contents.Content):
        broadcasting_ids[id(nan)] = len(broadcasting)
        broadcasting.append(nan_layout)

    posinf_layout = ak._v2.operations.convert.to_layout(posinf, allow_other=True)
    ...
    neginf_layout = ak._v2.operations.convert.to_layout(neginf, allow_other=True)
