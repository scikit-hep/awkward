...
at inner {2} of length {3}, using sub-slice {4}.{5}""".format(
    ...
    def _combinations_axis0(self, n, replacement, recordlookup, parameters):
        ...
        nplike_tocarryraw = nplike_tocarryraw.ctypes.data_as(
            ctypes.POINTER(ctypes.c_int64)
        )  # GPU?
        tocarryraw = ak.nplike.NumpyKernel._cast(
            nplike_tocarryraw, ctypes.POINTER(ctypes.POINTER(ctypes.c_int64))
        )  # GPU?
