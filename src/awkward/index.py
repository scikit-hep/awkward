...
from awkward._nplikes import metadata as meta
...
def _form_to_zero_length(form):
    if form == "i8":
        return Index8(numpy.zeros(0, dtype=meta.int8))
    elif form == "u8":
        return IndexU8(numpy.zeros(0, dtype=meta.uint8))
    elif form == "i32":
        return Index32(numpy.zeros(0, dtype=meta.int32))
    elif form == "u32":
        return IndexU32(numpy.zeros(0, dtype=meta.uint32))
    elif form == "i64":
        return Index64(numpy.zeros(0, dtype=meta.int64))
    else:
...
class Index:
    ...
    def __init__(self, data, *, metadata=None, nplike=None):
        ...
        if self._expected_dtype is None:
            if self._data.dtype == meta.int8:
                self.__class__ = Index8
            elif self._data.dtype == meta.uint8:
                self.__class__ = IndexU8
            elif self._data.dtype == meta.int32:
                self.__class__ = Index32
            elif self._data.dtype == meta.uint32:
                self.__class__ = IndexU32
            elif self._data.dtype == meta.int64:
    ...
    def to64(self):
        return Index(self._nplike.astype(self._data, meta.int64))
...
class Index8(Index):
    _expected_dtype = meta.int8


class IndexU8(Index):
    _expected_dtype = meta.uint8


class Index32(Index):
    _expected_dtype = meta.int32


class IndexU32(Index):
    _expected_dtype = meta.uint32


class Index64(Index):
    _expected_dtype = meta.int64
