...
namespace awkward {
  ...
  template <typename T, typename I>
  const std::string
  BitMaskedArrayBuilder<T, I>::to_buffers(BuffersContainer& container, int64_t& form_key_id, const ForthOutputBufferMap& outputs) const {
    throw std::runtime_error(
      std::string("'BitMaskedArrayBuilder<T, I>::to_buffers' is not implemented yet")
      + FILENAME(__LINE__));
  }
