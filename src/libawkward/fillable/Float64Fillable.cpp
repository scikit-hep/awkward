...
Float64Fillable::fromempty(const FillableOptions &options) {
  std::shared_ptr<Fillable> out(
      new Float64Fillable(options, GrowableBuffer<double>::empty(options)));
  out.get()->setthat(out);
  return out;
}

const std::shared_ptr<Fillable>
Float64Fillable::fromint64(const FillableOptions &options,
...
int64_t Float64Fillable::length() const { return buffer_.length(); }

void Float64Fillable::clear() { buffer_.clear(); }
...
const std::shared_ptr<Content> Float64Fillable::snapshot() const {
  ...
  return std::shared_ptr<Content>(new NumpyArray(Identity::none(), Type::none(),
                                                 buffer_.ptr(), shape, strides,
                                                 0, sizeof(double), "d"));
}

bool Float64Fillable::active() const { return false; }
...
const std::shared_ptr<Fillable>
Float64Fillable::string(const char *x, int64_t length, const char *encoding) {
  std::shared_ptr<Fillable> out = UnionFillable::fromsingle(options_, that_);
...
const std::shared_ptr<Fillable> Float64Fillable::beginrecord(const char *name,
                                                             bool check) {
  std::shared_ptr<Fillable> out = UnionFillable::fromsingle(options_, that_);
...
const std::shared_ptr<Fillable> Float64Fillable::field(const char *key,
                                                       bool check) {
  throw std::invalid_argument(
