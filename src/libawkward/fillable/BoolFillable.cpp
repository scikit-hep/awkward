...
BoolFillable::fromempty(const FillableOptions &options) {
  std::shared_ptr<Fillable> out(
      new BoolFillable(options, GrowableBuffer<uint8_t>::empty(options)));
  out.get()->setthat(out);
  return out;
}

int64_t BoolFillable::length() const { return buffer_.length(); }

void BoolFillable::clear() { buffer_.clear(); }
...
bool BoolFillable::active() const { return false; }
...
const std::shared_ptr<Fillable>
BoolFillable::string(const char *x, int64_t length, const char *encoding) {
  std::shared_ptr<Fillable> out = UnionFillable::fromsingle(options_, that_);
...
const std::shared_ptr<Fillable> BoolFillable::beginrecord(const char *name,
                                                          bool check) {
  std::shared_ptr<Fillable> out = UnionFillable::fromsingle(options_, that_);
...
const std::shared_ptr<Fillable> BoolFillable::field(const char *key,
                                                    bool check) {
  throw std::invalid_argument(
