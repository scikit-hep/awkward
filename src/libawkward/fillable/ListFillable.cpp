...
ListFillable::fromempty(const FillableOptions &options) {
  GrowableBuffer<int64_t> offsets = GrowableBuffer<int64_t>::empty(options);
  offsets.append(0);
  std::shared_ptr<Fillable> out(new ListFillable(
      options, offsets, UnknownFillable::fromempty(options), false));
  out.get()->setthat(out);
  return out;
}

int64_t ListFillable::length() const { return offsets_.length() - 1; }
...
const std::shared_ptr<Type> ListFillable::type() const {
  return std::shared_ptr<Type>(
      new ListType(Type::Parameters(), content_.get()->type()));
}

const std::shared_ptr<Content> ListFillable::snapshot() const {
  Index64 offsets(offsets_.ptr(), 0, offsets_.length());
  return std::shared_ptr<Content>(new ListOffsetArray64(
      Identity::none(), Type::none(), offsets, content_.get()->snapshot()));
}

bool ListFillable::active() const { return begun_; }
...
const std::shared_ptr<Fillable>
ListFillable::string(const char *x, int64_t length, const char *encoding) {
  if (!begun_) {
    std::shared_ptr<Fillable> out = UnionFillable::fromsingle(options_, that_);
...
const std::shared_ptr<Fillable> ListFillable::beginrecord(const char *name,
                                                          bool check) {
  if (!begun_) {
    std::shared_ptr<Fillable> out = UnionFillable::fromsingle(options_, that_);
...
const std::shared_ptr<Fillable> ListFillable::field(const char *key,
                                                    bool check) {
  if (!begun_) {
    throw std::invalid_argument(
