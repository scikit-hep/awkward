...
StringFillable::fromempty(const FillableOptions &options,
                          const char *encoding) {
  ...
  std::shared_ptr<Fillable> out(
      new StringFillable(options, offsets, content, encoding));
  out.get()->setthat(out);
  return out;
}

int64_t StringFillable::length() const { return offsets_.length() - 1; }
...
const std::shared_ptr<Type> StringFillable::type() const {
  ...
  if (encoding_ == nullptr) {
    ...
    std::string slashquoted = std::string("\\\"") +
                              quoted.substr(1, quoted.length() - 2) +
                              std::string("\\\"");
    if (std::string(encoding_) == std::string("utf-8")) {
      ...
      char_parameters["__str__"] =
          std::string("\"char[") + slashquoted + std::string("]\"");
      string_parameters["__str__"] =
          std::string("\"string[") + slashquoted + std::string("]\"");
    }
    char_parameters["encoding"] = std::string(quoted);
  }

  return std::shared_ptr<Type>(new ListType(
      string_parameters, std::shared_ptr<Type>(new PrimitiveType(
                             char_parameters, PrimitiveType::uint8))));
}
...
const std::shared_ptr<Content> StringFillable::snapshot() const {
  ...
  std::shared_ptr<Content> content(
      new NumpyArray(Identity::none(), stringtype.get()->inner(),
                     content_.ptr(), shape, strides, 0, sizeof(uint8_t), "B"));

  return std::shared_ptr<Content>(
      new ListOffsetArray64(Identity::none(), stringtype, offsets, content));
}

bool StringFillable::active() const { return false; }
...
const std::shared_ptr<Fillable>
StringFillable::string(const char *x, int64_t length, const char *encoding) {
  if (length < 0) {
...
const std::shared_ptr<Fillable> StringFillable::beginrecord(const char *name,
                                                            bool check) {
  std::shared_ptr<Fillable> out = UnionFillable::fromsingle(options_, that_);
...
const std::shared_ptr<Fillable> StringFillable::field(const char *key,
                                                      bool check) {
  throw std::invalid_argument(
