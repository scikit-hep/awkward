...
UnknownFillable::fromempty(const FillableOptions &options) {
  std::shared_ptr<Fillable> out(new UnknownFillable(options, 0));
  out.get()->setthat(out);
  return out;
}

int64_t UnknownFillable::length() const { return nullcount_; }

void UnknownFillable::clear() { nullcount_ = 0; }
...
bool UnknownFillable::active() const { return false; }
...
const std::shared_ptr<Fillable>
UnknownFillable::string(const char *x, int64_t length, const char *encoding) {
...
const std::shared_ptr<Fillable> UnknownFillable::beginrecord(const char *name,
                                                             bool check) {
...
const std::shared_ptr<Fillable> UnknownFillable::field(const char *key,
                                                       bool check) {
  throw std::invalid_argument(
