...
UnionFillable::fromsingle(const FillableOptions &options,
                          const std::shared_ptr<Fillable> firstcontent) {
  ...
  std::shared_ptr<Fillable> out(
      new UnionFillable(options, types, offsets, contents));
  out.get()->setthat(out);
  return out;
}

int64_t UnionFillable::length() const { return types_.length(); }
...
bool UnionFillable::active() const { return current_ != -1; }
...
const std::shared_ptr<Fillable> UnionFillable::real(double x) {
  if (current_ == -1) {
    ...
    if (tofill.get() == nullptr) {
      ...
      if (tofill.get() != nullptr) {
        tofill = Float64Fillable::fromint64(
            options_, dynamic_cast<Int64Fillable *>(tofill.get())->buffer());
...
const std::shared_ptr<Fillable>
UnionFillable::string(const char *x, int64_t length, const char *encoding) {
  if (current_ == -1) {
    std::shared_ptr<Fillable> tofill(nullptr);
    int8_t i = 0;
    for (auto content : contents_) {
...
const std::shared_ptr<Fillable> UnionFillable::beginrecord(const char *name,
                                                           bool check) {
  if (current_ == -1) {
    std::shared_ptr<Fillable> tofill(nullptr);
    int8_t i = 0;
    for (auto content : contents_) {
      if (RecordFillable *raw = dynamic_cast<RecordFillable *>(content.get())) {
        if (raw->length() == -1 || ((check && raw->name() == name) ||
...
const std::shared_ptr<Fillable> UnionFillable::field(const char *key,
                                                     bool check) {
  if (current_ == -1) {
    throw std::invalid_argument(
