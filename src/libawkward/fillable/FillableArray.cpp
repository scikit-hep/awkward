...
const std::string FillableArray::tostring() const {
  ...
  out << "<FillableArray length=\"" << length() << "\" type=\""
      << type().get()->tostring() << "\"/>";
  return out.str();
}

int64_t FillableArray::length() const { return fillable_.get()->length(); }

void FillableArray::clear() { fillable_.get()->clear(); }

const std::shared_ptr<Type> FillableArray::type() const {
  return std::shared_ptr<Type>(new ArrayType(
      Type::Parameters(), fillable_.get()->type(), fillable_.get()->length()));
}
...
FillableArray::getitem_range(int64_t start, int64_t stop) const {
  return snapshot().get()->getitem_range(start, stop);
}

const std::shared_ptr<Content>
FillableArray::getitem_field(const std::string &key) const {
  return snapshot().get()->getitem_field(key);
}

const std::shared_ptr<Content>
FillableArray::getitem_fields(const std::vector<std::string> &keys) const {
  return snapshot().get()->getitem_fields(keys);
}

const std::shared_ptr<Content>
FillableArray::getitem(const Slice &where) const {
  return snapshot().get()->getitem(where);
}

void FillableArray::null() { maybeupdate(fillable_.get()->null()); }
...
void FillableArray::real(double x) { maybeupdate(fillable_.get()->real(x)); }
...
void FillableArray::beginlist() { maybeupdate(fillable_.get()->beginlist()); }
...
void FillableArray::endtuple() { maybeupdate(fillable_.get()->endtuple()); }

void FillableArray::beginrecord() { beginrecord_fast(nullptr); }
...
void FillableArray::endrecord() { maybeupdate(fillable_.get()->endrecord()); }
...
uint8_t awkward_FillableArray_length(void *fillablearray, int64_t *result) {
  awkward::FillableArray *obj =
      reinterpret_cast<awkward::FillableArray *>(fillablearray);
  try {
    *result = obj->length();
  } catch (...) {
    return 1;
...
uint8_t awkward_FillableArray_bytestring_length(void *fillablearray,
...
uint8_t awkward_FillableArray_string_length(void *fillablearray, const char *x,
                                            int64_t length) {
  awkward::FillableArray *obj =
      reinterpret_cast<awkward::FillableArray *>(fillablearray);
  try {
...
uint8_t awkward_FillableArray_begintuple(void *fillablearray,
                                         int64_t numfields) {
  awkward::FillableArray *obj =
      reinterpret_cast<awkward::FillableArray *>(fillablearray);
  try {
...
uint8_t awkward_FillableArray_beginrecord_fast(void *fillablearray,
                                               const char *name) {
  awkward::FillableArray *obj =
      reinterpret_cast<awkward::FillableArray *>(fillablearray);
  try {
...
uint8_t awkward_FillableArray_beginrecord_check(void *fillablearray,
                                                const char *name) {
  awkward::FillableArray *obj =
      reinterpret_cast<awkward::FillableArray *>(fillablearray);
  try {
...
uint8_t awkward_FillableArray_field_check(void *fillablearray,
                                          const char *key) {
  awkward::FillableArray *obj =
      reinterpret_cast<awkward::FillableArray *>(fillablearray);
  try {
