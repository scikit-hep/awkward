...
namespace awkward {
const std::shared_ptr<Fillable>
TupleFillable::fromempty(const FillableOptions &options) {
  std::shared_ptr<Fillable> out(new TupleFillable(
      options, std::vector<std::shared_ptr<Fillable>>(), -1, false, -1));
  out.get()->setthat(out);
  return out;
}

int64_t TupleFillable::length() const { return length_; }
...
const std::shared_ptr<Content> TupleFillable::snapshot() const {
  if (length_ == -1) {
    return std::shared_ptr<Content>(
        new EmptyArray(Identity::none(), Type::none()));
  } else if (contents_.size() == 0) {
    return std::shared_ptr<Content>(
        new RecordArray(Identity::none(), Type::none(), length_, true));
  } else {
    ...
    return std::shared_ptr<Content>(
        new RecordArray(Identity::none(), Type::none(), contents));
  }
}

bool TupleFillable::active() const { return begun_; }
...
const std::shared_ptr<Fillable> TupleFillable::null() {
  if (!begun_) {
    ...
    throw std::invalid_argument("called 'null' immediately after 'begintuple'; "
...
const std::shared_ptr<Fillable> TupleFillable::boolean(bool x) {
  if (!begun_) {
    ...
    throw std::invalid_argument("called 'boolean' immediately after "
                                "'begintuple'; needs 'index' or 'endtuple'");
  } else if (!contents_[(size_t)nextindex_].get()->active()) {
...
const std::shared_ptr<Fillable> TupleFillable::integer(int64_t x) {
  if (!begun_) {
    ...
    throw std::invalid_argument("called 'integer' immediately after "
                                "'begintuple'; needs 'index' or 'endtuple'");
  } else if (!contents_[(size_t)nextindex_].get()->active()) {
...
const std::shared_ptr<Fillable> TupleFillable::real(double x) {
  if (!begun_) {
    ...
    throw std::invalid_argument("called 'real' immediately after 'begintuple'; "
                                "needs 'index' or 'endtuple'");
  } else if (!contents_[(size_t)nextindex_].get()->active()) {
...
const std::shared_ptr<Fillable>
TupleFillable::string(const char *x, int64_t length, const char *encoding) {
  if (!begun_) {
    std::shared_ptr<Fillable> out = UnionFillable::fromsingle(options_, that_);
    out.get()->string(x, length, encoding);
    return out;
  } else if (nextindex_ == -1) {
    throw std::invalid_argument("called 'string' immediately after "
                                "'begintuple'; needs 'index' or 'endtuple'");
  } else if (!contents_[(size_t)nextindex_].get()->active()) {
...
const std::shared_ptr<Fillable> TupleFillable::beginlist() {
  if (!begun_) {
    ...
    throw std::invalid_argument("called 'beginlist' immediately after "
                                "'begintuple'; needs 'index' or 'endtuple'");
  } else if (!contents_[(size_t)nextindex_].get()->active()) {
...
const std::shared_ptr<Fillable> TupleFillable::endlist() {
  if (!begun_) {
    ...
    throw std::invalid_argument(
        "called 'endlist' immediately after 'begintuple'; needs 'index' or "
        "'endtuple' and then 'beginlist'");
  } else {
...
const std::shared_ptr<Fillable> TupleFillable::begintuple(int64_t numfields) {
  if (length_ == -1) {
    for (int64_t i = 0; i < numfields; i++) {
      contents_.push_back(
          std::shared_ptr<Fillable>(UnknownFillable::fromempty(options_)));
    }
    length_ = 0;
  }
  ...
  if (!begun_ && numfields == (int64_t)contents_.size()) {
    ...
    throw std::invalid_argument("called 'begintuple' immediately after "
                                "'begintuple'; needs 'index' or 'endtuple'");
  } else if (!contents_[(size_t)nextindex_].get()->active()) {
  ...
  if (!begun_) {
    ...
    for (auto content : contents_) {
      ...
      if (content.get()->length() != length_ + 1) {
        throw std::invalid_argument(std::string("tuple index ") +
                                    std::to_string(i) +
...
const std::shared_ptr<Fillable> TupleFillable::beginrecord(const char *name,
                                                           bool check) {
  if (!begun_) {
    std::shared_ptr<Fillable> out = UnionFillable::fromsingle(options_, that_);
    ...
    throw std::invalid_argument("called 'beginrecord' immediately after "
                                "'begintuple'; needs 'index' or 'endtuple'");
  } else if (!contents_[(size_t)nextindex_].get()->active()) {
    maybeupdate(nextindex_,
...
const std::shared_ptr<Fillable> TupleFillable::field(const char *key,
                                                     bool check) {
  if (!begun_) {
    throw std::invalid_argument("called 'field_fast' without 'beginrecord' at "
                                "the same level before it");
  } else if (nextindex_ == -1) {
    throw std::invalid_argument(
        "called 'field_fast' immediately after 'begintuple'; needs 'index' or "
        "'endtuple' and then 'beginrecord'");
  } else {
...
const std::shared_ptr<Fillable> TupleFillable::endrecord() {
  if (!begun_) {
    ...
    throw std::invalid_argument(
        "called 'endrecord' immediately after 'begintuple'; needs 'index' or "
        "'endtuple' and then 'beginrecord'");
  } else {
...
void TupleFillable::maybeupdate(int64_t i,
                                const std::shared_ptr<Fillable> &tmp) {
