...
Int64Fillable::fromempty(const FillableOptions &options) {
  std::shared_ptr<Fillable> out(
      new Int64Fillable(options, GrowableBuffer<int64_t>::empty(options)));
  out.get()->setthat(out);
  return out;
}

int64_t Int64Fillable::length() const { return buffer_.length(); }

void Int64Fillable::clear() { buffer_.clear(); }
...
bool Int64Fillable::active() const { return false; }
...
const std::shared_ptr<Fillable>
Int64Fillable::string(const char *x, int64_t length, const char *encoding) {
  std::shared_ptr<Fillable> out = UnionFillable::fromsingle(options_, that_);
...
const std::shared_ptr<Fillable> Int64Fillable::beginrecord(const char *name,
                                                           bool check) {
  std::shared_ptr<Fillable> out = UnionFillable::fromsingle(options_, that_);
...
const std::shared_ptr<Fillable> Int64Fillable::field(const char *key,
                                                     bool check) {
  throw std::invalid_argument(
