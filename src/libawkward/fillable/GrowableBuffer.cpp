...
namespace awkward {
  template <typename T>
  GrowableBuffer<T> GrowableBuffer<T>::empty(const FillableOptions& options) {
    return GrowableBuffer<T>::empty(options, 0);
  }

  template <typename T>
  GrowableBuffer<T> GrowableBuffer<T>::empty(const FillableOptions& options, int64_t minreserve) {
  ...
  template <typename T>
  GrowableBuffer<T> GrowableBuffer<T>::full(const FillableOptions& options, T value, int64_t length) {
  ...
  template <typename T>
  GrowableBuffer<T> GrowableBuffer<T>::arange(const FillableOptions& options, int64_t length) {
    size_t actual = (size_t)options.initial();
  ...
  template <typename T>
  GrowableBuffer<T>::GrowableBuffer(const FillableOptions& options, std::shared_ptr<T> ptr, int64_t length, int64_t reserved)
      : options_(options)
      , ptr_(ptr)
      , length_(length)
      , reserved_(reserved) { }
