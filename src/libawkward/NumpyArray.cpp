...
const std::string NumpyArray::repr(const std::string indent, const std::string pre, const std::string post) const {
  ...
  for (ssize_t i = 0;  i < shape_.size();  i++) {
    if (i != 0) {
      out << " ";
    }
    out << shape_[i];
  ...
  if (!iscompact()) {
    out << "strides=\"";
    for (ssize_t i = 0;  i < strides_.size();  i++) {
      if (i != 0) {
        out << ", ";
      }
      out << strides_[i];
...
ssize_t NumpyArray::length() const {
  if (isscalar()) {
    return -1;
  }
  else {
...
std::shared_ptr<Content> NumpyArray::get(AtType at) const {
  assert(!isscalar());
  ssize_t byteoffset = byteoffset_ + strides_[0]*at;
...
std::shared_ptr<Content> NumpyArray::slice(AtType start, AtType stop) const {
  assert(!isscalar());
  ssize_t byteoffset = byteoffset_ + strides_[0]*start;
  std::vector<ssize_t> shape;
  shape.push_back(stop - start);
