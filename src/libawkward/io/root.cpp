...
namespace awkward {
void FromROOT_nestedvector_fill(std::vector<GrowableBuffer<int64_t>> &levels,
                                GrowableBuffer<int64_t> &bytepos_tocopy,
                                int64_t &bytepos, const NumpyArray &rawdata,
                                int64_t whichlevel, int64_t itemsize) {
  if (whichlevel == levels.size()) {
    ...
    for (uint32_t i = 0; i < length; i++) {
      FromROOT_nestedvector_fill(levels, bytepos_tocopy, bytepos, rawdata,
                                 whichlevel + 1, itemsize);
    }
...
FromROOT_nestedvector(const Index64 &byteoffsets, const NumpyArray &rawdata,
                      int64_t depth, int64_t itemsize, std::string format,
  ...
  for (int64_t i = 0; i < byteoffsets.length() - 1; i++) {
    ...
    FromROOT_nestedvector_fill(levels, bytepos_tocopy, bytepos, rawdata, 0,
                               itemsize);
  ...
  std::shared_ptr<void> ptr(
      ...
      awkward::util::array_deleter<uint8_t>());
  ...
  for (int64_t i = 0; i < bytepos_tocopy.length(); i++) {
    ...
    std::memcpy(&toptr[(ssize_t)(i * itemsize)], &fromptr[offset + bytepos],
                (size_t)itemsize);
  }

  std::vector<ssize_t> shape = {(ssize_t)bytepos_tocopy.length()};
  std::vector<ssize_t> strides = {(ssize_t)itemsize};
  std::shared_ptr<Content> out(new NumpyArray(Identity::none(), Type::none(),
                                              ptr, shape, strides, 0,
                                              (ssize_t)itemsize, format));

  for (int64_t i = depth - 1; i >= 0; i--) {
    out = std::shared_ptr<Content>(new ListOffsetArray64(
        Identity::none(), Type::none(), levels[(size_t)i].toindex(), out));
  }
  return out;
}
