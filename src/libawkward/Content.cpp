...
namespace awkward {
  ...
  const std::shared_ptr<Content> Content::reduce(const Reducer& reducer, int64_t axis, bool mask, bool keepdims, bool semigroup) const {
    ...
    std::shared_ptr<Content> next = reduce_next(reducer, negaxis, parents, 1, mask, keepdims, semigroup);
