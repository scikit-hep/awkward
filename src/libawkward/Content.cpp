...
namespace awkward {
bool Content::isscalar() const { return false; }

const std::shared_ptr<Identity> Content::id() const { return id_; }
  ...
  if (type_.get() == nullptr) {
    if (isscalar()) {
      ...
      return std::shared_ptr<Type>(
          new ArrayType(Type::Parameters(), innertype(false), length()));
    }
  } else {
    return std::shared_ptr<Type>(
        new ArrayType(Type::Parameters(), type_, length()));
  }
}
  ...
  if (ArrayType *raw = dynamic_cast<ArrayType *>(type.get())) {
    if (raw->length() != length()) {
      throw std::invalid_argument(
          std::string(
              "provided ArrayType is incompatible with length of array: ") +
          std::to_string(raw->length()) + std::string(" versus ") +
...
bool Content::isbare() const { return type_.get() == nullptr; }
...
const std::shared_ptr<Type> Content::baretype() const {
  if (isscalar()) {
    ...
    return std::shared_ptr<Type>(
        new ArrayType(Type::Parameters(), innertype(true), length()));
  }
}
...
void Content::tojson(FILE *destination, bool pretty, int64_t maxdecimals,
...
const std::shared_ptr<Content> Content::getitem(const Slice &where) const {
  std::shared_ptr<Content> next(new RegularArray(Identity::none(), Type::none(),
                                                 shallow_copy(), length()));
  ...
  std::shared_ptr<Content> out =
      next.get()->getitem_next(nexthead, nexttail, nextadvanced);
...
Content::getitem_next(const std::shared_ptr<SliceItem> head, const Slice &tail,
...
const std::shared_ptr<Content>
Content::getitem_next(const SliceEllipsis &ellipsis, const Slice &tail,
                      const Index64 &advanced) const {
  ...
  if (tail.length() == 0 ||
      (mindepth - 1 == tail.dimlength() && maxdepth - 1 == tail.dimlength())) {
    ...
    std::vector<std::shared_ptr<SliceItem>> items = {
        std::shared_ptr<SliceItem>(new SliceEllipsis())};
    items.insert(items.end(), tailitems.begin(), tailitems.end());
    std::shared_ptr<SliceItem> nexthead(
...
const std::shared_ptr<Content>
Content::getitem_next(const SliceNewAxis &newaxis, const Slice &tail,
                      const Index64 &advanced) const {
  std::shared_ptr<SliceItem> nexthead = tail.head();
  Slice nexttail = tail.tail();
  return std::shared_ptr<Content>(
      new RegularArray(Identity::none(), Type::none(),
                       getitem_next(nexthead, nexttail, advanced), 1));
}

const std::shared_ptr<Content>
Content::getitem_next(const SliceField &field, const Slice &tail,
                      const Index64 &advanced) const {
  std::shared_ptr<SliceItem> nexthead = tail.head();
  Slice nexttail = tail.tail();
...
const std::shared_ptr<Content>
Content::getitem_next(const SliceFields &fields, const Slice &tail,
                      const Index64 &advanced) const {
  std::shared_ptr<SliceItem> nexthead = tail.head();
  Slice nexttail = tail.tail();
...
const std::shared_ptr<Content>
Content::getitem_next_array_wrap(const std::shared_ptr<Content> outcontent,
                                 const std::vector<int64_t> &shape) const {
  std::shared_ptr<Content> out(
      new RegularArray(Identity::none(), Type::none(), outcontent,
                       (int64_t)shape[shape.size() - 1]));
  for (int64_t i = (int64_t)shape.size() - 2; i >= 0; i--) {
    out = std::shared_ptr<Content>(new RegularArray(
        Identity::none(), Type::none(), out, (int64_t)shape[(size_t)i]));
  }
  return out;
}
