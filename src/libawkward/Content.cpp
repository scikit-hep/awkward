...
namespace awkward {
  ...
  const std::shared_ptr<Content> Content::getitem(const Slice& where) const {
    ...
    *nextstops.ptr().get() = length();
    ...
    std::shared_ptr<Content> out = next.getitem_next(nexthead, nexttail, nextadvanced, true);
    return out.get()->getitem_at(0);
  }

  const std::shared_ptr<Content> Content::getitem_ellipsis(const Slice& tail, const Index64& advanced, bool fake) const {
    ...
    if (tail.length() == 0  ||  (mindepth - 1 == tail.dimlength()  &&  maxdepth - 1 == tail.dimlength())) {
      ...
      return getitem_next(nexthead, nexttail, advanced, fake);
    }
  ...
  const std::shared_ptr<Content> Content::getitem_newaxis(const Slice& tail, const Index64& advanced, bool fake) const {
