...
namespace awkward {
std::string OptionType::tostring_part(std::string indent, std::string pre,
  ...
  if (parameters_.size() == 0) {
    if (dynamic_cast<ListType *>(type_.get()) != nullptr ||
      ...
      out << indent << pre << "?" << type_.get()->tostring_part("", "", "")
          << post;
    }
  } else {
    out << indent << pre << "option["
...
bool OptionType::equal(const std::shared_ptr<Type> other,
...
std::shared_ptr<Type> OptionType::level() const { return type_.get()->level(); }

std::shared_ptr<Type> OptionType::inner() const { return type_.get()->inner(); }

std::shared_ptr<Type> OptionType::inner(const std::string &key) const {
  throw std::runtime_error("FIXME: OptionType::inner(key)");
}

int64_t OptionType::numfields() const { return type_.get()->numfields(); }
...
OptionType::keyaliases(int64_t fieldindex) const {
  return type_.get()->keyaliases(fieldindex);
}

const std::vector<std::string>
OptionType::keyaliases(const std::string &key) const {
