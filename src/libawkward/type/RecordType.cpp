...
namespace awkward {
std::string RecordType::tostring_part(std::string indent, std::string pre,
...
const std::shared_ptr<Type> RecordType::shallow_copy() const {
  return std::shared_ptr<Type>(
      new RecordType(parameters_, types_, lookup_, reverselookup_));
}

bool RecordType::equal(const std::shared_ptr<Type> other,
  ...
  if (RecordType *t = dynamic_cast<RecordType *>(other.get())) {
    ...
    if (reverselookup_.get() == nullptr) {
      ...
      for (auto pair : *lookup_.get()) {
        ...
        if (!field((int64_t)pair.second)
                 .get()
                 ->equal(t->field(otherindex), check_parameters)) {
          return false;
        }
      }
      return true;
...
std::shared_ptr<Type> RecordType::level() const { return shallow_copy(); }
...
int64_t RecordType::numfields() const { return (int64_t)types_.size(); }
  ...
  if (lookup_.get() != nullptr) {
    ...
    if (out != -1 && out >= numfields()) {
      throw std::invalid_argument(
          ...
          std::string(" for RecordType with only " +
  ...
  if (out == -1) {
    try {
      ...
      throw std::invalid_argument(std::string("key \"") + key +
                                  std::string("\" is not in RecordType"));
    }
    if (out >= numfields()) {
      throw std::invalid_argument(
...
const std::string RecordType::key(int64_t fieldindex) const {
  if (fieldindex >= numfields()) {
    throw std::invalid_argument(
        ...
        std::string(" for RecordType with only " + std::to_string(numfields()) +
                    std::string(" fields")));
  }
  if (reverselookup_.get() != nullptr) {
...
RecordType::keyaliases(int64_t fieldindex) const {
...
const std::vector<std::string>
RecordType::keyaliases(const std::string &key) const {
...
RecordType::fielditems() const {
  ...
  if (reverselookup_.get() == nullptr) {
    ...
    for (size_t j = 0; j < cols; j++) {
      out.push_back(std::pair<std::string, std::shared_ptr<Type>>(
          std::to_string(j), types_[j]));
    }
  } else {
    size_t cols = types_.size();
    for (size_t j = 0; j < cols; j++) {
      out.push_back(std::pair<std::string, std::shared_ptr<Type>>(
          reverselookup_.get()->at(j), types_[j]));
    }
  }
  return out;
}
