...
namespace awkward {
std::string RegularType::tostring_part(std::string indent, std::string pre,
...
bool RegularType::equal(const std::shared_ptr<Type> other,
...
std::shared_ptr<Type> RegularType::level() const { return shallow_copy(); }

std::shared_ptr<Type> RegularType::inner() const { return type_; }

std::shared_ptr<Type> RegularType::inner(const std::string &key) const {
  throw std::runtime_error("FIXME: RegularType::inner(key)");
}

int64_t RegularType::numfields() const { return type_.get()->numfields(); }
...
RegularType::keyaliases(int64_t fieldindex) const {
  return type_.get()->keyaliases(fieldindex);
}

const std::vector<std::string>
RegularType::keyaliases(const std::string &key) const {
...
const std::shared_ptr<Type> RegularType::type() const { return type_; }

int64_t RegularType::size() const { return size_; }
} // namespace awkward
