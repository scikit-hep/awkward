...
namespace awkward {
std::string ListType::tostring_part(std::string indent, std::string pre,
...
bool ListType::equal(const std::shared_ptr<Type> other,
...
std::shared_ptr<Type> ListType::level() const { return shallow_copy(); }

std::shared_ptr<Type> ListType::inner() const { return type_; }

std::shared_ptr<Type> ListType::inner(const std::string &key) const {
  throw std::runtime_error("FIXME: ListType::inner(key)");
}

int64_t ListType::numfields() const { return type_.get()->numfields(); }
...
ListType::keyaliases(const std::string &key) const {
...
const std::shared_ptr<Type> ListType::type() const { return type_; }
} // namespace awkward
