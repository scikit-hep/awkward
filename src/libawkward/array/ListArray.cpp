...
namespace awkward {
template <typename T> const std::string ListArrayOf<T>::classname() const {
...
void ListArrayOf<int32_t>::setid(const std::shared_ptr<Identity> id) {
  if (id.get() == nullptr) {
    ...
    if (length() != id.get()->length()) {
      util::handle_error(failure("content and its id must have the same length",
                                 kSliceNone, kSliceNone),
                         classname(), id_.get());
    }
    ...
    if (Identity32 *rawid = dynamic_cast<Identity32 *>(bigid.get())) {
      Identity32 *rawsubid =
          new Identity32(Identity::newref(), rawid->fieldloc(),
                         rawid->width() + 1, content_.get()->length());
      std::shared_ptr<Identity> subid(rawsubid);
      struct Error err = awkward_identity32_from_listarray32(
          rawsubid->ptr().get(), rawid->ptr().get(), starts_.ptr().get(),
          stops_.ptr().get(), rawid->offset(), starts_.offset(),
          stops_.offset(), content_.get()->length(), length(), rawid->width());
      util::handle_error(err, classname(), id_.get());
      content_.get()->setid(subid);
    } else if (Identity64 *rawid = dynamic_cast<Identity64 *>(bigid.get())) {
      Identity64 *rawsubid =
          new Identity64(Identity::newref(), rawid->fieldloc(),
                         rawid->width() + 1, content_.get()->length());
      std::shared_ptr<Identity> subid(rawsubid);
...
template <typename T> void ListArrayOf<T>::setid() {
  if (length() <= kMaxInt32) {
    Identity32 *rawid =
        new Identity32(Identity::newref(), Identity::FieldLoc(), 1, length());
    ...
    Identity64 *rawid =
        new Identity64(Identity::newref(), Identity::FieldLoc(), 1, length());
    std::shared_ptr<Identity> newid(rawid);
...
template <typename T>
const std::string ListArrayOf<T>::tostring_part(const std::string indent,
  ...
  out << starts_.tostring_part(indent + std::string("    "), "<starts>",
                               "</starts>\n");
  out << stops_.tostring_part(indent + std::string("    "), "<stops>",
                              "</stops>\n");
...
template <typename T> void ListArrayOf<T>::tojson_part(ToJson &builder) const {
  int64_t len = length();
  builder.beginlist();
...
const std::shared_ptr<Type> ListArrayOf<T>::innertype(bool bare) const {
  if (bare || content_.get()->isbare()) {
    return std::shared_ptr<Type>(
        new ListType(Type::Parameters(), content_.get()->innertype(bare)));
  } else {
    return std::shared_ptr<Type>(new ListType(
        Type::Parameters(), content_.get()->type().get()->nolength()));
  }
}
...
void ListArrayOf<T>::settype_part(const std::shared_ptr<Type> type) {
  if (accepts(type)) {
    ...
    throw std::invalid_argument(
        std::string("provided type is incompatible with array: ") +
        ArrayType(Type::Parameters(), type, length()).compare(baretype()));
  }
}
...
template <typename T> int64_t ListArrayOf<T>::length() const {
...
const std::shared_ptr<Content> ListArrayOf<T>::shallow_copy() const {
  return std::shared_ptr<Content>(
      new ListArrayOf<T>(id_, type_, starts_, stops_, content_));
}

template <typename T> void ListArrayOf<T>::check_for_iteration() const {
  ...
  if (id_.get() != nullptr && id_.get()->length() < starts_.length()) {
    util::handle_error(failure("len(id) < len(array)", kSliceNone, kSliceNone),
                       id_.get()->classname(), nullptr);
  }
}
...
ListArrayOf<T>::getitem_at_nowrap(int64_t at) const {
  ...
  if (start < 0) {
    util::handle_error(failure("starts[i] < 0", kSliceNone, at), classname(),
                       id_.get());
  }
  if (start > stop) {
    util::handle_error(failure("starts[i] > stops[i]", kSliceNone, at),
                       classname(), id_.get());
  }
  if (stop > lencontent) {
    util::handle_error(
        failure("starts[i] != stops[i] and stops[i] > len(content)", kSliceNone,
                at),
        classname(), id_.get());
  }
...
ListArrayOf<T>::getitem_range(int64_t start, int64_t stop) const {
  ...
  awkward_regularize_rangeslice(&regular_start, &regular_stop, true,
                                start != Slice::none(), stop != Slice::none(),
                                starts_.length());
...
ListArrayOf<T>::getitem_range_nowrap(int64_t start, int64_t stop) const {
  ...
  return std::shared_ptr<Content>(
      new ListArrayOf<T>(id, type_, starts_.getitem_range_nowrap(start, stop),
                         stops_.getitem_range_nowrap(start, stop), content_));
}

template <typename T>
const std::shared_ptr<Content>
ListArrayOf<T>::getitem_field(const std::string &key) const {
  return std::shared_ptr<Content>(new ListArrayOf<T>(
      id_, Type::none(), starts_, stops_, content_.get()->getitem_field(key)));
}
...
ListArrayOf<T>::getitem_fields(const std::vector<std::string> &keys) const {
  ...
  return std::shared_ptr<Content>(new ListArrayOf<T>(
      id_, type, starts_, stops_, content_.get()->getitem_fields(keys)));
}
...
ListArrayOf<T>::carry(const Index64 &carry) const {
  ...
  struct Error err = util::awkward_listarray_getitem_carry_64<T>(
      nextstarts.ptr().get(), nextstops.ptr().get(), starts_.ptr().get(),
      stops_.ptr().get(), carry.ptr().get(), starts_.offset(), stops_.offset(),
      lenstarts, carry.length());
  util::handle_error(err, classname(), id_.get());
  std::shared_ptr<Identity> id(nullptr);
  if (id_.get() != nullptr) {
  ...
  return std::shared_ptr<Content>(
      new ListArrayOf<T>(id, type_, nextstarts, nextstops, content_));
}
...
template <typename T> int64_t ListArrayOf<T>::numfields() const {
...
ListArrayOf<T>::keyaliases(int64_t fieldindex) const {
  return content_.get()->keyaliases(fieldindex);
}

template <typename T>
const std::vector<std::string>
ListArrayOf<T>::keyaliases(const std::string &key) const {
...
template <typename T>
const std::shared_ptr<Content>
ListArrayOf<T>::getitem_next(const SliceAt &at, const Slice &tail,
  ...
  struct Error err = util::awkward_listarray_getitem_next_at_64<T>(
      nextcarry.ptr().get(), starts_.ptr().get(), stops_.ptr().get(), lenstarts,
      starts_.offset(), stops_.offset(), at.at());
  util::handle_error(err, classname(), id_.get());
...
template <typename T>
const std::shared_ptr<Content>
ListArrayOf<T>::getitem_next(const SliceRange &range, const Slice &tail,
                             const Index64 &advanced) const {
  int64_t lenstarts = starts_.length();
  if (stops_.length() < lenstarts) {
    util::handle_error(
  ...
  struct Error err1 = util::awkward_listarray_getitem_next_range_carrylength<T>(
      &carrylength, starts_.ptr().get(), stops_.ptr().get(), lenstarts,
      starts_.offset(), stops_.offset(), start, stop, step);
  ...
  struct Error err2 = util::awkward_listarray_getitem_next_range_64<T>(
      nextoffsets.ptr().get(), nextcarry.ptr().get(), starts_.ptr().get(),
      stops_.ptr().get(), lenstarts, starts_.offset(), stops_.offset(), start,
      stop, step);
  util::handle_error(err2, classname(), id_.get());
  std::shared_ptr<Content> nextcontent = content_.get()->carry(nextcarry);

  if (advanced.length() == 0) {
    return std::shared_ptr<Content>(new ListOffsetArrayOf<T>(
        id_, type_, nextoffsets,
    ...
    struct Error err1 = util::awkward_listarray_getitem_next_range_counts_64<T>(
        &total, nextoffsets.ptr().get(), lenstarts);
    util::handle_error(err1, classname(), id_.get());
    Index64 nextadvanced(total);
    struct Error err2 =
        util::awkward_listarray_getitem_next_range_spreadadvanced_64<T>(
            nextadvanced.ptr().get(), advanced.ptr().get(),
            nextoffsets.ptr().get(), lenstarts);
    util::handle_error(err2, classname(), id_.get());
    return std::shared_ptr<Content>(new ListOffsetArrayOf<T>(
        id_, type_, nextoffsets,
...
template <typename T>
const std::shared_ptr<Content>
ListArrayOf<T>::getitem_next(const SliceArray64 &array, const Slice &tail,
                             const Index64 &advanced) const {
  int64_t lenstarts = starts_.length();
  if (stops_.length() < lenstarts) {
    util::handle_error(
  ...
  if (advanced.length() == 0) {
    ...
    struct Error err = util::awkward_listarray_getitem_next_array_64<T>(
        nextcarry.ptr().get(), nextadvanced.ptr().get(), starts_.ptr().get(),
        stops_.ptr().get(), flathead.ptr().get(), starts_.offset(),
        stops_.offset(), lenstarts, flathead.length(),
        content_.get()->length());
    util::handle_error(err, classname(), id_.get());
    std::shared_ptr<Content> nextcontent = content_.get()->carry(nextcarry);
    ...
    struct Error err =
        util::awkward_listarray_getitem_next_array_advanced_64<T>(
            nextcarry.ptr().get(), nextadvanced.ptr().get(),
            starts_.ptr().get(), stops_.ptr().get(), flathead.ptr().get(),
            advanced.ptr().get(), starts_.offset(), stops_.offset(), lenstarts,
            flathead.length(), content_.get()->length());
    util::handle_error(err, classname(), id_.get());
    std::shared_ptr<Content> nextcontent = content_.get()->carry(nextcarry);
