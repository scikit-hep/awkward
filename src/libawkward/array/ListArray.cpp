...
namespace awkward {
  ...
  const std::shared_ptr<Content> ListArrayOf<T>::rpad(int64_t length, int64_t axis) const {
    ...
    Index64 self_index(tolength);
    struct Error err2 = util::awkward_listarray_broadcast_toindex_64<T>(
      self_index.ptr().get(),
      starts_.ptr().get(),
      stops_.ptr().get(),
      starts_.length(),
    ...
    if (toaxis == 0) {
      if (length > starts_.length()) {
        ...
        return std::make_shared<ListArrayOf<T>>(identities_, parameters_, starts_, stops_, out);
      }
    }
    else if (toaxis == 1) {
      // FIXME: do not clip
      Index64 outindex(length*starts_.length());
      struct Error err6 = util::awkward_listarray_broadcast_toindex_rpad_64<T>(
        outindex.ptr().get(),
        self_index.ptr().get(),
        starts_.ptr().get(),
        stops_.ptr().get(),
  ...
  const std::shared_ptr<Content> ListArrayOf<T>::rpad_and_clip(int64_t length, int64_t axis) const {
    ...
    if (toaxis == 0) {
      if (length > starts_.length()) {
        ...
        struct Error err5 = util::awkward_listarray_rpad_64<T>(
          starts.ptr().get(),
          stops.ptr().get(),
          starts_.ptr().get(),
          stops_.ptr().get(),
