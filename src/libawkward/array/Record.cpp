...
namespace awkward {
bool Record::isscalar() const { return true; }

const std::string Record::classname() const { return "Record"; }
...
const std::string Record::tostring_part(const std::string indent,
...
void Record::tojson_part(ToJson &builder) const {
  ...
  if (istuple()) {
    keys = std::shared_ptr<RecordArray::ReverseLookup>(
        new RecordArray::ReverseLookup);
...
void Record::settype(const std::shared_ptr<Type> type) {
  if (dynamic_cast<ArrayType *>(type.get())) {
    throw std::invalid_argument("provided ArrayType is incompatible with "
                                "Record because Record is a scalar");
  }
...
void Record::check_for_iteration() const {
  if (array_.id().get() != nullptr && array_.id().get()->length() != 1) {
    util::handle_error(
        failure("len(id) != 1 for scalar Record", kSliceNone, kSliceNone),
        array_.id().get()->classname(), nullptr);
  }
}
...
const std::shared_ptr<Content> Record::getitem_at(int64_t at) const {
  throw std::invalid_argument(
      ...
      util::quote(std::to_string(at), true));
}
...
const std::shared_ptr<Content> Record::getitem_range(int64_t start,
                                                     int64_t stop) const {
  throw std::invalid_argument(
...
int64_t Record::numfields() const { return array_.numfields(); }
...
bool Record::haskey(const std::string &key) const { return array_.haskey(key); }
...
Record::keyaliases(const std::string &key) const {
  return array_.keyaliases(key);
}

const std::vector<std::string> Record::keys() const { return array_.keys(); }
  ...
  if (istuple()) {
    ...
    for (int64_t j = 0; j < cols; j++) {
      out.push_back(std::pair<std::string, std::shared_ptr<Content>>(
          std::to_string(j), array_.field(j).get()->getitem_at_nowrap(at_)));
    }
  } else {
    int64_t cols = numfields();
    for (int64_t j = 0; j < cols; j++) {
      out.push_back(std::pair<std::string, std::shared_ptr<Content>>(
          ...
          array_.field(j).get()->getitem_at_nowrap(at_)));
    }
  }
  return out;
}

const Record Record::astuple() const { return Record(array_.astuple(), at_); }

const std::shared_ptr<Content>
Record::getitem_next(const SliceAt &at, const Slice &tail,
                     const Index64 &advanced) const {
  throw std::runtime_error("undefined operation: Record::getitem_next(at)");
}

const std::shared_ptr<Content>
Record::getitem_next(const SliceRange &range, const Slice &tail,
                     const Index64 &advanced) const {
  throw std::runtime_error("undefined operation: Record::getitem_next(range)");
}

const std::shared_ptr<Content>
Record::getitem_next(const SliceArray64 &array, const Slice &tail,
                     const Index64 &advanced) const {
  throw std::runtime_error("undefined operation: Record::getitem_next(array)");
}

const std::shared_ptr<Content>
Record::getitem_next(const SliceField &field, const Slice &tail,
                     const Index64 &advanced) const {
  throw std::runtime_error("undefined operation: Record::getitem_next(field)");
}

const std::shared_ptr<Content>
Record::getitem_next(const SliceFields &fields, const Slice &tail,
                     const Index64 &advanced) const {
