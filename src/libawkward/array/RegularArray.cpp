...
namespace awkward {
const std::string RegularArray::classname() const { return "RegularArray"; }
  ...
  if (id.get() == nullptr) {
    ...
    if (Identity32 *rawid = dynamic_cast<Identity32 *>(bigid.get())) {
      ...
      struct Error err = awkward_identity32_from_regulararray(
          rawsubid->ptr().get(), rawid->ptr().get(), rawid->offset(), size_,
          content_.get()->length(), length(), rawid->width());
...
const std::string RegularArray::tostring_part(const std::string indent,
...
const std::shared_ptr<Type> RegularArray::innertype(bool bare) const {
  if (bare || content_.get()->isbare()) {
    return std::shared_ptr<Type>(new RegularType(
        Type::Parameters(), content_.get()->innertype(bare), size_));
  } else {
    return std::shared_ptr<Type>(new RegularType(
        Type::Parameters(), content_.get()->type().get()->nolength(), size_));
  }
}
...
bool RegularArray::accepts(const std::shared_ptr<Type> type) {
  if (RegularType *raw =
          dynamic_cast<RegularType *>(type.get()->level().get())) {
...
const std::shared_ptr<Content> RegularArray::shallow_copy() const {
  return std::shared_ptr<Content>(
      new RegularArray(id_, type_, content_, size_));
}
...
const std::shared_ptr<Content> RegularArray::getitem_range(int64_t start,
                                                           int64_t stop) const {
...
RegularArray::getitem_range_nowrap(int64_t start, int64_t stop) const {
  ...
  return std::shared_ptr<Content>(new RegularArray(
      id_, type_,
...
RegularArray::getitem_field(const std::string &key) const {
  return std::shared_ptr<Content>(new RegularArray(
      id_, Type::none(), content_.get()->getitem_field(key), size_));
}
...
RegularArray::getitem_fields(const std::vector<std::string> &keys) const {
  ...
  return std::shared_ptr<Content>(
      new RegularArray(id_, type, content_.get()->getitem_fields(keys), size_));
}
...
const std::shared_ptr<Content> RegularArray::carry(const Index64 &carry) const {
  ...
  struct Error err = awkward_regulararray_getitem_carry_64(
      nextcarry.ptr().get(), carry.ptr().get(), carry.length(), size_);
  util::handle_error(err, classname(), id_.get());
  ...
  return std::shared_ptr<Content>(
      new RegularArray(id, type_, content_.get()->carry(nextcarry), size_));
}
...
int64_t RegularArray::numfields() const { return content_.get()->numfields(); }
...
RegularArray::keyaliases(int64_t fieldindex) const {
  return content_.get()->keyaliases(fieldindex);
}

const std::vector<std::string>
RegularArray::keyaliases(const std::string &key) const {
...
const std::shared_ptr<Content>
RegularArray::getitem_next(const SliceAt &at, const Slice &tail,
  ...
  struct Error err = awkward_regulararray_getitem_next_at_64(
      nextcarry.ptr().get(), at.at(), len, size_);
  util::handle_error(err, classname(), id_.get());
...
const std::shared_ptr<Content>
RegularArray::getitem_next(const SliceRange &range, const Slice &tail,
                           const Index64 &advanced) const {
  int64_t len = length();
  std::shared_ptr<SliceItem> nexthead = tail.head();
  Slice nexttail = tail.tail();
  ...
  awkward_regularize_rangeslice(&regular_start, &regular_stop, range.step() > 0,
                                range.start() != Slice::none(),
                                range.stop() != Slice::none(), size_);
  int64_t nextsize = 0;
  ...
  struct Error err = awkward_regulararray_getitem_next_range_64(
      nextcarry.ptr().get(), regular_start, range.step(), len, size_, nextsize);
  util::handle_error(err, classname(), id_.get());
  ...
  if (type_.get() != nullptr) {
    ...
    outtype = std::shared_ptr<Type>(
        new RegularType(Type::Parameters(), raw->type(), nextsize));
  }
  ...
  if (advanced.length() == 0) {
    ...
    struct Error err =
        awkward_regulararray_getitem_next_range_spreadadvanced_64(
            nextadvanced.ptr().get(), advanced.ptr().get(), len, nextsize);
    util::handle_error(err, classname(), id_.get());
...
const std::shared_ptr<Content>
RegularArray::getitem_next(const SliceArray64 &array, const Slice &tail,
                           const Index64 &advanced) const {
  int64_t len = length();
  std::shared_ptr<SliceItem> nexthead = tail.head();
  Slice nexttail = tail.tail();
  ...
  struct Error err = awkward_regulararray_getitem_next_array_regularize_64(
      regular_flathead.ptr().get(), flathead.ptr().get(), flathead.length(),
      size_);
  util::handle_error(err, classname(), id_.get());
  ...
  if (advanced.length() == 0) {
    ...
    struct Error err = awkward_regulararray_getitem_next_array_64(
        ...
        regular_flathead.ptr().get(), len, regular_flathead.length(), size_);
    util::handle_error(err, classname(), id_.get());
    ...
    struct Error err = awkward_regulararray_getitem_next_array_advanced_64(
        nextcarry.ptr().get(), nextadvanced.ptr().get(), advanced.ptr().get(),
        regular_flathead.ptr().get(), len, regular_flathead.length(), size_);
    util::handle_error(err, classname(), id_.get());
