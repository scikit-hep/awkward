...
namespace awkward {
const std::string RecordArray::classname() const { return "RecordArray"; }

void RecordArray::setid() {
  int64_t len = length();
  if (len <= kMaxInt32) {
    Identity32 *rawid =
        new Identity32(Identity::newref(), Identity::FieldLoc(), 1, len);
    ...
    Identity64 *rawid =
        new Identity64(Identity::newref(), Identity::FieldLoc(), 1, len);
    std::shared_ptr<Identity> newid(rawid);
  ...
  if (id.get() == nullptr) {
    ...
    if (istuple()) {
      for (size_t j = 0; j < contents_.size(); j++) {
        Identity::FieldLoc fieldloc(id.get()->fieldloc().begin(),
                                    id.get()->fieldloc().end());
        fieldloc.push_back(std::pair<int64_t, std::string>(
            id.get()->width() - 1, std::to_string(j)));
      ...
      for (size_t j = 0; j < contents_.size(); j++) {
        ...
        fieldloc.push_back(std::pair<int64_t, std::string>(
            id.get()->width() - 1, reverselookup_.get()->at(j)));
        contents_[j].get()->setid(id.get()->withfieldloc(fieldloc));
      }
    }
  }
...
const std::string RecordArray::tostring_part(const std::string indent,
...
const std::shared_ptr<Type> RecordArray::innertype(bool bare) const {
  ...
  return std::shared_ptr<Type>(
      new RecordType(Type::Parameters(), types, lookup_, reverselookup_));
}
...
const std::shared_ptr<Content> RecordArray::shallow_copy() const {
  if (contents_.size() == 0) {
    return std::shared_ptr<Content>(
        new RecordArray(id_, type_, length(), istuple()));
  } else {
    return std::shared_ptr<Content>(
        new RecordArray(id_, type_, contents_, lookup_, reverselookup_));
  }
}
...
const std::shared_ptr<Content> RecordArray::getitem_range(int64_t start,
                                                          int64_t stop) const {
  if (contents_.size() == 0) {
    ...
    awkward_regularize_rangeslice(&regular_start, &regular_stop, true,
                                  ...
                                  length());
    return std::shared_ptr<Content>(
        new RecordArray(id_, type_, regular_stop - regular_start, istuple()));
  } else {
    ...
    return std::shared_ptr<Content>(
        new RecordArray(id_, type_, contents, lookup_, reverselookup_));
  }
}
...
RecordArray::getitem_range_nowrap(int64_t start, int64_t stop) const {
  if (contents_.size() == 0) {
    return std::shared_ptr<Content>(
        new RecordArray(id_, type_, stop - start, istuple()));
  } else {
    std::vector<std::shared_ptr<Content>> contents;
    for (auto content : contents_) {
...
RecordArray::getitem_fields(const std::vector<std::string> &keys) const {
  ...
  if (type_.get() != nullptr && type_.get()->numfields() != -1 &&
      util::subset(keys, type_.get()->keys())) {
    type = type_;
  }
...
const std::shared_ptr<Content> RecordArray::carry(const Index64 &carry) const {
  if (contents_.size() == 0) {
    ...
    return std::shared_ptr<Content>(
        new RecordArray(id, type_, carry.length(), istuple()));
  } else {
    std::vector<std::shared_ptr<Content>> contents;
    for (auto content : contents_) {
    ...
    return std::shared_ptr<Content>(
        new RecordArray(id, type_, contents, lookup_, reverselookup_));
  }
}
...
int64_t RecordArray::numfields() const { return (int64_t)contents_.size(); }
...
int64_t RecordArray::fieldindex(const std::string &key) const {
  ...
  if (!istuple()) {
    ...
    if (out != -1 && out >= numfields()) {
      throw std::invalid_argument(
          std::string("key \"") + key +
          std::string("\" points to fieldindex ") + std::to_string(out) +
          std::string(" for RecordArray with only " +
                      std::to_string(numfields()) + std::string(" fields")));
    }
  }
  if (out == -1) {
    try {
      ...
      throw std::invalid_argument(std::string("key \"") + key +
                                  std::string("\" is not in RecordArray"));
    }
    if (out >= numfields()) {
      throw std::invalid_argument(
...
RecordArray::keyaliases(int64_t fieldindex) const {
...
const std::vector<std::string>
RecordArray::keyaliases(const std::string &key) const {
...
const std::vector<std::string> RecordArray::keys() const {
  ...
  if (istuple()) {
    ...
    out.insert(out.end(), reverselookup_.get()->begin(),
               reverselookup_.get()->end());
  }
  return out;
}
...
RecordArray::fielditems() const {
  ...
  if (istuple()) {
    size_t cols = contents_.size();
    for (size_t j = 0; j < cols; j++) {
      out.push_back(std::pair<std::string, std::shared_ptr<Content>>(
          std::to_string(j), contents_[j]));
    }
  } else {
    size_t cols = contents_.size();
    for (size_t j = 0; j < cols; j++) {
      out.push_back(std::pair<std::string, std::shared_ptr<Content>>(
          reverselookup_.get()->at(j), contents_[j]));
    }
  }
  return out;
}
...
const RecordArray RecordArray::astuple() const {
  ...
  if (type_.get() != nullptr && type_.get()->numfields() != -1 &&
      util::subset(out.keys(), type_.get()->keys())) {
    out.type_ = type_;
  }
  return out;
}
...
const std::shared_ptr<Content>
RecordArray::getitem_next(const std::shared_ptr<SliceItem> head,
  ...
  if (head.get() == nullptr) {
    ...
    for (auto content : contents_) {
      contents.push_back(
          content.get()->getitem_next(head, emptytail, advanced));
    }
    std::shared_ptr<Type> type = Type::none();
...
const std::shared_ptr<Content>
RecordArray::getitem_next(const SliceAt &at, const Slice &tail,
...
const std::shared_ptr<Content>
RecordArray::getitem_next(const SliceRange &range, const Slice &tail,
                          const Index64 &advanced) const {
  throw std::invalid_argument(
...
const std::shared_ptr<Content>
RecordArray::getitem_next(const SliceArray64 &array, const Slice &tail,
                          const Index64 &advanced) const {
  throw std::invalid_argument(
...
const std::shared_ptr<Content>
RecordArray::getitem_next(const SliceField &field, const Slice &tail,
                          const Index64 &advanced) const {
  std::shared_ptr<SliceItem> nexthead = tail.head();
  Slice nexttail = tail.tail();
...
const std::shared_ptr<Content>
RecordArray::getitem_next(const SliceFields &fields, const Slice &tail,
                          const Index64 &advanced) const {
  std::shared_ptr<SliceItem> nexthead = tail.head();
  Slice nexttail = tail.tail();
