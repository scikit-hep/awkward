...
template <typename T> IndexOf<T> make_starts(const IndexOf<T> &offsets) {
  return IndexOf<T>(offsets.ptr(), offsets.offset(), offsets.length() - 1);
}

template <typename T> IndexOf<T> make_stops(const IndexOf<T> &offsets) {
  ...
  if (id.get() == nullptr) {
    ...
    if (Identity32 *rawid = dynamic_cast<Identity32 *>(bigid.get())) {
      ...
      struct Error err = awkward_identity32_from_listoffsetarray32(
          rawsubid->ptr().get(), rawid->ptr().get(), offsets_.ptr().get(),
          rawid->offset(), offsets_.offset(), content_.get()->length(),
          length(), rawid->width());
...
template <typename T> void ListOffsetArrayOf<T>::setid() {
  if (length() <= kMaxInt32) {
    Identity32 *rawid =
...
ListOffsetArrayOf<T>::tostring_part(const std::string indent,
  ...
  out << offsets_.tostring_part(indent + std::string("    "), "<offsets>",
                                "</offsets>\n");
...
template <typename T> int64_t ListOffsetArrayOf<T>::length() const {
...
const std::shared_ptr<Content> ListOffsetArrayOf<T>::shallow_copy() const {
  return std::shared_ptr<Content>(
      new ListOffsetArrayOf<T>(id_, type_, offsets_, content_));
}

template <typename T> void ListOffsetArrayOf<T>::check_for_iteration() const {
...
ListOffsetArrayOf<T>::getitem_at_nowrap(int64_t at) const {
  ...
  if (start < 0) {
    util::handle_error(failure("offsets[i] < 0", kSliceNone, at), classname(),
                       id_.get());
  }
  if (start > stop) {
    util::handle_error(failure("offsets[i] > offsets[i + 1]", kSliceNone, at),
                       classname(), id_.get());
  }
  if (stop > lencontent) {
    util::handle_error(
        failure(
            "offsets[i] != offsets[i + 1] and offsets[i + 1] > len(content)",
            kSliceNone, at),
        classname(), id_.get());
  }
...
template <typename T>
const std::shared_ptr<Content>
ListOffsetArrayOf<T>::getitem_range_nowrap(int64_t start, int64_t stop) const {
  std::shared_ptr<Identity> id(nullptr);
  if (id_.get() != nullptr) {
    id = id_.get()->getitem_range_nowrap(start, stop);
  }
  return std::shared_ptr<Content>(new ListOffsetArrayOf<T>(
      id, type_, offsets_.getitem_range_nowrap(start, stop + 1), content_));
}
...
ListOffsetArrayOf<T>::getitem_field(const std::string &key) const {
  return std::shared_ptr<Content>(new ListOffsetArrayOf<T>(
      id_, Type::none(), offsets_, content_.get()->getitem_field(key)));
}

template <typename T>
const std::shared_ptr<Content> ListOffsetArrayOf<T>::getitem_fields(
  ...
  return std::shared_ptr<Content>(new ListOffsetArrayOf<T>(
      id_, type, offsets_, content_.get()->getitem_fields(keys)));
}
...
ListOffsetArrayOf<T>::carry(const Index64 &carry) const {
  ...
  struct Error err = util::awkward_listarray_getitem_carry_64<T>(
      nextstarts.ptr().get(), nextstops.ptr().get(), starts.ptr().get(),
      stops.ptr().get(), carry.ptr().get(), starts.offset(), stops.offset(),
      offsets_.length() - 1, carry.length());
  util::handle_error(err, classname(), id_.get());
  std::shared_ptr<Identity> id(nullptr);
  if (id_.get() != nullptr) {
...
template <typename T> int64_t ListOffsetArrayOf<T>::numfields() const {
...
ListOffsetArrayOf<T>::keyaliases(int64_t fieldindex) const {
  return content_.get()->keyaliases(fieldindex);
}

template <typename T>
const std::vector<std::string>
ListOffsetArrayOf<T>::keyaliases(const std::string &key) const {
...
template <typename T>
const std::shared_ptr<Content>
ListOffsetArrayOf<T>::getitem_next(const SliceAt &at, const Slice &tail,
  ...
  struct Error err = util::awkward_listarray_getitem_next_at_64<T>(
      nextcarry.ptr().get(), starts.ptr().get(), stops.ptr().get(), lenstarts,
      starts.offset(), stops.offset(), at.at());
  util::handle_error(err, classname(), id_.get());
...
template <typename T>
const std::shared_ptr<Content>
ListOffsetArrayOf<T>::getitem_next(const SliceRange &range, const Slice &tail,
                                   const Index64 &advanced) const {
  int64_t lenstarts = offsets_.length() - 1;
  IndexOf<T> starts = make_starts(offsets_);
  IndexOf<T> stops = make_stops(offsets_);
  ...
  struct Error err1 = util::awkward_listarray_getitem_next_range_carrylength<T>(
      &carrylength, starts.ptr().get(), stops.ptr().get(), lenstarts,
      starts.offset(), stops.offset(), start, stop, step);
  ...
  struct Error err2 = util::awkward_listarray_getitem_next_range_64<T>(
      nextoffsets.ptr().get(), nextcarry.ptr().get(), starts.ptr().get(),
      stops.ptr().get(), lenstarts, starts.offset(), stops.offset(), start,
      stop, step);
...
template <typename T>
const std::shared_ptr<Content>
ListOffsetArrayOf<T>::getitem_next(const SliceArray64 &array, const Slice &tail,
                                   const Index64 &advanced) const {
  int64_t lenstarts = offsets_.length() - 1;
  IndexOf<T> starts = make_starts(offsets_);
  IndexOf<T> stops = make_stops(offsets_);
  ...
  if (advanced.length() == 0) {
    ...
    struct Error err = util::awkward_listarray_getitem_next_array_64<T>(
        nextcarry.ptr().get(), nextadvanced.ptr().get(), starts.ptr().get(),
        stops.ptr().get(), flathead.ptr().get(), starts.offset(),
        stops.offset(), lenstarts, flathead.length(), content_.get()->length());
    util::handle_error(err, classname(), id_.get());
    std::shared_ptr<Content> nextcontent = content_.get()->carry(nextcarry);
    ...
    struct Error err =
        util::awkward_listarray_getitem_next_array_advanced_64<T>(
            nextcarry.ptr().get(), nextadvanced.ptr().get(), starts.ptr().get(),
            stops.ptr().get(), flathead.ptr().get(), advanced.ptr().get(),
            starts.offset(), stops.offset(), lenstarts, flathead.length(),
            content_.get()->length());
    util::handle_error(err, classname(), id_.get());
    std::shared_ptr<Content> nextcontent = content_.get()->carry(nextcarry);
