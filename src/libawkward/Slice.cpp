...
namespace awkward {
const std::string SliceAt::tostring() const { return std::to_string(at_); }
...
const std::string SliceEllipsis::tostring() const { return std::string("..."); }

const std::string SliceNewAxis::tostring() const {
  return std::string("newaxis");
}

template <typename T> const std::string SliceArrayOf<T>::tostring() const {
  return std::string("array(") + tostring_part() + std::string(")");
}

template <typename T> const std::string SliceArrayOf<T>::tostring_part() const {
  std::stringstream out;
  out << "[";
  if (shape_.size() == 1) {
    if (shape_[0] < 6) {
    ...
    if (shape_[0] < 6) {
      for (int64_t i = 0; i < shape_[0]; i++) {
        ...
        IndexOf<T> index(index_.ptr(), index_.offset() + i * strides_[0],
                         shape_[1]);
...
template <typename T> const IndexOf<T> SliceArrayOf<T>::ravel() const {
  int64_t length = 1;
  ...
  if (std::is_same<T, int64_t>::value) {
    awkward_slicearray_ravel_64(index.ptr().get(), index_.ptr().get(), ndim(),
...
int64_t Slice::length() const { return (int64_t)items_.size(); }
...
template <typename T> void Slice::append(const SliceArrayOf<T> &item) {
  items_.push_back(item.shallow_copy());
}
  ...
  if (shape.size() != 0) {
    for (size_t i = 0; i < items_.size(); i++) {
      if (SliceAt *at = dynamic_cast<SliceAt *>(items_[i].get())) {
        ...
        items_[i] =
            std::shared_ptr<SliceItem>(new SliceArray64(index, shape, strides));
        ...
        items_[i] = std::shared_ptr<SliceItem>(
            new SliceArray64(array->index(), shape, strides));
      }
    }
