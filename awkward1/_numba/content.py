...
def lower_getitem_tuple(context, builder, sig, args):
    ...
    nexttpe = awkward1._numba.array.regulararray.RegularArrayType(arraytpe, numba.none, numba.none)   # FIXME: Type::none()
    outtpe = nexttpe.getitem_next(wheretpe, False)
    if outtpe.lower_getitem_nothing is None:
        nexttpe = awkward1._numba.array.listarray.ListArrayType(util.index64tpe, util.index64tpe, arraytpe, numba.none, numba.none)   # FIXME: Type::none()
        outtpe = nexttpe.getitem_next(wheretpe, False)
